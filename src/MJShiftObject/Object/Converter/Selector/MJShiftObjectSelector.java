		...
		m_selector.execute(String.format("select * from %s where objid=?", MJEShiftDBName.CHARACTERS.get_table_name(m_server_identity)), new SelectorHandler(){
			...
			public void result(ResultSet rs) throws Exception {
				if(rs.next()){
					...
					pc.setSurvivalGauge(rs.getInt("SurvivalGauge")); // 生存量表
...
//					pc.setBuffTime(rs.getInt("Exp_Buff_Time"));
					pc.refresh();
					pc.setMoveSpeed(0);
					pc.setBraveSpeed(0);
					pc.setGmInvis(false);
				...
				new SelectorHandler(){
					...
					public void result(ResultSet rs) throws Exception {
						while (rs.next()) {
							...
							if (template == null) {
								System.out.println(String.format("伺服器移動過程中遺失了物品，因此取消該物品。ID：%d", item_id));
								continue;
							}
		...
		m_selector.execute(
			...
			public void result(ResultSet rs) throws Exception {
				while(rs.next()){
					...
					tInfo.character_id = object_id;	// 強制使用角色信息
