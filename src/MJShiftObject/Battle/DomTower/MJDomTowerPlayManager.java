...
public class MJDomTowerPlayManager extends MJShiftBattlePlayManager<MJDomTowerTeamInfo> implements 
	...
	public MJDomTowerPlayManager(long ended_millis, boolean is_local_server){
		...
		for(int i=4; i<=6; ++i)
			m_teams.put(i + REVISION_TEAM_ID, MJDomTowerTeamInfo.newInstance(i + REVISION_TEAM_ID));
	...
	protected String get_default_auto_message_db_name(){
		return MJShiftBattleMessage.DBNAME_DOM_TOWER;
	}
	@Override
		...
		GeneralThreadPool.getInstance().execute(new Runnable(){
			...
			public void run() {
				try{
					...
					ServerBasePacket[] pcks = MJPacketFactory.create_duplicate_message_packets("傲慢之塔即將開始。");
	...
	public void on_enter(final L1PcInstance pc){
		...
		MJDomTowerTeamInfo tInfo = m_players_to_team.get(object_id);
		if(tInfo == null){
			int team_id = 0;
			...
			if(!IntRange.includes(team_id, 4, 6)){
				System.out.println(String.format("%s 無法找到您的團隊ID。(ID: %d, 伺服器標識: %s)", pc.getName(), team_id, pc.getNetConnection().get_server_identity()));
				return;
			}
	...
	public void on_closed(){
		...
		for(MJDomTowerTeamInfo bInfo : m_teams.values())
			bInfo.clear();
	...
	public boolean on_teleported(L1PcInstance owner, int next_x, int next_y, int map_id, int old_mapid){
		...
		owner.sendPackets(MJPacketFactory.create_duplicate_message_packets(m_is_ready ? "目前是等待時間。" : "傲慢之塔正在進行中。"), true);
		return false;
	}
	@Override
