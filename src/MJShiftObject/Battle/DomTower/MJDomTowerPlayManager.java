 ...
 public class MJDomTowerPlayManager
   extends MJShiftBattlePlayManager<MJDomTowerTeamInfo>
   implements MJIWalkFilterHandler, MJIMonsterKillHandler, MJICharacterKillHandler, MJITeleportHandler, MJIPickupHandler, MJIRestartHandler, MJIDeadRestartHandler
 {
   ...
   public MJDomTowerPlayManager(long ended_millis, boolean is_local_server) {
     super(ended_millis, is_local_server);
     for (int i = 4; i <= 6; i++)
       this.m_teams.put(Integer.valueOf(i + REVISION_TEAM_ID), MJDomTowerTeamInfo.newInstance(i + REVISION_TEAM_ID));
   ...
   protected int get_default_ready_seconds() {
     return MJShiftBattleArgs.DOMTOWER_READY_SECONDS;
   }

   protected int get_default_auto_message_remain_seconds() {
     return MJShiftBattleArgs.DOMTOWER_AUTO_MESSAGE_REMAIN_SECONDS;
   }

   protected String get_default_auto_message_db_name() {
     return "server_battle_message_domtower";
   }

   protected boolean get_default_is_auto_sequence_message() {
     return MJShiftBattleArgs.DOMTOWER_IS_AUTO_SEQUENCE_MESSAGE;
   }
     ...
     GeneralThreadPool.getInstance().execute(new Runnable()
         {
           public void run() {
             try {
               SC_EVENT_COUNTDOWN_NOTI_PACKET noti = MJDomTowerPlayManager.this.create_countdown();
               ServerBasePacket[] pcks = MJPacketFactory.create_duplicate_message_packets("統治之塔開始。");
   ...
   public void on_enter(L1PcInstance pc) {
     ...
     MJDomTowerTeamInfo tInfo = (MJDomTowerTeamInfo)this.m_players_to_team.get(Integer.valueOf(object_id));
     if (tInfo == null) {
       int team_id = 0;
       ...
       tInfo = (MJDomTowerTeamInfo)this.m_teams.get(Integer.valueOf(team_id + REVISION_TEAM_ID));
   ...
   public void on_closed() {
     ...
     for (MJDomTowerTeamInfo bInfo : this.m_teams.values())
       bInfo.clear();
   ...
   public boolean on_teleported(L1PcInstance owner, int next_x, int next_y, int map_id, int old_mapid) {
     ...
     if (cInfo == null || !(cInfo instanceof MJDomTowerCharacterInfo)) {
       return false;
     }
     ...
     owner.sendPackets(MJPacketFactory.create_duplicate_message_packets(this.m_is_ready ? "這是目前的等待時間。" : "《統治之塔》正在進行中。"), true);
     return false;
   }
   ...
   public boolean on_pickup(L1PcInstance pc, L1ItemInstance item, int amount) {
     ...
     if (bInfo == null || !(bInfo instanceof MJDomTowerCharacterInfo)) {
       return false;
     }
   ...
   public void on_death_restarted(final L1PcInstance pc) {
     ...
     GeneralThreadPool.getInstance().schedule(new Runnable()
         {
           public void run() {
             MJDomTowerPlayManager.this.on_teleported(pc, pc.getX(), pc.getY(), pc.getMapId(), pc.getMapId());
           }
         }10L);
   }
