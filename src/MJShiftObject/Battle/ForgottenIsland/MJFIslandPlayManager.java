...
public class MJFIslandPlayManager extends MJShiftBattlePlayManager<MJFIslandTeamInfo> implements 
	...
	public MJFIslandPlayManager(long ended_millis, boolean is_local_server){
		...
		for(int i=4; i<=6; ++i)
			m_teams.put(i + REVISION_TEAM_ID, MJFIslandTeamInfo.newInstance(i + REVISION_TEAM_ID));
	...
	protected String get_default_auto_message_db_name(){
		return MJShiftBattleMessage.DBNAME_FOR_ISLAND;
	}
	@Override
		...
		GeneralThreadPool.getInstance().execute(new Runnable(){
			...
			public void run() {
				try{
					...
					ServerBasePacket[] pcks = MJPacketFactory.create_duplicate_message_packets("被遺忘的島嶼即將開始。");
	...
	public void on_enter(final L1PcInstance pc){
		...
		MJFIslandTeamInfo tInfo = m_players_to_team.get(object_id);
		if(tInfo == null){
			int team_id = 0;
	...
	public void on_closed(){
		...
		for(MJFIslandTeamInfo bInfo : m_teams.values())
			bInfo.clear();
	...
	public boolean on_teleported(L1PcInstance owner, int next_x, int next_y, int map_id, int old_mapid){
		...
		owner.sendPackets(MJPacketFactory.create_duplicate_message_packets(m_is_ready ? "目前是等待時間。" : "被遺忘的島嶼正在進行中。"), true);
		return false;
	}
	@Override
