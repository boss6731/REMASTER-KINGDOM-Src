...
public class AutoSaveThread implements Runnable {
	...
	public AutoSaveThread() {
		_saveCharTime = Config.Synchronization.AutosaveInterval;
		_saveInvenTime = Config.Synchronization.AutosaveIntervalOfInventory;
		_saveExpTime = Config.Synchronization.AutosaveExpInterval;
		_saveFatigueime = FatigueProperty.getInstance().get_fatigue_point_stackable_millis();
	}
		...
		while (true) {
			try {
				...
				for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
					...
					if (_saveFatigueime < System.currentTimeMillis() - pc.getlastSavedTime_Fatigue()) {
					...
					// 캐릭터 경험치 정보 저장
					if (_saveExpTime * 1000 < System.currentTimeMillis() - pc.getlastSavedTime_exp()) {
