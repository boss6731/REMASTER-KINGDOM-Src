...
public class MJNCoinAdenaInfo {
	public static List<MJNCoinAdenaInfo> load_page(int page, int page_size, int id_offset){
		List<MJNCoinAdenaInfo> adenas_info = new ArrayList<MJNCoinAdenaInfo>(page_size);
		Selector.exec("select * from ncoin_trade_adena where complete_date is null and trade_id<? order by trade_id desc limit ?,?", new SelectorHandler(){
			...
			public void result(ResultSet rs) throws Exception {
				while(rs.next()){
					MJNCoinAdenaInfo aInfo = newInstance(rs);
	...
	public static List<MJNCoinAdenaInfo> load_page(int page, int page_size){
		List<MJNCoinAdenaInfo> adenas_info = new ArrayList<MJNCoinAdenaInfo>(page_size);
	...
	public static void do_store(final MJNCoinAdenaInfo aInfo){
		Updator.exec("insert into ncoin_trade_adena set "
				+ "trade_id=?, character_object_id=?, character_name=?, account_name=?, "
				+ "adena_amount=?, ncoin_value=?, commission=?, commission_ncoin=?, drain_ncoin=?, ncoin_per_adena=?, ncoin_per_adena_unit=?, generate_date=?", new Handler(){
					@Override
					public void handle(PreparedStatement pstm) throws Exception {
						int idx = 0;
						...
						pstm.setString(++idx, String.format("%.2f", aInfo.get_commission()));
	...
	public static MJNCoinAdenaInfo from_trade_id(final int trade_id) {
		final MJObjectWrapper<MJNCoinAdenaInfo> wrapper = new MJObjectWrapper<MJNCoinAdenaInfo>();
	...
	private int m_commission_ncoin;
	private int m_drain_ncoin;
	private int m_ncoin_per_adena;
	private int m_ncoin_per_adena_unit;
	private String m_generate_date;
	private String m_complete_date;
	private int m_customer_id;
	private String m_customer_name;
	private MJNCoinAdenaInfo(){}

	@override
	public String toString() {
		StringBuilder sb = new StringBuilder();
		sb.append("交易編號: ").append(m_trade_id).append("\r\n");
		sb.append("賣家: ").append(m_character_name).append("\r\n");
		sb.append("買家: ").append(MJString.isNullOrEmpty(m_customer_name) ? MJString.EmptyString : m_customer_name).append("\r\n");
		sb.append("亞丁那數量: ").append(String.format("%,d", m_adena_amount)).append("\r\n");
		sb.append("價格: ").append(String.format("%,d", m_ncoin_value)).append("\r\n");
		sb.append("註冊日期: ").append(m_generate_date).append("\r\n");
		sb.append("銷售日期: ").append(MJString.isNullOrEmpty(m_complete_date) ? MJString.EmptyString : m_complete_date).append("\r\n");
		return sb.toString();
	}
	...
	public String to_provider() {
		...
		.append("N幣存入金額: ").append(String.format("%,d", m_drain_ncoin)).append("\n")
		.append("適用手續費: ").append(String.format("%,d(", m_commission_ncoin)).append((int)(m_commission * 100)).append("%)\r\n")
		.append("按下 F8 鍵上方的 'C' 按鈕確認！")
		.toString();
	}
	...
	public int get_commission_ncoin(){
		return m_commission_ncoin;
	}
	public int get_drain_ncoin(){
		return m_drain_ncoin;
	}
	public int get_ncoin_per_adena(){
		return m_ncoin_per_adena;
	}
	public int get_ncoin_per_adena_unit(){
		return m_ncoin_per_adena_unit;
	}
