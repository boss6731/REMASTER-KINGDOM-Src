...
public class MJUIAdapter {
	...
	public static void on_login_user(GameClient clnt, L1PcInstance pc) {
		if (is_mjfx()) {
			MJFxLogger.LOGIN_CHARACTER.append_log(String.format("<登入> (%d)%s - %s", L1World.getInstance().get_player_size(), pc.getName(), clnt == null ? "" : clnt.getHostname()));
		} else {
			try {
	...
	public static void on_logout_user(GameClient clnt, L1PcInstance pc) {
		if (is_mjfx()) {
			MJFxLogger.LOGIN_CHARACTER.append_log(String.format("<登出> (%d)%s - %s", L1World.getInstance().get_player_size(), pc.getName(), clnt == null ? "" : clnt.getHostname()));
		} else {
			try {
				l1j.server.swing.chocco.count -= 1;
				l1j.server.swing.chocco.label2.setText(" " + l1j.server.swing.chocco.count);
				l1j.server.swing.chocco.userlist.remove(pc.getName());
			} catch (Exception e) {}
		}
	...
	public static void on_create_account(String account, String hostname) {
		if (is_mjfx()) {
			MJFxLogger.ACCOUNT_CREATE.append_log(String.format("<帳號創建> %s - %s", account, hostname));
		} else {
	...
	public static void on_gm_command_append(String s) {
		if (is_mjfx()) {
			MJFxLogger.GM_COMMAND.append_log(String.format("<GM指令結果>%s", s));
		} else {
	...
	public static void on_boss_append(int npc_id, String boss_name, int x, int y, int map_id) {
		if (is_mjfx()) {
			MJFxLogger.BOSS_TIMER.append_log(String.format("<BOSS檢查> %s(%d) %d,%d,%d", boss_name, npc_id, x, y, map_id));
		}
	}
