...
public class CPMWBQSystemProvider {
	...
	public void BQTeleport(L1PcInstance pc, CS_HUNTING_TELEPORT_REQ req) {
		...
		if(pc.getInventory().checkItem(CPMWBQmapInfoTable.Get_Map_Info(mapname).costItem())) {
			...
			pc.start_teleport(CPMWBQmapInfoTable.Get_Map_Info(mapname).locX(), CPMWBQmapInfoTable.Get_Map_Info(mapname).locY(), CPMWBQmapInfoTable.Get_Map_Info(mapname).LocMapid(), pc.getHeading(), 169, true, false);
		} else {
			pc.sendPackets("不符合進入的條件。");
		}
	}
	...
	public void BQQuestComplete(L1PcInstance pc, CS_HUNTING_QUEST_REWARD_REQ req) {
		...
		if(!Is_Complete(pc, req)) {
			System.out.println(String.format("[怪物圖鑑]要求不符合狀態的獎勵！角色名 : %s", pc.getName()));
			return;
		}
	...
	public void BQUpdateAmount(L1PcInstance pc, L1MonsterInstance mon) {
		...
		for(CPMWBQinfo info : pc.Get_BQ_Info()) {
			if(mon.getMapId() == 4) {
				if(info.getMapid() == mon.getMapId() && info.getMapdesc() == CPMWBQNpcInfoTable.Get_Desc(mon.getNpcClassId())) {
