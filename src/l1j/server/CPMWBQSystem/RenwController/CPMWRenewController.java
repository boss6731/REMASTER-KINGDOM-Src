...
public class CPMWRenewController implements Runnable {
	...
	public void initBQUser() {
		try {
			if (day.getHours() == CPMWBQReward.service().gethour() && day.getMinutes() == CPMWBQReward.service().getminiute() && CPMWBQReward.service().use()) {
				...
				L1World.getInstance().broadcastServerMessage(String.format("\\aH[怪物圖鑑重置通知]（每天 %s 時 %s 分怪物圖鑑重置）", CPMWBQReward.service().gethour(), CPMWBQReward.service().getminiute()));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
