...
public class DungeonTimePotion {
	...
	public void use_potion(L1PcInstance pc, L1ItemInstance item) {
		ServerBasePacket pck = MJSurveySystemLoader.getInstance().registerSurvey(String.format("是否使用 %s 來充值時間？", item.getName()), item.getId(), new MJInterfaceSurvey() {
			@Override
			public void survey(L1PcInstance pc, int num, boolean isYes) {
				...
				if (progress == null) {
					pc.sendPackets(String.format("在使用充值石之前，您需要先進入一次地城(%s)。", dtInfo.get_description()));
					return;
					...
					pc.sendPackets(String.format("%s 已經充滿電。", dtInfo.get_description()));
					return;
					}*/
				}
				...
				progress.inc_charge_count();
				DungeonTimeProgressLoader.update(progress);
				pc.sendPackets(String.format("%s 已經充值完成。", dtInfo.get_description()));
