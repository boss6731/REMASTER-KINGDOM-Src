...
public class DungeonTimeUpdator implements TimeListener{
	...
	public void run(){
		System.out.println(String.format("[%s]地下城時間更新器::執行()", MJString.get_current_datetime()));
	...
	public void dispose(){
		System.out.println(String.format("[%s]地下城時間更新器::部署()", MJString.get_current_datetime()));
				...
				try {
					L1World.getInstance().getAllPlayerStream().filter((L1PcInstance pc) -> {
						...
						try {
							...
							System.out.println("地下城時間更新器::進行初始化()");
							e.printStackTrace();
						}
					});
				} catch (Exception e) {
...
//						pc.getAccount().setFeatherCount(0);
	...
	public void onSecondChanged(BaseTime time) {
		/** 地下城重置時間 **/
		if (time.equals_time(Config.ServerAdSetting.hour, Config.ServerAdSetting.minute, Config.ServerAdSetting.second)) {
			do_initialize(time.getCalendar(), false);
		}
		if (time.equals_day_time(Config.ServerAdSetting.day, Config.ServerAdSetting.hour, Config.ServerAdSetting.minute, Config.ServerAdSetting.second)) {
			do_initialize(time.getCalendar(), true);
		}
		if (time.equals_time(Config.ServerAdSetting.limit_hour, Config.ServerAdSetting.limit_minute, Config.ServerAdSetting.limit_second)) {
			ItemDropLimitLoader.getInstance().clearLimitItem();
			System.out.println(String.format("[%s]物品掉落限制加載器::明確限制物品", getLocalTime()));
		}
		if (Config.ServerAdSetting.Quest_true) {
			if (time.equals_time(Config.ServerAdSetting.Quest_hour, Config.ServerAdSetting.Quest_minute, Config.ServerAdSetting.Quest_second)) {
				String message = String.format("\fT[任務通知] 每日任務重置時間已到，進行中的任務將被刪除。");
				...
				if (time.equals_day_time(Config.ServerAdSetting.Quest_day, Config.ServerAdSetting.Quest_hour, Config.ServerAdSetting.Quest_minute, Config.ServerAdSetting.Quest_second)) {
					String message_week = String.format("\fT[任務通知] 每週任務重置時間已到，進行中的任務將被刪除。");
					L1World.getInstance().broadcastPacketToAll(new S_SystemMessage(message_week));
					L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, message_week));
					DELETE_CUSTOM_WEEK_QUEST();
				}
			}
		}
		if (time.equals_time(Config.ServerAdSetting.craft_hour, Config.ServerAdSetting.craft_minute, Config.ServerAdSetting.craft_second)) {
			...
			L1World.getInstance().broadcastPacketToAll(new S_SystemMessage(message));
			L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, message));
			System.out.println(String.format("[%s]製作清單加載器::明確的製作限制", getLocalTime()));
		} else {
			if (_consumer != null) {
...
//			pstm = con.prepareStatement("DELETE FROM character_custom_quest WHERE quest_type=?");
