...
public class DungeonTimeUserInformation {
	...
	public void dec_dungeon_progress(L1PcInstance pc, DungeonTimeInformation dtInfo){
		...
		if (progress.dec_remain_seconds() <= 0) {
			// TODO 地城時間已結束，但由於重新進入的嘗試導致 -1++處理，強制設置為0（必須為0~-1才能使用充值藥水）
...
//            System.out.println(progress.dec_remain_seconds());
			...
			pc.sendPackets(String.format("\f2使用帳號的時間限制地城(%s)使用時間已超過。(每天上午6點重置)", dtInfo.get_description()));
		}
	}
	...
	private void sendDungeonTime(L1PcInstance pc, DungeonTimeProgress<?> progress, boolean send) {
		...
		/*if (remain_hours > 0) {
			pc.sendPackets(new S_ServerMessage(1525, String.valueOf(remain_hours), String.valueOf(remain_minute)));
		...
		if (pc.getMapId() == 54) {
			if (GameTimeClock.getInstance().is_night())
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "在夜間無法使用隨機傳送的區域。"));
		}
	}
