...
public class DungeonTimeLoadManager implements MJCommand{
	...
	private MJCommandTree createCommand(){
		return new MJCommandTree(".地城計時器", "執行與地城計時器相關的命令.", null)
	...
	private MJCommandTree createReloadCommand(){
		return new MJCommandTree(".重新載入", "執行與重新載入相關的命令。", null)
				.add_command(new MJCommandTree("信息", "", null){
					...
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						...
						args.notify("已重新載入 tb_dungeon_time_information 表格。");
					}
				})
				.add_command(new MJCommandTree("藥水", "", null){
					...
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						...
						args.notify("tb_dungeon_time_potion 表已重新載入。");
					}
				});
	}
	...
	private MJCommandTree createInitializeCommand(){
		return new MJCommandTree(".初始化", "執行與初始化相關的命令。", null)
				.add_command(new MJCommandTree(".角色", "重置指定角色的地下城時間。", new String[]{"角色名稱"}) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						...
						pc.sendPackets("\f2用戶帳號的所有地下城時間已重置。");
						args.notify(String.format("已重置 %s 的所有地下城時間。", character_name));
				...
				.add_command(new MJCommandTree(".全體重置", "重置所有角色的地下城時間。", null) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						DungeonTimeUpdator.getInstance().do_initialize(RealTimeClock.getInstance().getRealTimeCalendar(), false);
						DungeonTimeUpdator.getInstance().do_initialize(RealTimeClock.getInstance().getRealTimeCalendar(), true);
						args.notify("已完成全部重置。");
					}
				});
	}
	...
	private MJCommandTree createSchedulerCommand(){
		return new MJCommandTree(".排程", "執行與排程相關的命令。", null)
				.add_command(new MJCommandTree(".狀態", "顯示當前排程的狀態。", null) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						args.notify(String.format("當前排程狀態 : %s", DungeonTimeUpdator.getInstance().is_running() ? "開啟" : "關閉"));
					}
				})
				.add_command(new MJCommandTree("重啟", "重新啟動排程。", null) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
