...
public class MJAttendanceRewardExecutor implements Runnable{
	...
	private void do_bonus_store(boolean is_broadcast, boolean is_probability_rewards){
		try {
			...
			user_data.get_groups().get(m_group_type.toInt()).get_groupData().get(m_attendance_index - 1).set_state(UserAttendanceState.CLEAR);
			SC_ATTENDANCE_USER_DATA_EXTEND.update(m_pc.getAccountName(), user_data);

			L1ItemInstance item = m_pc.getInventory().storeItem(m_bonus_info.get_itemId(), m_bonus_info.get_itemCount());
			m_pc.getAccount().addBlessOfAin((m_bonus_info.getAinHasadCharge() * 10000), m_pc, "出席");
			SC_ATTENDANCE_REWARD_ACK.send(m_pc, get_attendance_index(),	m_group_type, item.getItem().getItemDescId(), m_bonus_info.get_itemCount(), is_broadcast);
			if(!m_pc.isGm() && is_broadcast){
				L1World.getInstance().broadcastPacketToAll(new ServerBasePacket[]{
						new S_SystemMessage(String.format("有人在出席檢查獎勵中獲得了 %d 號的 %s 物品。", m_attendance_index, m_bonus_info.get_itemName())),
						new S_PacketBox(S_PacketBox.GREEN_MESSAGE, String.format("有人在出席檢查獎勵中獲得了 %d 號的 %s 物品。", m_attendance_index, m_bonus_info.get_itemName()))
				});
			}
				...
				if (m_group_type != AttendanceGroupType.PC_CAFE) {
					if (m_group_type == dGroup.get_groupType()) {
						...
						if (clearcount >= dGroup.get_groupData().size()) {
							...
							userData.get_groups().set(dGroup.get_groupType().toInt(), UserAttendanceDataGroup.newInstance(m_pc, dGroup.get_groupType()));
							...
							if (!m_pc.is_shift_battle()) {
								...
								if (account.getAttendance_Premium()) {
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(m_pc, AttendanceGroupType.fromInt(2), 0), true);
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(m_pc, AttendanceGroupType.fromInt(2), 0), true);
								}
								if (account.getAttendance_Special()) {
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(m_pc, AttendanceGroupType.fromInt(3), 0), true);
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(m_pc, AttendanceGroupType.fromInt(3), 0), true);
								}
								if (account.getAttendance_Brave_Warrior()) {
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(m_pc, AttendanceGroupType.fromInt(4), 1), true);
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(m_pc, AttendanceGroupType.fromInt(4), 1), true);
								}
								if (account.getAttendance_Aden_World()) {
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(m_pc, AttendanceGroupType.fromInt(5), 1), true);
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(m_pc, AttendanceGroupType.fromInt(5), 1), true);
								}
								if (account.getAttendance_Bravery_Medal()) {
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(m_pc, AttendanceGroupType.fromInt(6), 1), true);
									m_pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(m_pc, AttendanceGroupType.fromInt(6), 1), true);
								}
							}
						}
					}
...
//		Updator.exec("delete from attendance_rewards_history where object_id=?", new Handler() {
//			@Override
...
//	}
	...
	public void isInit(){
		...
		if(m_pc.getAttendanceData().get_groups().get(0).get_currentAttendanceIndex() + 1 < AttendanceGroupType.NORMAL.getBonusSize() && m_group_type == AttendanceGroupType.NORMAL) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(1).get_currentAttendanceIndex() + 1 < AttendanceGroupType.PC_CAFE.getBonusSize() && m_group_type == AttendanceGroupType.PC_CAFE) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(2).get_currentAttendanceIndex() + 1 < AttendanceGroupType.PREMIUM.getBonusSize() && m_group_type == AttendanceGroupType.PREMIUM) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(3).get_currentAttendanceIndex() + 1 < AttendanceGroupType.SPECIAL.getBonusSize() && m_group_type == AttendanceGroupType.SPECIAL) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(4).get_currentAttendanceIndex() + 1 < AttendanceGroupType.BRAVE_WARRIOR.getBonusSize() && m_group_type == AttendanceGroupType.BRAVE_WARRIOR) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(5).get_currentAttendanceIndex() + 1 < AttendanceGroupType.ADEN_WORLD.getBonusSize() && m_group_type == AttendanceGroupType.ADEN_WORLD) {
			return;
		}
		if(m_pc.getAttendanceData().get_groups().get(6).get_currentAttendanceIndex() + 1 < AttendanceGroupType.BRAVERY_MEDAL.getBonusSize() && m_group_type == AttendanceGroupType.BRAVERY_MEDAL) {
			return;
		}
...
// System.out.println("出席檢查重置");
	...
	public boolean isclearAll() {
		...
		if(m_group_type == AttendanceGroupType.NORMAL) {
			for(int i = 0; i < AttendanceGroupType.NORMAL.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.NORMAL.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		} else if(m_group_type == AttendanceGroupType.PC_CAFE) {
			for(int i = 0; i < AttendanceGroupType.PC_CAFE.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.PC_CAFE.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		} else if(m_group_type == AttendanceGroupType.PREMIUM) {
			for(int i = 0; i < AttendanceGroupType.PREMIUM.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.PREMIUM.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		} else if(m_group_type == AttendanceGroupType.SPECIAL) {
			for(int i = 0; i < AttendanceGroupType.SPECIAL.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.SPECIAL.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		}else if(m_group_type == AttendanceGroupType.BRAVE_WARRIOR) {
			for(int i = 0; i < AttendanceGroupType.BRAVE_WARRIOR.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.BRAVE_WARRIOR.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		}else if(m_group_type == AttendanceGroupType.ADEN_WORLD) {
			for(int i = 0; i < AttendanceGroupType.ADEN_WORLD.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.ADEN_WORLD.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		}else if(m_group_type == AttendanceGroupType.BRAVERY_MEDAL) {
			for(int i = 0; i < AttendanceGroupType.BRAVERY_MEDAL.getBonusSize(); i++) {
				if(user_data.get_groups().get(AttendanceGroupType.BRAVERY_MEDAL.toInt()).get_groupData().get(i).get_state() != UserAttendanceState.CLEAR) {
					return false;
				}
			}
		}
	...
	public void run() {
		try{
			...
			e.printStackTrace();
			System.out.println(String.format("[出席檢查獎勵例外信息] - 角色 : %s, 出席ID : %d, 組別 : %s", m_pc.getName(), get_attendance_index(), get_group_type()));
		}
