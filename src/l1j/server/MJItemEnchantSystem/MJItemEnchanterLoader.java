...
public class MJItemEnchanterLoader{
	...
	public boolean do_enchant(L1PcInstance pc, L1ItemInstance enchanter_item, L1ItemInstance enchantee_item){
		...
		MJItemEnchantee enchantee = MJItemEnchanteeLoader.getInstance().find_enchantee(enchanter.get_enchanter_id(), enchantee_item.getItemId());
		if(enchantee == null){
			pc.sendPackets(String.format("%s 是無法強化的物品。", enchantee_item.getName()));
			return true;
		}
		
		if(enchanter_item.getCount() < enchanter.get_item_amount()){
			pc.sendPackets(String.format("使用 %s 需要 %d 個。", enchanter_item.getName(), enchanter.get_item_amount()));
			return true;
		}
