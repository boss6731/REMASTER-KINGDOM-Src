	...
	public void processEQC(L1PcInstance pc, int useItemId, L1ItemInstance oItem, L1ItemInstance tItem){
		try {
			if (!pc.getInventory().checkItem(useItemId, 1) || oItem == null || tItem == null) {
				//System.out.println("裝備交換憑證錯誤: !pc.getInventory().checkItem(useItemId, 1) || useItem == null || targetItem == null / 使用者: " + pc.getName());
				return;
			}
			L1Item targetTemp = tItem.getItem();
			if (pc.getInventory().checkEquipped(targetTemp.getItemId())) {
				pc.sendPackets(new S_ServerMessage(4302)); // 無法使用於已穿戴的物品。
				return;
			}
			...
			// 1: 頭盔, 2: 盔甲, 3: T, 4: 披風, 5: 手套, 6: 靴子, 7: 盾牌, 8: 項鍊, 9: 戒指, 10: 腰帶, 11: 第二枚戒指,
			// 12: 耳環, 13: 吊襪帶, 25: 握力竿, 15: 護脛, 31: 肩甲, 32: 煥章, 33: 吊墜
			int subType = targetTemp.getType();
			if (targetItemType == 2 && (subType >= 8 && subType <= 12)) targetItemType = 3;

			if (useItemId == 847 && subType != 10) { // 四大法書
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。")); // 一般物品
				return;
			}
			if (useItemId == 848 && subType != 12) { // 魔法人偶
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。")); // 一般物品
				return;
			}
			if (useItemId == 846 && targetItemType != 1) { // 武器
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。"));
				return;
			}
			if (useItemId == 844 && (targetItemType != 2 || targetItemType == 2 && subType == 25)) { // 裝備
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。"));
				return;
			}

			if (useItemId == 845 && targetItemType != 3) { // 飾品
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。"));
				return;
			}

			if (useItemId == 854 && (targetItemType != 2 || targetItemType == 2 && subType != 25)) { // 長生藥符文
				pc.sendPackets(new S_SystemMessage("無法使用於該物品。")); // 長生藥符文
				return;
			}
			...
			pc.sendPackets(S_EQCPacket.get(pc, get(itemClass), tItem.getEnchantLevel(), tItem.getAttrEnchantLevel(), tItem.getBless()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
