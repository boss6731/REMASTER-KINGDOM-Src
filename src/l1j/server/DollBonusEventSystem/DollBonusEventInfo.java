...
public class DollBonusEventInfo {
	...
	private static DollBonusEventInfo newInstance() {
	...
	public int trans_npc(int i) {
		String[] transe_id = (String[]) MJArrangeParser
				.parsing(get_trans_id(), ",", MJArrangeParseeFactory.createStringArrange()).result();
		int npcid = 0;
	...
	public int calc_probability(L1PcInstance pc, int i) {
		...
		String[] drop_min_chance = (String[]) MJArrangeParser
				.parsing(_min_probability, ",", MJArrangeParseeFactory.createStringArrange()).result();
		String[] drop_max_chance = (String[]) MJArrangeParser
				.parsing(_max_probability, ",", MJArrangeParseeFactory.createStringArrange()).result();
		...
		if (_is_loggin) {
			if (pc.isGm()) {
				System.out.println(String.format("地圖ID: %d, 名稱: %s, 更改概率:(%d)", _map_id, _map_name, probability));
			}
		}
