...
public class MJCombatLoadManager implements MJCommand{
	...
	public static ProtoOutputStream create_result_message(boolean is_winner, MJSimpleRgb rgb, int duration){
		return createResultMessage(String.format("比賽已結束。%s！%d秒後將移動到村莊。", is_winner ? "勝利" : "失敗", duration), rgb, duration);
	}
	...
	private MJCommandTree createCommand(){
		return new MJCommandTree(".戰鬥系統", "戰鬥系統指令：[1.實例][2.重載]", null)
				.add_command(createViewCommand())
				.add_command(createReloadCommand());
	}

	private MJCommandTree createViewCommand(){
		return new MJCommandTree("1", "實例指令：[1.開啟列表][2.強制終止]", null)
				...
				.add_command(new MJCommandTree("2", "", new String[]{"戰鬥場ID"}){
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						int combat_id = args.nextInt();
						MJCombatObserver observer = _current_observers.get(combat_id);
						if(observer == null){
							args.notify(String.format("無法找到戰鬥場ID %d的當前開啟實例。", combat_id));
							return;
						}

						String msg = String.format("由管理員進行中的%s將在%d秒後強制結束。", observer.getCombatInfo().getGameType().toKrDesc(), COMBAT_QUIT_DELAY_SECOND);
	...
	private MJCommandTree createReloadCommand() {
		return new MJCommandTree("2", "執行重載相關命令。[1.配置][2.信息表][3.獎勵][4.陷阱]", null)
				.add_command(new MJCommandTree("1", "", null) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						load_config();
						args.notify("重載了mj_combat.properties。");
					}
				})
				.add_command(new MJCommandTree("2", "", null) {
					...
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						...
						args.notify("重載了tb_combat_informations表。");
					}
				})
				.add_command(new MJCommandTree("3", "", null) {
					...
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						...
						args.notify("重載了tb_combat_rewards表。");
					}
				})
				.add_command(new MJCommandTree("4", "", null) {
					...
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						...
						args.notify("重載了tb_combat_traps表。");
					}
				});
	}
}