...
import static l1j.server.ForgottenIsland.FIController.isReady;
...
public abstract class MJCombatObserver extends MJAbstractCopyMapObserver implements Runnable{
	...
	public String toString() {
		...
		sb.append("[ID:").append(_combat_info.get_combat_id()).append("(").append(_combat_info.get_game_type().toKrDesc()).append(")");
		...
		sb.append(",開始時間:").append(cal.get(Calendar.HOUR_OF_DAY)).append(":").append(cal.get(Calendar.MINUTE)).append(":").append(cal.get(Calendar.SECOND));
		return sb.toString();
	}
	...
	public void enter(L1PcInstance pc){
		...
		if(!isReady()) {
			pc.sendPackets("進場時間已超過。")
			return
		}

		if(!pc.isWorld()) {
			pc.sendPackets("現在是無法進入的狀態。")
			return
		}

		int nextId = nextTeamId()
		MJCombatTeamInformation tInfo = _combat_info.getTeamInformations(nextId)
		if(tInfo == null) {
			pc.sendPackets("現在是無法進入的狀態。")
			return
		}
	...
	protected boolean validation_users(){
		...
		if(col.size() < _team_size){
			String message = String.format("參加人數不足，%s已終止。", _combat_info.get_game_type().toKrDesc());
			L1World.getInstance().broadcastPacketToAll(message);
			ProtoOutputStream stream = SC_NOTIFICATION_MESSAGE.makeStream(
					String.format("%s\\r\\n%d秒後將移動到村莊。", message, MJCombatLoadManager.COMBAT_QUIT_DELAY_SECOND),
		...
		ProtoOutputStream stream = SC_NOTIFICATION_MESSAGE.makeStream("隊伍調整後遊戲將開始。", MJCombatLoadManager.COMBAT_DELAY_MESSAGE_RGB, 5);
	...
	public void leave(L1PcInstance pc){
		...
		if(!pc.isDead())
			pc.start_teleport(MJCopyMapObservable.RESET_X, MJCopyMapObservable.RESET_Y, MJCopyMapObservable.RESET_MAPID, pc.getHeading(), 18339, true, false);
		try{
			...
			public void run(){
				...
				try{
					do{
						...
						if(sec % MJCombatLoadManager.COMBAT_TRAPS_SPAWN_SECOND == 0 && MJRnd.isWinning(1000000, MJCombatLoadManager.COMBAT_TRAPS_SPAWN_PROB_BY_MILLION)){
							Collection<L1PcInstance> col = L1World.getInstance().getVisiblePlayers(_copyMapId);
