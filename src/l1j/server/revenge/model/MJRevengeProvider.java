...
public class MJRevengeProvider {
	...
	private static final S_SystemMessage failureMessage = new S_SystemMessage("\f2自己或對方的等級低於75時，不會登記為復仇。");
	...
	private MJRevengeModel onNewLoser(final L1PcInstance owner, final L1PcInstance target) {
		...
		MJRevengeModel pursuiteModel = owner.attribute().getNotExistsNew(L1PcInstance.revengePursuitModelKey).get();
		if(pursuiteModel != null && pursuiteModel.targetId() == target.getId()) {
