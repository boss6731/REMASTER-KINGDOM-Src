		...
		try {
			...
			GeneralThreadPool.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					noti.set_notificationMessage(String.format("\f3%s 已經擊敗了 %s。", pc.getName(), target.getName()));
					...
					ServerBasePacket[] pcks = new ServerBasePacket[] {
							new S_SystemMessage(String.format("\\aD挑釁: [%s]的攻擊使得 [%s] 已經死亡。", pc.getName(), target.getName(), Opcodes.S_MESSAGE)),
					};
