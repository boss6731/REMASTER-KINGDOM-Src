...
class MJRevengeLoserModel extends MJRevengeModel {
		...
		try {
			...
			GeneralThreadPool.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					if (castle_id != 0) {
						pc.sendPackets("\f3對方在無法移動的地區，無法傳送。");
                        return null;
					}
	...
	public RevengeInfoT newNetworkModel() {
		...
		int activate_duration = currentSeconds - rInfo.get_register_timestamp(); // 當前時間 10 , 發生時間 3 = 7;
		...
		rInfo.set_activate_duration(activate_duration); // 2小時內啟用點擊功能
		return rInfo;
	}
