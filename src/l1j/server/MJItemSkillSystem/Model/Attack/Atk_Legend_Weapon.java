...
public class Atk_Legend_Weapon extends MJItemSkillModel {
	...
	public double get(L1Character attacker, L1Character defender, L1ItemInstance t_item, double dwd) {
		...
		if (itemId == 203041) {
			...
			if (dmp > 0) {
				// 40 額外法力吸收幾率
				int pure = getProbability(attacker, t_item) + 40;
				int success = MJRnd.next(0, 100);

				if (success < pure) {
					int drn = MJRnd.next(2, 5); // 2 到 5 之間的隨機吸收
					if (drn > dmp)
						drn = dmp;
		...
		if (defender instanceof L1PcInstance) {
			...
			if (!defender.hasSkillEffect(L1SkillId.STATUS_FREEZE)) {
				int t = 2000;
			...
			if (pve_eff_id > 0) {
				broadcast(defender, new S_SkillSound(defender.getId(), pve_eff_id));
				...
				for (L1Object obj : list) {
					...
					if (c.getMap().isSafetyZone(c.getLocation()))
						continue;
					if (c.isDead() || MJCommons.getDistance(defender.getX(), defender.getY(), c.getX(),
							c.getY()) > Location_count)
						continue;
					if (c == defender)
						continue;
