		...
		if (item.getItemId() == 420104 || item.getItemId() ==  420108 || item.getItemId() == 420111){
			...
			for (L1Object obj : L1World.getInstance().getVisiblePoint(targetLoc, range)){
				...
				if (obj instanceof L1MonsterInstance){
					...
					if ((!targetLocType && !mon.isDead()) || (targetLocType && !mon.isDead() && mon.getId() != target.getId())){
...
//						Broadcaster.broadcastPacket(mon, new S_DoActionGFX(mon.getId(), 2), true);
	...
	public boolean validation() {
		if(owner == null || owner.is_non_action() || owner.isDead() || MJCommons.isNonAction(owner) || owner.isGhost() || owner.get_teleport() || owner.isInvisDelay())
			return false;
		if ((item.getItemId() == 420104 || item.getItemId() ==  420108 || item.getItemId() == 420111) && !(owner.getMapId() >= 732 && owner.getMapId() <= 776)){
			owner.sendPackets(new S_ServerMessage(563)); // 這裡無法使用。
			return false;
		}
