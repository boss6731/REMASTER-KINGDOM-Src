...
public class AttackContinueActionHandler extends AbstractActionHandler implements MJPacketParser {
	...
	public void doWork() {
		try {
			...
			/*
			 * if (owner.hasSkillEffect(L1SkillId.MOEBIUS)) { owner.removeSkillEffect(L1SkillId.MOEBIUS); }
			 */
	...
	public void handle() {
		...
		if (owner.getWeapon() != null && owner.getWeapon().getItemId() != 190 && owner.getWeapon().getItemId() != 10000 && owner.getWeapon().getItemId() != 202011) {
			if (owner.getWeapon().getItem().getType2() == 1
					&& (owner.getWeapon().getItem().getType() == 4 || owner.getWeapon().getItem().getType() == 10 || owner.getWeapon().getItem().getType() == 13)
...
/*		System.out.println((System.currentTimeMillis() - owner.getLastMoveActionMillis()) + " " + getInterval());
	...
	public boolean validation() {
		...
		if (owner.isGmInvis() || owner.is_non_action() || owner.isFishing() || owner.isDead() /* || (owner.isInvisble()) */ || owner.isGhost() || owner.get_teleport() || owner.isInvisDelay()) {
			unregister();
			return false;
		}
		...
		if (!owner.isGm()) {
			if (obj != null && obj instanceof L1PcInstance) {
				if (owner.getCurrentSpriteId() == 12015 || owner.getCurrentSpriteId() == 5641 || owner.getCurrentSpriteId() == 11685 || owner.getCurrentSpriteId() == 11620
						|| owner.getCurrentSpriteId() == 11621) {
					owner.sendPackets("在傳說/神話變身狀態下無法進行PVP。");
					return false;
				}
			}
		}
		...
		if (previous_heading != next_heading) {
			...
			// TODO 精靈射箭時，隨著標題更改，動作異常（下面的代碼似乎是幻術師的配置方向）
