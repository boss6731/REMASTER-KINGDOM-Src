					...
					if (temp.getType() == 1) {
						if (temp.isMentuse()) {
							...
							if (temp.getMent() != null) {
								...
								String message = String.format("%s 在 %s 獲得了 %s \fH。", men, locationName, itemName);
								String message2 = String.format("%s 在 %s 獲得了 %s \fH。", men, locationName, itemName);
								L1World.getInstance()
			...
			try {
				...
				if (item.getItemId() == L1ItemId.ADENA && amount > len) {
					...
					for (L1PcInstance pc : members) {
						...
						ServerBasePacket pck = new S_ServerMessage(813, npc.get_name(), item.getLogName(), pc.getName());
	...
	private boolean validationAmount() {
		...
		if (item.isStackable()) {
			...
			if (amount + sum > MAX_STACKABLE) {
				owner.sendPackets("超過可堆疊的物品（亞丁幣），無法拾取。（請使用倉庫）");
				return false;
			} else if (item.getItemId() != L1ItemId.ADENA && amount > 1000000) {
				owner.sendPackets("無法掉落過多物品。");
				return false;
			}
		} else if (amount > 1) {
			GameServer.disconnectChar(owner);
