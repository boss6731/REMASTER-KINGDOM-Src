...
public class MJKDALoadManager {
		...
		try{
			...
			case 1:
				...
				case 4:
					...
					gm.sendPackets(new S_SystemMessage("[MJKDA 系統存儲完成。]"));
					break;
				default:
					throw new Exception("");
			}
			...
			gm.sendPackets(new S_SystemMessage("[1.開啟/關閉殺戮圖表][2.重新加載][3.初始化用戶][4.強制存儲]"));
		}
	}
	...
	private static void initCommands(L1PcInstance gm, String s){
		try{
			...
			if(pc == null){
				gm.sendPackets(new S_SystemMessage(String.format("無法找到 %s。", s)));
			}else{
				pc.getKDA().onInit(pc);
				gm.sendPackets(new S_SystemMessage(String.format("已初始化 %s 的擊殺死亡數據。", s)));
			}
		}catch(Exception e){
		...
		try{
			...
			switch(argsQ.poll()){
				case 0:
					...
					gm.sendPackets(new S_SystemMessage("擊殺死亡圖表系統已關閉。"));
					break;
				case 1:
					if(MJKDAChartScheduler.isLoaded())
						gm.sendPackets(new S_SystemMessage("系統已在運行中。"));
					else{
						KDA_CHART_RUN = true;
						MJKDAChartScheduler.getInstance().run();
						gm.sendPackets(new S_SystemMessage("擊殺死亡圖表系統已啟動。"));
				}
				break;
			default:
				throw new Exception("");
