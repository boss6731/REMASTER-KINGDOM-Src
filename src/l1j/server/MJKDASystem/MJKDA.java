...
public class MJKDA {
	...
	private static ServerBasePacket[] _kdaMessages = new ServerBasePacket[] {
			new S_SystemMessage("您的擊殺/死亡比已重置。"),
			new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "攻城點數增加了1點。（PK勝利）"),
			new S_SystemMessage("目前處於保護模式，不會有懲罰。請安心狩獵。"),
	};
	...
	public void onKill(L1PcInstance killer, L1PcInstance victim) {
		...
		killer.sendPackets(new S_PacketBox(S_PacketBox.BATTLE_SHOT, victim.getId()));
		...
		try {
			...
			if (diff < MJKDALoadManager.KDA_DEATH_DELAY_MS) {
				killer.sendPackets(new S_SystemMessage(String.format("[%s] 角色在短時間內死得太多，不會發放點數。", victim.getName())));
				return;
			} else if (tKda.objid == oKda.lastKillId && diff < MJKDALoadManager.KDA_KILL_DUPL_DELAY_MS) {
				killer.sendPackets(new S_SystemMessage(String.format("[%s] 角色在短時間內被殺得太多，不會發放點數。", victim.getName())));
				return;
			}
	...
	public void do_assassination(L1PcInstance owner) {
		...
		if (kill == Config.ServerAdSetting.KillCount1) {
			...
			owner.sendPackets("\f3由於您濫用 PVP，將被困在地獄中 [60 分鐘]。");
	...
	public void notifyKill(L1PcInstance killer, L1PcInstance victim) {
		...
		ServerBasePacket[] pcks = new ServerBasePacket[]{ // \\da
				// new S_ChatPacket(String.format("\\f=[KILL]:\\aG[%s] → [%s]", killer.getName(), victim.getName()), Opcodes.S_MESSAGE),
				new S_SystemMessage(String.format("\\f3PVP: [%s]的攻擊使[%s]死亡.", killer.getName(), victim.getName(), Opcodes.S_MESSAGE)),
				// new S_ChatPacket(String.format("\f=[KILL]:\aG[%s]\fH的攻擊使敵人 \f2[%s] \fH死亡", killer_name, victim_name), Opcodes.S_MESSAGE),
				new S_ChatPacket(String.format("[PVP位置] %s", MapsTable.getInstance().getMapName(killer.getMapId())), Opcodes.S_MESSAGE), };
	...
	public void notifyKillForGotten(L1PcInstance killer, L1PcInstance victim) {
		...
		ServerBasePacket equalsPck = S_ShowCmd.getPkMessageAtBattleServer(String.format("\\aG%s", killer_name), victim_name);
		ServerBasePacket nonEqualsPck = S_ShowCmd.getPkMessageAtBattleServer(killer_name, victim_name);
		for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
			if (pc == null)
				continue;
	...
	public static int calculate_lawful(L1PcInstance killer) { // 23-03-21 變更
		int nLaw = 0;
