...
public class SpellExtractorLoader {
	...
	public boolean useItem(L1PcInstance pc, L1ItemInstance item, int type, int skillid){
		...
		if(pInfo == null) {
			if (pc.isGm()) {
				System.out.println(String.format("沒有關於技能 ID : %d 的信息。", skillid));
				return false;
			}
		}
	...
	public boolean Spell_Extractor(L1PcInstance pc, SpellExtractorInfo info, int type) {
		if (type == 0) {
			if (!pc.isPassive(info.getSkillId())) {
				pc.sendPackets("尚未習得的魔法。");
				return false;
			}
			if (!pc.getInventory().checkItem(4100718, 1)) {
				pc.sendPackets("不存在技能溶解道具。");
				return false;
			}
			if (ItemTable.getInstance().getTemplate(info.getItemId()) == null) {
				if (pc.isGm()) {
					pc.sendPackets("\f3- spell_extractor 表中不存在 item_id（列）：" + info.getItemId());
					return false;
				} else {
					pc.sendPackets("設置不正確，請聯繫管理員。");
					return false;
				}
			}
			...
			pc.sendPackets(SC_NOTIFICATION_MESSAGE.make_stream("\f3請稍候，技能正在移除中。", MJSimpleRgb.green(), 5));
			pc.sendPackets("\f3請稍候，技能正在移除中。");
			...
			GeneralThreadPool.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					C_NewCharSelect.restartProcess(pc);//先釋放
					try {
