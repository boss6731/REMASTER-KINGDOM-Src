...
public class S_ItemExSelectPacket extends ServerBasePacket{
	...
	public void do_select(L1PcInstance pc, int index) {
		...
		if (isAppear) {
			if (doll != null) {
				pc.sendPackets("在使用物品之前，請先解除召喚娃娃。");
				return;
			}
		}
		...
		pc.sendPackets(String.format("%s 已經交換。", reward.getLogName()));
	...
	public byte[] getBytes() {
		...
		l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream stream =
				l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream.newInstance(m_packet.getSerializedSize() + WireFormat.WRITE_EXTENDED_SIZE, MJEProtoMessages.SC_WAREHOUSE_ITEM_LIST_NOTI.toInt());
		try{
