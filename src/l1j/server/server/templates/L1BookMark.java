...
public class L1BookMark {
	...
	public static void addBookmark(final L1PcInstance pc, List<L1BookMark> bookmarks) {
		if(bookmarks.size() <= 0)
			return;
		
		final MJObjectWrapper<Integer> wrapper = new MJObjectWrapper<Integer>();
		wrapper.value = get_next_numid(pc.getId());
		Updator.batch("INSERT INTO character_teleport SET id=?,num_id=?,speed_id=?, char_id=?, name=?, locx=?, locy=?, mapid=?", new BatchHandler() {
			...
			public void handle(PreparedStatement pstm, int callNumber) throws Exception {
				...
				bookmark.setId(L1BookMarkIdFactory.getInstance().next_id());
				bookmark.setNumId(wrapper.value);
				bookmark.setTemp_id(wrapper.value);
				bookmark.setCharId(pc.getId());
				++wrapper.value;
	...
	public static void WriteBookmark(L1PcInstance pc) {
		final int size = pc._bookmarks.size();
		if(size > 0) {
	...
	public static void Bookmarkitem(L1PcInstance pc, L1ItemInstance useItem, int obj_id, boolean isChange) {
		List<L1BookMark> bookmarks = ShowBookmarkitem(pc, obj_id);
		...
		if(size > 0) {
			Updator.batch("insert ignore into character_teleport set id=?, num_id=?, speed_id=?, char_id=?, name=?, locx=?, locy=?, mapid=?, randomX=?, randomY=?", new BatchHandler() {
				...
				public void handle(PreparedStatement pstm, int callNumber) throws Exception {
					...
					pc.addBookMark(book);
					pc.sendPackets(new S_Bookmarks(book.getName(), book.getMapId(), book.getLocX(), book.getLocY(), book.getId()));
				}
			}, size);
		}
	...
	public static List<L1BookMark> ShowBookmarkitem(L1PcInstance pc, int obj_id) {
		final List<L1BookMark> bookmarks = new LinkedList<L1BookMark>();
	...
	public static void MakeBookmarkItem(final L1PcInstance pc, final L1ItemInstance item) {
		final int num_id = get_next_numid(pc.getId());
		final int size = pc._speedbookmarks.size();
		if(size > 0) {
	...
	public static void addBookmarkItem(final L1PcInstance pc, final L1ItemBookMark item) {
		final L1BookMark bookmark = new L1BookMark();
		final int num_id = get_next_numid(pc.getId());
		bookmark.setId(L1BookMarkIdFactory.getInstance().next_id());
		bookmark.setNumId(num_id);
