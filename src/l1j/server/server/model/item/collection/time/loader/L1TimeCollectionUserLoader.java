...
public class L1TimeCollectionUserLoader {
	...
	private void load(){
		...
		try {
			...
			while (rs.next()) {
				...
				if (obj == null) {
					System.out.println(String.format("[L1TimeCollectionUserLoader] TEMPLATE_NOT_FOUND FLAG(%d)", flag));
					continue;
				}
	...
	private static final String UPSERT_QUERY = "INSERT INTO character_time_collection SET "
			+ "charObjId=?, flag=?, type=?, collectionIndex=?, registItem=?, registComplet=?, buffType=?, buffTime=?, refill_count=? "
			+ "ON DUPLICATE KEY UPDATE "
			+ "type=?, collectionIndex=?, registItem=?, registComplet=?, buffType=?, buffTime=?, refill_count=?";
