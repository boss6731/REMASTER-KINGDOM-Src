	...
	private void load(){
		...
		try {
			...
			while (rs.next()) {
				...
				if (obj == null) {
					System.out.println(String.format("[L1FavorBookUserLoader] FAVOR OBJ NOT FOUND listType(%s), type(%d), index(%d)", 
							listType.getName(), type.getType(), index));
					continue;
				}
...
//					item.updateItemAblity();
	...
	private static final String UPSERT_QUERY = "INSERT INTO character_favorbook SET "
			+ "charObjId=?, listType=?, type=?, widthIndex=?, itemObjId=?, itemId=?, itemName=?, count=?, enchantLevel=?, attrLevel=?, bless=?, endTime=? "
			+ "ON DUPLICATE KEY UPDATE "
			+ "itemObjId=?, itemId=?, itemName=?, count=?, enchantLevel=?, attrLevel=?, bless=?, endTime=?";
	...
	public boolean insert(L1PcInstance pc, L1FavorBookUserObject user){
		int result = Updator.exec(UPSERT_QUERY, new Handler() {
			@Override
