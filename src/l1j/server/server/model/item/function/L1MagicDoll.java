...
public class L1MagicDoll {
	...
	private static HashMap<Integer, L1MagicDoll> _dataMap = new HashMap<Integer, L1MagicDoll>();

	private static HashMap<Integer, ArrayList<Integer>> _gradeMap = new HashMap<Integer, ArrayList<Integer>>();
	...
	public static L1MagicDoll get(int itemId) {
		return (L1MagicDoll) _dataMap.get(Integer.valueOf(itemId));
	}
	...
	private static void loadXml(HashMap<Integer, L1MagicDoll> dataMap,
			HashMap<Integer, ArrayList<Integer>> gradeMap) {
		try {
			JAXBContext context = JAXBContext
					.newInstance(new Class[] { ItemList.class });
			...
			for (L1MagicDoll each : list)
				if (each.init()) {
					...
					ArrayList<Integer> gradeList = gradeMap.get(Integer
							.valueOf(each.getInfo().getGrade()));
					if (gradeList == null) {
						gradeList = new ArrayList<Integer>();
					}
					gradeList.add(Integer.valueOf(each.getItemId()));
					gradeMap.put(Integer.valueOf(each.getInfo().getGrade()),
							gradeList);
				}
		} catch (Exception e) {
			_log.log(Level.SEVERE, "./data/xml/Item/MagicDoll.xmlload failed.",
					e);
		}
	}
	...
	public static void reload() {
		HashMap<Integer, L1MagicDoll> dataMap = new HashMap<Integer, L1MagicDoll>();
		HashMap<Integer, ArrayList<Integer>> gradeMap = new HashMap<Integer, ArrayList<Integer>>();
	...
	public boolean use(L1PcInstance pc, L1ItemInstance item) {
		...
		if (item.getLastUsed() != null) {
			if (System.currentTimeMillis() - item.getLastUsed().getTime() < 3000) {
				return false;
			}
		}
		...
		if (info != null && effect != null) {
			...
			// SC_SPELL_LATE_HANDLING_NOTI.send(pc, true,
			// eLevel.NOT_CORRECTION);
	...
	public static int getDamageAddByDoll(L1Character _master) {
		...
		if (_master.getMagicDoll() != null) {
			int prob = _master.getMagicDoll().getDoll().getEffect()
					.getDmgChance();
	...
	public static int getDamageReductionByDoll(L1Character _attacker,
			L1Character _master) {
		int dmg = 0;
		if (_master.getMagicDoll() != null) {
			if (((_attacker instanceof L1PcInstance))
					&& (_master.getMagicDoll().getItemId() == 49326)) {
				return 0;
			}
			int prob = _master.getMagicDoll().getDoll().getEffect()
					.getDmgReductionChance();
			if ((prob > 0) && (prob > CommonUtil.random(100))) {
	...
	public static int useSkillByDoll(L1Character _master, L1Character _target) {
		int dmg = 0;
		if (_master.getMagicDoll() != null) {
			L1MagicDoll magicDoll = (L1MagicDoll) _dataMap.get(Integer
					.valueOf(_master.getMagicDoll().getItemId()));
			if (magicDoll != null) {
				int skillId = magicDoll.getEffect().getSkillId();
				if (skillId > 0
						&& magicDoll.getEffect().getSkillChance() > CommonUtil
								.random(100)) {
					...
					dmg = (int) skill.getDamageValue()
							+ CommonUtil.random(skill.getDamageDice() + 1);

					if (skill.getTarget().equals("attack")) {
						if ((_target instanceof L1PcInstance)) {
							((L1PcInstance) _target)
									.sendPackets(new S_SkillSound(_target
											.getId(), skill.getCastGfx()));
						}
						doll.setHeading(doll.targetDirection(_target.getX(),
								_target.getY()));
						_master.sendPackets(new S_ChangeHeading(doll));
						_master.sendPackets(new S_DoActionGFX(doll.getId(), 1));
						...
						_master.sendPackets(new S_ChangeHeading(doll));
						_master.sendPackets(new S_DoActionGFX(doll.getId(), 1));
						doll.broadcastPacket(new S_SkillSound(doll.getId(),
								skill.getCastGfx()));
					}
				}
			}
	...
	public static class Effect {
		...
		public int getPvpReduction() {
			return _pvpReduction;
		}

		public int getDragonDmgDecrease() {
			return _dragonDmgDecrease;
		}
	...
	private static class ItemList implements Iterable<L1MagicDoll> {
	...
	public static byte[] getItemView(L1MagicDoll magicDoll, L1ItemInstance item) {
		...
		try {
			...
			int technique_tolerance = magicDoll.getEffect()
					.getTechniqueTolerance();
			...
			int dragonlang_tolerance = magicDoll.getEffect()
					.getDragonLangTolerance();
			...
			int dragonDmgDecrease = magicDoll.getEffect()
					.getDragonDmgDecrease();
			if (dragonDmgDecrease > 0) {
				os.write(39);
				os.writeS("受到龍的傷害減少 " + dragonDmgDecrease + "%");
			}

			double AdenBonus = magicDoll.getEffect().getAdenBonus();
			if (AdenBonus > 0) {
				os.write(39);
				os.writeS("額外獲得金幣: +" + (int) (AdenBonus * 100) + "%");
			}

			double ItemBonus = magicDoll.getEffect().getItemBonus();
			if (ItemBonus > 0) {
				os.write(39);
				os.writeS("物品額外掉落率: +" + (int) (ItemBonus * 100) + "%");
			}

			if (item.get_Doll_Bonus_Value() > 0) {
				if (item.get_Doll_Bonus_Value() >= 144 && item.get_Doll_Bonus_Value() <= 146 
					|| item.get_Doll_Bonus_Value() >= 148 && item.get_Doll_Bonus_Value() <= 163 ) {
			...
			if (ShortCritical > 0) {
				os.write(39);
				os.writeS("\fI近距離致命一擊: \\aA+" + ShortCritical);
			}

			int LongCritical = magicDoll.getEffect().getLongCritical();
			if (LongCritical > 0) {
				os.write(39);
				os.writeS("\fI遠距離致命一擊: \\aA+" + LongCritical);
			}

			int MagicCritical = magicDoll.getEffect().getMagicCritical();
			if (MagicCritical > 0) {
				os.write(39);
				os.writeS("\fI魔法致命一擊: \\aA+" + MagicCritical);
			}
