...
public abstract class ItemDelayTimer implements Runnable {
	public static ItemDelayTimer newTimer(L1Character cha, int item_id, String item_name, long delay_time){
		...
		return new ItemDelayTimerController(cha, item_id, item_name, delay_time * 10, total_time);
	}
	...
	static class DelayInfo {
		...
		public Timestamp skilltime;
	}
	
	public static void loadItemDelay(L1PcInstance pc) {
		List<DelayInfo> delayList = new ArrayList<DelayInfo>();
	...
	public static void SaveItemDelay(L1PcInstance pc) {
		...
		Updator.batch("insert into character_delayitems SET char_id=?, item_id=?, item_name=?, remaining_time=?, log_delay_time=?", new BatchHandler() {
			...
			public void handle(PreparedStatement pstm, int callNumber) throws Exception {
				ItemDelayModel model = delayModels.get(callNumber);
				int idx = 0;
	...
	private static class ItemDelayModel{
		int itemId;
		String ItemName;
		int timeSec;
		Timestamp delaytime;
