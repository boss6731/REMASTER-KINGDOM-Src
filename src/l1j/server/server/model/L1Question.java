	...
	public void run(){
		try{
			switch(_executeStatus)
			{
				...
				{
					mainstart = true;
					L1World.getInstance().broadcastServerMessage(" \fY稍後將開始問卷調查。（限制時間30秒）");
					L1World.getInstance().broadcastServerMessage(" YES = 贊成, NO = 反對, 其他無效~!");
				...
				{
					L1World.getInstance().broadcastPacketToAll(new S_Message_YN(C_Attr.MSGCODE_6008_QUESTION, 6008, maintext));
				...
				{
					L1World.getInstance().broadcastServerMessage("稍後將公佈問卷調查結果。");
				...
				{
					L1World.getInstance().broadcastServerMessage(" \fW[結果] 贊成 : " + good + "票, 反對 : " + bad + "票");
					_instance = null;
					mainstart = false;
					maintext = "";
				}
