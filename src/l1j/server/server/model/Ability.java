...
public class Ability {
	...
	private byte str = 0; // 基礎力量 + 升級或藥劑提升的力量
	private byte baseStr = 0; // 基礎力量
	private byte addedStr = 0; // 由魔法或物品提升的力量
	...
	public int getSp() {
		int total_sp = getTrueSp() + sp;

		if (character.hasSkillEffect(L1SkillId.POTENTIAL)) {
			total_sp += total_sp * 0.2;
		}

		if (character != null && character.isPassive(MJPassiveID.IllUSION_LICH_PASSIVE.toInt())) {
			total_sp += 4;
		}
		return total_sp;
	}
	...
	public int getMagicBonus() {
		...
		/*
		 * if (i <= 5) return -2; else if (i <= 8) return -1; else if (i <= 11) return
		 * 0; else if (i <= 14) return 1; else if (i <= 17) return 2; else if (i <= 24)
		 * return i - 15; else if (i <= 35) return 10; else if (i <= 42) return 11; else
		 * if (i <= 49) return 12; else if (i <= 50) return 13; else return i - 25;
		 */
	}
