...
public abstract class MJNotiSkillDatabaseProvider {
	...
	private static class MJNotiSkillEntireProvider extends MJNotiSkillDatabaseProvider{
		@Override
		Map<Integer, MJNotiSkillModel> models(){
			final Map<Integer, MJNotiSkillModel> models = new HashMap<>();
			Selector.exec("select * from skills_noti", new FullSelectorHandler(){
				@Override
				public void result(ResultSet rs) throws Exception {
					while(rs.next()){
						MJNotiSkillModel model = MJNotiSkillModel.newModel(rs);
						models.put(model.skillId(), model);
					}
				}
			});
			return models;
