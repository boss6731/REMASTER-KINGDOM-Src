...
import l1j.server.server.datatables.TownNpcTax.L1TownNpcTax;
...
public class L1TownLocation {
	...
	private static final Point[] GETBACK_LOC_TALKING_ISLAND = { new Point(32600, 32942), new Point(32574, 32944),
			new Point(32580, 32923), new Point(32557, 32975), new Point(32594, 32917), new Point(32580, 32974), };
	...
	private static final Point[] GETBACK_LOC_OREN = { new Point(34072, 32280), new Point(34037, 32230),
	...
	private static final Point[] GETBACK_LOC_GIRAN_CASTLE = { new Point(33643, 32778), new Point(33621, 32780),
			new Point(33626, 32788), new Point(33635, 32794), new Point(33615, 32795), new Point(33628, 32802),
			new Point(33631, 32775), };
	...
	private static final int[] TOWN_LOC_MIN_TALK_LOCATION = { 32549, 32923 };
	private static final int[] TOWN_LOC_MAX_TALK_LOCATION = { 32602, 33008 };
	private static final int[] TOWN_LOC_MIN_SILVER_LOCATION = { 33056, 33351 };
	private static final int[] TOWN_LOC_MAX_SILVER_LOCATION = { 33144, 33418 };
	private static final int[] TOWN_LOC_MIN_GLUDIO_LOCATION = { 32592, 32716 };
	private static final int[] TOWN_LOC_MAX_GLUDIO_LOCATION = { 32634, 32827 };
	private static final int[] TOWN_LOC_MIN_ORCISH_LOCATION = { 32710, 32421 };
	private static final int[] TOWN_LOC_MAX_ORCISH_LOCATION = { 32779, 32472 };
	private static final int[] TOWN_LOC_MIN_WINDAWOOD_LOCATION = { 32573, 32586 };
	private static final int[] TOWN_LOC_MAX_WINDAWOOD_LOCATION = { 32662, 33287 };
	private static final int[] TOWN_LOC_MIN_KENT_LOCATION = { 33031, 32723 };
	private static final int[] TOWN_LOC_MAX_KENT_LOCATION = { 33082, 32817 };
	private static final int[] TOWN_LOC_MIN_GIRAN_LOCATION = { 33340, 32652 };
	private static final int[] TOWN_LOC_MAX_GIRAN_LOCATION = { 33536, 32892 };
	private static final int[] TOWN_LOC_MIN_HEINE_LOCATION = { 33573, 33207 };
	private static final int[] TOWN_LOC_MAX_HEINE_LOCATION = { 33661, 33452 };
	private static final int[] TOWN_LOC_MIN_WERLDAN_LOCATION = { 33681, 32481 };
	private static final int[] TOWN_LOC_MAX_WERLDAN_LOCATION = { 33747, 32532 };
	private static final int[] TOWN_LOC_MIN_OREN_LOCATION = { 33996, 32203 };
	private static final int[] TOWN_LOC_MAX_OREN_LOCATION = { 34081, 32340 };
	private static final int[] TOWN_LOC_MIN_ADEN_LOCATION = { 33862, 33095 };
	private static final int[] TOWN_LOC_MAX_ADEN_LOCATION = { 34168, 33423 };
	...
	public static int getTownIdByLoc(int x, int y) {
		int townid = 0;
		if (x >= TOWN_LOC_MIN_TALK_LOCATION[0]
				&& y >= TOWN_LOC_MIN_TALK_LOCATION[1]
				&& x <= TOWN_LOC_MAX_TALK_LOCATION[0]
				&& y <= TOWN_LOC_MAX_TALK_LOCATION[1]) {
			townid = TOWNID_TALKING_ISLAND;
		} else if (x >= TOWN_LOC_MIN_SILVER_LOCATION[0]
				&& y >= TOWN_LOC_MIN_SILVER_LOCATION[1]
				&& x <= TOWN_LOC_MAX_SILVER_LOCATION[0]
				&& y <= TOWN_LOC_MAX_SILVER_LOCATION[1]) {
			townid = TOWNID_SILVER_KNIGHT_TOWN;
		} else if (x >= TOWN_LOC_MIN_GLUDIO_LOCATION[0]
				&& y >= TOWN_LOC_MIN_GLUDIO_LOCATION[1]
				&& x <= TOWN_LOC_MAX_GLUDIO_LOCATION[0]
				&& y <= TOWN_LOC_MAX_GLUDIO_LOCATION[1]) {
			townid = TOWNID_GLUDIO;
		} else if (x >= TOWN_LOC_MIN_ORCISH_LOCATION[0]
				&& y >= TOWN_LOC_MIN_ORCISH_LOCATION[1]
				&& x <= TOWN_LOC_MAX_ORCISH_LOCATION[0]
				&& y <= TOWN_LOC_MAX_ORCISH_LOCATION[1]) {
			townid = TOWNID_ORCISH_FOREST;
		} else if (x >= TOWN_LOC_MIN_WINDAWOOD_LOCATION[0]
				&& y >= TOWN_LOC_MIN_WINDAWOOD_LOCATION[1]
				&& x <= TOWN_LOC_MAX_WINDAWOOD_LOCATION[0]
				&& y <= TOWN_LOC_MAX_WINDAWOOD_LOCATION[1]) {
			townid = TOWNID_WINDAWOOD;
		} else if (x >= TOWN_LOC_MIN_KENT_LOCATION[0]
				&& y >= TOWN_LOC_MIN_KENT_LOCATION[1]
				&& x <= TOWN_LOC_MAX_KENT_LOCATION[0]
				&& y <= TOWN_LOC_MAX_KENT_LOCATION[1]) {
			townid = TOWNID_KENT;
		} else if (x >= TOWN_LOC_MIN_GIRAN_LOCATION[0]
				&& y >= TOWN_LOC_MIN_GIRAN_LOCATION[1]
				&& x <= TOWN_LOC_MAX_GIRAN_LOCATION[0]
				&& y <= TOWN_LOC_MAX_GIRAN_LOCATION[1]) {
			townid = TOWNID_GIRAN;
		} else if (x >= TOWN_LOC_MIN_HEINE_LOCATION[0]
				&& y >= TOWN_LOC_MIN_HEINE_LOCATION[1]
				&& x <= TOWN_LOC_MAX_HEINE_LOCATION[0]
				&& y <= TOWN_LOC_MAX_HEINE_LOCATION[1]) {
			townid = TOWNID_HEINE;
		} else if (x >= TOWN_LOC_MIN_WERLDAN_LOCATION[0]
				&& y >= TOWN_LOC_MIN_WERLDAN_LOCATION[1]
				&& x <= TOWN_LOC_MAX_WERLDAN_LOCATION[0]
				&& y <= TOWN_LOC_MAX_WERLDAN_LOCATION[1]) {
			townid = TOWNID_WERLDAN;
		} else if (x >= TOWN_LOC_MIN_OREN_LOCATION[0]
				&& y >= TOWN_LOC_MIN_OREN_LOCATION[1]
				&& x <= TOWN_LOC_MAX_OREN_LOCATION[0]
				&& y <= TOWN_LOC_MAX_OREN_LOCATION[1]) {
			townid = TOWNID_OREN;
		} else if (x >= TOWN_LOC_MIN_ADEN_LOCATION[0]
				&& y >= TOWN_LOC_MIN_ADEN_LOCATION[1]
				&& x <= TOWN_LOC_MAX_ADEN_LOCATION[0]
				&& y <= TOWN_LOC_MAX_ADEN_LOCATION[1]) {
			townid = TOWNID_ADEN;
		}
		return townid;
	}
	...
	public static int[] getGetBackLoc(int town_id) {
		...
		case TOWNID_HIDDEN_VALLEY: { // 숨계진 계곡
			...
			break;
		case TOWNID_GIRAN_CASTLE: { // 기란
			int rnd = random.nextInt(GETBACK_LOC_GIRAN_CASTLE.length);
			loc[0] = GETBACK_LOC_GIRAN_CASTLE[rnd].getX();
			loc[1] = GETBACK_LOC_GIRAN_CASTLE[rnd].getY();
			loc[2] = GETBACK_MAP_GIRAN_CASTLE;
		}
			break;
		default: { // 은기사
			int rnd = random.nextInt(GETBACK_LOC_SILVER_KNIGHT_TOWN.length);
...
//			tax_rate = town.get_tax_rate() + 2; // 原本的稅率計算
//			tax_rate = town.get_tax_rate(); // 修改後的稅率計算
		}
		return tax_rate;
	}
	...
	public static int getTownIdByNpcid(int npcid) {
		...
		if (TownNpcTax.getInstance().getTownNpcInfo(npcid) != null) {
			L1TownNpcTax temp = TownNpcTax.getInstance().getTownNpcInfo(npcid);
			if (temp != null) {
