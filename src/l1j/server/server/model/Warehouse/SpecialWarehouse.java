...
public class SpecialWarehouse extends Warehouse {
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_special_warehouse SET id = ?, char_name = ?, item_id = ?, item_name = ?, count = ?, "
					+ "is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, "
					+ "attr_enchantlvl = ?, bless = ?, special_enchant = ?, package=?, item_level = ?, Hotel_Town =?, end_time =?, bless_type=?, bless_type_value=?");
