...
public class ElfWarehouse extends Warehouse {
	...
	public void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_elf_warehouse SET id = ?, account_name = ?, "
					+ "item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, "
					+ "charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?, special_enchant = ?, "
					+ "item_level = ?, Hotel_Town =?, end_time =?, carving =?, doll_bonus_level =?, doll_bonus_value =?,bless_type=?, bless_type_value=?");
