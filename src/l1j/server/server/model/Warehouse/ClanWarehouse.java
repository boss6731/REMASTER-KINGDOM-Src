...
public class ClanWarehouse extends Warehouse {
		...
		try {
			...
			while (rs.next()) {
				...
				if (itemTemplate == null) {
					try{
						throw new IllegalArgumentException(String.format("item_id=%d not found.", itemId));
					}catch(Exception e){
						e.printStackTrace();
					}
					continue;
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO clan_warehouse SET id = ?, clan_name = ?, item_id = ?, "
					+ "item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id= ?, durability = ?, charge_count = ?, "
					+ "remaining_time = ?, last_used = ?, attr_enchantlvl = ?, special_enchant = ?, package = ?, item_level = ?, "
					+ "Hotel_Town =?, end_time =?, doll_bonus_level =?, doll_bonus_value =?,bless_type=?, bless_type_value=?");
