...
public class SupplementaryService extends Warehouse {
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			...
			if (!rs.next()) {
				pstm = con.prepareStatement("INSERT INTO character_supplementary_service SET id = ?, account_name = ?, item_id = ?, "
						+ "item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, "
