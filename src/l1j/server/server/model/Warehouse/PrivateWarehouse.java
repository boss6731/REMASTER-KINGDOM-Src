...
public class PrivateWarehouse extends Warehouse {
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_warehouse SET id = ?, account_name = ?, item_id = ?, "
					+ "item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, "
					+ "remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?, special_enchant = ?, package=?, "
					+ "item_level =?, Hotel_Town =?, end_time =?, carving =?, bless_level=?, doll_bonus_level=?, "
					+ "doll_bonus_value=?, bless_type=?, bless_type_value=?");
			...
			pstm.setInt(19, item.get_Carving());
			pstm.setInt(20, item.get_bless_level());
			pstm.setInt(21, item.get_Doll_Bonus_Level());
			pstm.setInt(22, item.get_Doll_Bonus_Value());
			pstm.setInt(23, item.getBlessType());
			pstm.setInt(24, item.getBlessTypeValue());
			pstm.execute();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
