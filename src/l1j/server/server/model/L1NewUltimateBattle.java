...
public class L1NewUltimateBattle {
	...
	private boolean _enterWarrior;  // 전사 = 戰士 (Warrior)
	...
	private void sendRoundMessage(int curRound) {
		switch (_ubId) {
			...
			case 5:
				if (curRound == 1) {
					sendMessage("科洛塞奧管理員: 第1軍投入!"); // 開始
				} else if (curRound == 2) {
					sendMessage("科洛塞奧管理員: 第2軍投入!"); // 開始
				} else if (curRound == 3) {
					sendMessage("科洛塞奧管理員: 第3軍投入!"); // 開始
				} else if (curRound == 4) {
					sendMessage("科洛塞奧管理員: 最終戰開始！限制時間為5分鐘"); // 開始
				}
				break;
			case 4:
				if (curRound == 1) {
	...
	private void spawnSupplies(int curRound) {
		switch (_ubId) {
			...
			case 2:
				...
				// spawnGroundItem(3000156, 1, 5);
				sendMessage("科洛塞奧管理員: 第2軍的投入已完成。");
				sendMessage("科洛塞奧管理員: 2分鐘後第3軍將開始投入。");
		} else if (curRound == 3) {
			...
			spawnGroundItem(40079, 1, 10);
			// spawnGroundItem(3000156, 1, 5);
			sendMessage("科洛塞奧管理員: 第3軍的投入已完成。");
			sendMessage("科洛塞奧管理員: 6分鐘後將開始最終戰。");
		}
				break;
			case 3:
				if (curRound == 1) {
					spawnGroundItem(40093, 1, 10);
					// spawnGroundItem(3000156, 1, 10);
					sendMessage("科洛塞奧管理員: 第2軍的投入已完成。");
					sendMessage("科洛塞奧管理員: 2分鐘後第3軍將開始投入。");
				} else if (curRound == 3) {
			...
			case 4:
				if (curRound == 1) {
					...
					spawnGroundItem(40079, 1, 10);
					// spawnGroundItem(3000156, 1, 15);
					sendMessage("科洛塞奧管理員: 第1軍的投入已完成。");
					sendMessage("科洛塞奧管理員: 2分鐘後第2軍將開始投入。");
				} else if (curRound == 2) {
			...
			case 5:
				if (curRound == 1) {
					...
					spawnGroundItem(40317, 1, 5);
					spawnGroundItem(40079, 1, 10);
					// spawnGroundItem(3000156, 1, 20);
					sendMessage("科洛塞奧管理員: 第1軍的投入已完成。");
					sendMessage("科洛塞奧管理員: 1分鐘後第2軍將開始投入。");
				} else if (curRound == 2) {
					spawnGroundItem(L1ItemId.ADENA, 5000, 50);
					spawnGroundItem(L1ItemId.POTION_OF_CURE_POISON, 7, 20);
					spawnGroundItem(L1ItemId.POTION_OF_EXTRA_HEALING, 10, 20);
...
class UbThread implements Runnable {
	private void countDown() throws InterruptedException {
		...
		sendMessage("競技場管理員：現在怪物即將出現。祝你們好運。"); // 10秒前
		Thread.sleep(5000);
		sendMessage("競技場管理員：10秒後比賽開始。"); // 10秒前

		Thread.sleep(5000);
		sendMessage("競技場管理員：5 !!"); // 5秒前

		Thread.sleep(1000);
		sendMessage("競技場管理員：4 !!"); // 4秒前

		Thread.sleep(1000);
		sendMessage("競技場管理員：3 !!"); // 3秒前

		Thread.sleep(1000);
		sendMessage("競技場管理員：2 !!"); // 2秒前

		Thread.sleep(1000);
		sendMessage("競技場管理員：1 !!"); // 1秒前
		...
		private void waitForNextRound(int curRound) throws InterruptedException {
			final int WAIT_TIME_TABLE[] = { 9, 12, 15, 18 };// �ð�
	...
	public void start() {
		switch (getUbId()) {
			// case 1: L1World.getInstance().broadcastServerMessage("由於多個內容，奇岩村的無限大戰已停止。"); break;
			case 2:
				L1World.getInstance().broadcastServerMessage("稍後，威爾頓村的無限大戰即將開始。");
				break;
			case 3:
				L1World.getInstance().broadcastServerMessage("稍後，古魯丁村的無限大戰即將開始。");
				break;
			case 4:
				L1World.getInstance().broadcastServerMessage("稍後，說話之島的無限大戰即將開始。");
				break;
			case 5:
				L1World.getInstance().broadcastServerMessage("稍後，銀騎士村的無限大戰即將開始。");
				break;
			default:
				break;
		}
	...
	public boolean canPcEnter(L1PcInstance pc) {
		...
		if (!((pc.isCrown() && _enterRoyal) || (pc.isKnight() && _enterKnight) || (pc.isWizard() && _enterMage) || (pc.isElf() && _enterElf) || (pc.isDarkelf() && _enterDarkelf)
				|| (pc.isDragonknight() && _enterDragonknight) || (pc.isBlackwizard() && _enterBlackwizard) || (pc.isWarrior() && _enterWarrior) || (pc.isFencer() && _enterFencer) || (pc.isLancer() && _enterLancer))) {
			return false;
		}
	...
	public String[] makeUbInfoStrings() {
		...
		String teleport = _location.getMap().isEscapable() ? "可行" : "不可行";
		String res = _location.getMap().isUseResurrection() ? "可行" : "不可行";
		String pot = "可行";
		String hpr = String.valueOf(_hpr);
		String mpr = String.valueOf(_mpr);
		String summon = _location.getMap().isTakePets() ? "可行" : "不可行";
		String summon2 = _location.getMap().isRecallPets() ? "可行" : "不可行";
