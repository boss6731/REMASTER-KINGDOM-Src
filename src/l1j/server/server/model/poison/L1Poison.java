...
public abstract class L1Poison {
	protected static boolean isValidTarget(L1Character cha) {
		...
		if (cha.hasSkillEffect(L1SkillId.ICE_LANCE) || cha.hasSkillEffect(L1SkillId.MOB_COCA) || cha.hasSkillEffect(L1SkillId.MOB_BASILL) || cha.hasSkillEffect(L1SkillId.EARTH_BIND)) {
			return false;
		}
		...
		if (player.getInventory().checkEquipped(20298)// 제니스의반지
				...
				|| player.getInventory().checkEquipped(900263)// 할파스의 완력
				|| player.getInventory().checkEquipped(900264)// 할파스의 예지력
				|| player.getInventory().checkEquipped(900265)// 할파스의 마력
