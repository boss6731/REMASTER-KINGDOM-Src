...
public class L1DamagePoison extends L1Poison {
			...
			do {
				...
				if (_target instanceof L1PcInstance) {
					...
					if (_target.hasSkillEffect(L1SkillId.STATUS_TOMAHAWK)) {
						_target.setPoisonEffect(0);// 독이미지 실시간 제거
						L1SkillUse.on_icons(player, L1SkillId.TOMAHAWK, 1000 / 1000);
	...
	private void doInfection() {
		if (_tomahawk) {
			...
			if (_target instanceof L1PcInstance) {
				...
				pc.sendPackets(new S_PacketBox(S_PacketBox.POSION_ICON, pc, 1, 30));// 포이즌
			}
	...
	public void cure() {
		...
		if (_tomahawk) {
			...
			_target.setPoisonEffect(0);// 독이미지 마지막 제거
			_target.sendPackets(new S_ServerMessage(3993));
		} else {
			if (_target instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) _target;
