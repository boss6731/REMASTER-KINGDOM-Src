...
public class L1NpcInstance extends L1Character {
	...
	// private static final long DELETE_TIME = 40000L; // 怪物刪除時間40秒原本
...
//		_aStar = new NpcAStar(getMap(), new Point(0, 0));
	...
	private boolean AIProcess() {
		...
		/*
		 * if (this instanceof L1SummonInstance) { if (this.getMapId() == 4 &&
		 * getMap().isSafetyZone(this.getLocation())) { deleteMe(); } }
		 */
...
/*	private void doAStarMove() {
		try {
			...
			// System.out.println("NPC錯誤1 : " + this.getNpcId());
		}
	}
*/
...
//			}
			...
			// System.out.println("NPC錯誤2 : " + this.getNpcId());
		}
	}*/
...
//			if(getAtkspeed() == 0 && getPassispeed() == 0)return;
			...
			if(getNpcId() == 8513)return;//검은 기사단 요리사
...
//					setSleepTime(calcSleepTime(mobSkill.getSleepTime(), MAGIC_SPEED));
				...
				if(getNpcId() == 45164 && getMapId() == 9 && target.getMapId() == 9 && (targetx == 32663 && targety == 33149 || targetx == 32669 && targety == 33158 || targetx == 32669 && targety == 33161) && getLocation().getTileDistance(target.getLocation()) <= 2){
					...
					if(isMovable()){
						...
						if(firstFound && getNpcTemplate().is_teleport() && distance > 3 && distance < 15 && nearTeleport(targetx, targety) == true){
							firstFound = false;
							return;
						}
						...
						if((getNpcId() == 7800007 || getNpcId() == 7800064) && (getX() > 32790 && getX() < 32808 && getY() > 32855 && getY() < 32872)){// 글루디오 연구실
							return;
						}
						...
						if(getNpcId() == 7800007 || getNpcId() == 7800064
								...
								|| (getNpcId() >= 7800060 && getNpcId() <= 7800063)){// 글루디오 연구실
...
//								setDirectionMove(dir);
						...
						if(dir == -1){
							...
							boolean door = World.문이동(getX(), getY(), getMapId(), calcheading(this, targetx, targety));
							boolean tail = World.isThroughObject(getX(), getY(), getMapId(), dir);

							if((getNpcId() >= 7200008 && getNpcId() <= 7200020) || getNpcId() == 7200055 || getNpcId() == 7200056 || (getNpcId() >= 7200030 && getNpcId() <= 7200041)){ //유니콘 사원
								door = false;
								tail = true;
							}
...
//				_moveType = MOVE_TYPE_NORMAL;
	...
	public void attackTarget(L1Character target) {
		...
		if (attack.calcHit()) {
			...
			} else if (target.isPassive(MJPassiveID.CONQUEROR.toInt())) { //확률 작업해야됨
						...
						if (temp.isMentuse()) {
							...
							if (temp.getMent() != null) {
								...
								String message = String
										.format("" + men + " %s \fH已從" + locationName + "獲得了。", itemName);
...
/*		L1World.getInstance().removeVisibleObject(this);
    ...
    //			broadcastPacket(new S_MoveCharPacket(this), true);
                ...
                if((npcid >= 45912 && npcid <= 45916) && getMapId() == 4){//글루딘 유령 원래 자리로 이동 시키기
                    if(!(getX() >= 32591 && getX() <= 32644 && getY() >= 32643 && getY() <= 32688 && getMapId() == 4))
                        teleport(getHomeX(), getHomeY(), getMoveState().getHeading());
                }else if((npcid == 45752 || npcid == 45753) && getMapId() == 15404){//발록 문밖으로 나가면 이동 시키기
                    if(!(getX() >= 32720 && getX() <= 32742 && getY() >= 32851 && getY() <= 32877))
                        teleport(getHomeX(), getHomeY(), getMoveState().getHeading());
                }else if(npcid == 7220091 && getMapId() == 1319){//분노한 발록
                    if(!(getX() >= 32719 && getX() <= 32785 && getY() >= 32919 && getY() <= 33002))
                        teleport(getHomeX(), getHomeY(), getMoveState().getHeading());
                }else if((npcid == 800185 || npcid == 800177 || npcid == 800187 || npcid == 800188) && getMapId() == 12853){//감시자 시어
                    if(!(getX() >= 32786 && getX() <= 32815 && getY() >= 32781 && getY() <= 32813))
                        teleport(getHomeX(), getHomeY(), getMoveState().getHeading());
                }else if((npcid == 800186 || npcid == 800178) && getMapId() == 12854){//박쥐 뱀파이어
                    if(!(getX() >= 32787 && getX() <= 32817 && getY() >= 32779 && getY() <= 32810))
                        teleport(getHomeX(), getHomeY(), getMoveState().getHeading());
                }else if((npcid == 800186 || npcid == 800178) && getMapId() == 12857){//모래바람 머미로드
			...
			if (getMovementDistance() > 0) {
				if (this instanceof L1GuardInstance || this instanceof L1CastleGuardInstance || this instanceof L1MerchantInstance || this instanceof L1MonsterInstance) {
					if (getLocation().getLineDistance(new Point(getHomeX(), getHomeY())) > getMovementDistance()) {
						// TODO 如果在NPC表中設置為true，則僅該怪物使用瞬間移動回原地。
...
//						}
	...
	public int directionFromCurrent(int cx, int cy, int tx, int ty) {
		float dis_x = Math.abs(cx - tx); // X방향의 타겟까지의 거리
		float dis_y = Math.abs(cy - ty); // Y방향의 타겟까지의 거리
		...
		try {
			...
			try {
				if(tail != null){
					...
					aStar.cleanTail();
					tail = aStar.nearestSearchTile(this, x, y, m, true);
		...
		protected DeleteTimer(int oId) {
			try {
				...
				if (!(L1World.getInstance().findObject(_id) instanceof L1NpcInstance)) {
					throw new IllegalArgumentException("allowed only L1NpcInstance [不要在意。]");
				}
			} catch (Exception e) {
				e.printStackTrace();
