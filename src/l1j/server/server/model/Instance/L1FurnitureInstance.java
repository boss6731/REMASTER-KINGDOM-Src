...
public class FurnitureSpawnTable {
	...
	private <L1FurnitureInstance> void FillFurnitureSpawnTable() {
		Connection con = null;
		...
		try {
			...
			do {
				...
				if (l1npc != null) {
					...
					l1j.server.server.model.Instance.L1FurnitureInstance furniture = (L1FurnitureInstance) constructor.newInstance(parameters);
	...
	public <L1FurnitureInstance> void insertFurniture(L1FurnitureInstance furniture) {
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
