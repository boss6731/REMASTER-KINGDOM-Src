...
public class L1DoorInstance extends L1NpcInstance {
	...
	public void receiveDamage(L1Character attacker, int damage) {
		//System.out.println("城門傷害 : " + damage);
		if (getMaxHp() == 0 || getMaxHp() == 1) {
			return;
		}
		...
		if (getCurrentHp() > 0 && !isDead()) {
			...
			if (newHp > 0) {
				...
				//System.out.println("城門被攻破了嗎? : " + newHp);
				setCurrentHp(newHp);
	...
	public void open() {
		...
		if (getOpenStatus() == ActionCodes.ACTION_Close) {
			...
			if (this.getDoorId() == 2110) {
				GeneralThreadPool.getInstance().schedule(new DoorTimer(), 60000);
			}
			if (this.getDoorId() == 2111) {
				GeneralThreadPool.getInstance().schedule(new DoorTimer(), 60000);
			}
			if (this.getDoorId() == 2112) {
				GeneralThreadPool.getInstance().schedule(new DoorTimer(), 60000);
			}
			if (this.getDoorId() == 2113) {
				GeneralThreadPool.getInstance().schedule(new DoorTimer(), 60000);
			}
...
/*
	public void openglass() {
		...
		if (getStatus() == ActionCodes.ACTION_SpellDirection) {
			broadcastPacket(new S_DoActionGFX(this.getId(),ActionCodes.ACTION_Entry));
			setOpenStatus(ActionCodes.ACTION_Open);
			setPassable(L1DoorInstance.PASS);
			sendDoorPacket(null);
		}
...
//		if (getFloorStatus() == ActionCodes.ACTION_Entry) {
			isPassibleDoor(false);//하딘 시스템
			broadcastPacket(new S_DoActionGFX(this.getId(),ActionCodes.ACTION_SpellDirection));
			setOpenStatus(ActionCodes.ACTION_Close);
			setPassable(L1DoorInstance.NOT_PASS);
			sendDoorPacket(null);
//		}
