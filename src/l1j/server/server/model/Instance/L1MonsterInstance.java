...
import l1j.server.server.model.poison.L1Osiris2;
...
public class L1MonsterInstance extends l1j.server.server.model.Instance.L1NpcInstance {
	...
	public void onItemUse() {
		if (!isActived() && _target != null) {
			...
			if (getNpcTemplate().is_doppel() && _target != null) {
				l1j.server.server.model.Instance.L1PcInstance targetPc = (l1j.server.server.model.Instance.L1PcInstance) _target;
				...
				for (l1j.server.server.model.Instance.L1PcInstance pc : L1World.getInstance().getRecognizePlayer(this)) {
					if (pc == null)
						continue;
	...
	@Override
	public void onPerceive(l1j.server.server.model.Instance.L1PcInstance perceivedFrom) {
...
//			perceivedFrom.sendPackets(new S_PinkName(getId(), 20));
			if (getHiddenStatus() == HIDDEN_STATUS_SINK) {
				...
				if (getCurrentSpriteId() == 15492 || getCurrentSpriteId() == 8055)
					// 測試並使用61號和13號以確保流暢度
					perceivedFrom.sendPackets(new S_DoActionGFX(getId(), 13));
				else
	...
	public static int[][] _classGfxId = { { 0, 1 }, { 20553, 61 }, { 37, 138 }, { 20278, 20279 }, { 2786, 2796 }, { 6658, 6661 }, { 6671, 6650 }, { 20567, 20577 }, { 18520, 18499 }, { 19296, 19299 } };
	...
	public void searchTarget() {
		...
		if (col.size() > 0) {
			...
			for (L1PcInstance pc : col) {
				...
				if (pc.hasSkillEffect(L1SkillId.BLIND_HIDING) && pc.isPassive(MJPassiveID.BLIND_HIDDING_ASSASSIN.toInt())){
					continue;
				}

				// TODO 當PC對NPC時，如果等級較高則處理為非攻擊
				if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) {
					if (pc.hasSkillEffect(L1SkillId.SHAPE_CHANGE)) {
						...
						} else if (getNpcTemplate().is_agrososc()) {
							target_q.offer(pc);
							continue;
						}
					} else if (getNpcTemplate().isAgro_lvl()) {
						if (pc.getLevel() > getNpcTemplate().get_level()) {
							continue;
						} else {
							target_q.offer(pc);
			...
			while (!target_q.isEmpty()) {
				...
				if (target != null && !target.isDead()) {
					if (backingTargets.contains(target.getId())) {
						continue;
					}
					targetPlayer = target;
					break;
		 ...
		 * 4. Monster vs Npc
		 * 為了上述情況，新增了載入物件的功能，目前僅針對第1種情況的代碼。
		 * 簡單地將物件聲明為Instance of即可設定。
		 */
		for (L1Object obj : L1World.getInstance().getVisibleObjects(this)) {
			if (obj instanceof L1MonsterInstance) {
				...
				if (this.getNpcTemplate().get_npcId() == 7310130 || this.getNpcTemplate().get_npcId() == 7310131 || this.getNpcTemplate().get_npcId() == 7310132
						|| this.getNpcTemplate().get_npcId() == 7310133) {// 敵人
					if (mon.getNpcTemplate().get_npcId() >= 5000119 || mon.getNpcTemplate().get_npcId() <= 5000126) { // 作為敵人
						targetMonster = mon;
						break;
					}
				}
				if (this.getNpcTemplate().get_npcId() == 45570) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
					}
				}

				if (this.getNpcTemplate().get_npcId() == 45571) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45582) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45587) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45605) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45685) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45391 || mon.getNpcTemplate().get_npcId() == 45450 || mon.getNpcTemplate().get_npcId() == 45482 || mon.getNpcTemplate().get_npcId() == 45569
							|| mon.getNpcTemplate().get_npcId() == 45579 || mon.getNpcTemplate().get_npcId() == 45315 || mon.getNpcTemplate().get_npcId() == 45647) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45391) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
					}
				}

				if (this.getNpcTemplate().get_npcId() == 45450) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45482) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45569) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45579) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45315) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 45647) { // 感知敵人
					if (mon.getNpcTemplate().get_npcId() == 45570 || mon.getNpcTemplate().get_npcId() == 45571 || mon.getNpcTemplate().get_npcId() == 45582 || mon.getNpcTemplate().get_npcId() == 45587
							|| mon.getNpcTemplate().get_npcId() == 45605) { // 作為敵人
						targetMonster = mon;
						break;
				...
				if (this.getNpcTemplate().get_npcId() == 8503173) { // 會感知敵人的怪物（祭司）
					if (mon.getNpcTemplate().get_npcId() == 8503172 || mon.getNpcTemplate().get_npcId() == 8503174) { // 會被感知為敵人的怪物
						targetMonster = mon;
						break;
					}
				}
				if (this.getNpcTemplate().get_npcId() == 8503174) { // 會感知敵人的怪物（祭司）
					if (mon.getNpcTemplate().get_npcId() == 8503172 || mon.getNpcTemplate().get_npcId() == 8503173) { // 會被感知為敵人的怪物
						targetMonster = mon;
						break;
					}
				}
				if (this.getNpcTemplate().get_npcId() == 8503176) { // 會感知敵人的怪物（祭司）
					if (mon.getNpcTemplate().get_npcId() == 8503172) { // 會被感知為敵人的怪物
						targetMonster = mon;
						break;
					}
				}
				if (this.getNpcTemplate().get_npcId() == 8503177) { // 會感知敵人的怪物（祭司）
					if (mon.getNpcTemplate().get_npcId() == 8503178) { // 會被感知為敵人的怪物
						targetMonster = mon;
						break;
					}
				}
	...
	public synchronized void receiveDamage(L1Character attacker, int damage) {
		...
		if (newHp <= 0 && !isDead()) {
			...
			if(getNpcTemplate().get_npcId() == 120910) {
				if(!LuunDugeon.getInstance().isactive) {
					...
					LuunDugeon.getInstance().LuunDungeonStart();
				}
			}
			...
			if (transformId == -1) {
				...
				L1TransFormCheck.removeTransFormList(getMapId(), getNpcTemplate().get_npcId());
			} else {
				if (attacker instanceof L1PcInstance) {
					L1PcInstance player = (L1PcInstance) attacker;
					if (HunterEventDoll(player)) {
				...
				if (transformId == 120699 || transformId == 120700 || transformId == 120701 || transformId == 120702 || transformId == 120703 ||
						transformId == 120704 || transformId == 120705 || transformId == 120706 || transformId == 120707 || transformId == 120708) {
					if (MJRnd.isWinning(1000000, getNpcTemplate().getTransformProbability()) && getMap().isCrackIntheTower()) {
						if (getNpcTemplate().isTransformHard()) {
							if (L1TransFormCheck.isTransFormList(getMapId(), transformId)) {
								setCurrentHp(0);
								setStatus(ActionCodes.ACTION_Die);
								...
								setDead(false);
								L1TransFormCheck.addTransFormList(getMapId(), transformId);
								transform(transformId, attacker);
							}
						} else {
							setDead(false);
							transform(transformId, attacker);
				...
				} else if (MJRnd.isWinning(1000000, getNpcTemplate().getTransformProbability())) {
					if (getNpcTemplate().isTransformHard()) {
						if (L1TransFormCheck.isTransFormList(getMapId(), transformId)) {
							setCurrentHp(0);
							setStatus(ActionCodes.ACTION_Die);
			...
			if (attacker instanceof L1PcInstance) {
				...
				if (getNpcClassId() != 0 && !pc.noPlayerCK && BQSLoadManager.BQS_IS_ONUPDATE_BOOKS) {
					BQSInformation bqsInfo = BQSCriteriaNpcMappedService.service().findInformationFromNpcClassId(getNpcClassId());
					BQSCharacterData bqs = pc.getBqs();
					if (bqsInfo != null && bqs != null)
						bqs.onUpdate(bqsInfo);
				}

				if(CPMWBQNpcInfoTable.Get_Info(getNpcClassId()) != null && !pc.noPlayerCK && CPMWBQReward.service().use()) {
					CPMWBQSystemProvider.provider().BQUpdateAmount(pc, this);
				}
				...
				if (getNpcTemplate().get_npcId() == 7320014) {
					pc.setBuffnoch(1);
					new L1SkillUse().handleCommands(pc, L1SkillId.BLESS_WEAPON, pc.getId(), pc.getX(), pc.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
					pc.setBuffnoch(0);
				}
				if (getNpcTemplate().get_npcId() == 7320015) {
					pc.setBuffnoch(1);
					new L1SkillUse().handleCommands(pc, L1SkillId.PATIENCE, pc.getId(), pc.getX(), pc.getY(), null, 0, L1SkillUse.TYPE_GMBUFF);
					pc.setBuffnoch(0);
				}
				...
				if(getNpcTemplate().get_npcId() == 8500129 && getMapId() == 12862 && !MJempleantiqueController.getInstance().isopen && MJempleantiqueController.templeantique.use) {
					L1SpawnUtil.spawnnpc(getX(), getY(), (short) getMapId(), 120718, 0, MJempleantiqueController.templeantique.gametime * 1000, 0);
					MJempleantiqueController.getInstance().Start(System.currentTimeMillis());
					ClanTable.getInstance().TempleantiqueclanId = pc.getClanid();
					L1World.getInstance().broadcastServerMessage("\\aH지배의 탑 정상에 균열의 틈새가 발생 되었습니다. 균열의 틈새를 찾으세요.");
				}
		...
		if (getCurrentHp() > 0 && !isDead()) {
			...
			if (attacker instanceof L1PcInstance && damage > 0) {
				...
				BonusDropSystemInfo bInfo = BonusDropSystemLoader.getInstance().getBonusDropSystemInfo(this.getNpcId());
				if (bInfo != null) {
					if (player.getBonusDropNpc() != this.getId())
						player.setBonusDropNpc(this.getId());
				}

				// TODO HP 50이하였을때 true면 주변으로 텔레포트 해서 피한다.
				if (newHp > 0 && getNpcTemplate().is_teleport_run() && 70 > _random.nextInt(100) && 100 * getCurrentHp() / getMaxHp() <= 50 && !_run_teleport) {
					if (RunTeleport(player.getX() + CommonUtil.random(-5, 5), player.getY() + CommonUtil.random(-5, 5)) == true) {
						_run_teleport = true;
						broadcastPacket(SC_WORLD_PUT_OBJECT_NOTI.make_stream(this)); // 추가
					}
				}
			}

			if (!Escape) {
				L1NewNpcChat npcChat = NewNpcChatTable.getInstance().getTemplateEscape(this.getNpcId());
				if (npcChat != null) {
					int hpRatio = 100;
					if (0 < getMaxHp()) {
						hpRatio = 100 * getCurrentHp() / getMaxHp();
					}
					if (hpRatio < npcChat.getMentChance()) {
						Escape = true;
						STATUS_Escape = true;
						...
						GeneralThreadPool.getInstance().schedule(new RunTimer(), 500);
					}
				}
			}
		...
		if (attacker != null && FatigueProperty.getInstance().use_fatigue() && attacker instanceof L1PcInstance) {
			L1PcInstance attacker_pc = (L1PcInstance) attacker;
			Account account = attacker_pc.getAccount();
			if (account != null && attacker_pc.getAI() == null && account.has_fatigue()) {
				damage -= (damage * FatigueProperty.getInstance().get_fatigue_effect_damage());
			}
		}
	}
	...
	/** 몬스터와의 distance 거리 안에 있다면 true 없다면 false */
	private boolean isDistance(L1PcInstance pc, int distance) {
		return getLocation().getTileLineDistance(pc.getLocation()) < distance;
	}

	public void giveAinhasadBonus(L1Character attacker, ArrayList<L1Character> hateList, Integer ainhasadBonus) {
		L1Character _attacker = attacker;
		ArrayList<L1Character> _hateList = hateList;
		int _ainhasadBonus;
		_ainhasadBonus = ainhasadBonus;
		for (L1Character cha : _hateList) {
			if (cha.instanceOf(MJL1Type.L1TYPE_PC)) {
				L1PcInstance pc = (L1PcInstance) cha;
				if (pc == null || pc.noPlayerCK || pc.noPlayerck2 || pc.noPlayerRobot || pc.isDead() || _attacker.getMapId() != pc.getMapId() || !isDistance(pc, 20))
					continue;
				pc.getAccount().addBlessOfAin((_ainhasadBonus * 10000), pc, "몬스터");
			}
		}
	}
	...
	private void DominanceMonster(L1Character lastAttacker) {
		int chance = _random.nextInt(10000) + 1;// 보스 소환 확률
		int boss = 0;
		String name = lastAttacker.getName();
		String bossName = "";
		switch (getNpcId()) {// 소막 버그 방지
			...
			case 8500109:// 10층
				if (chance < 3) {
					if (getMapId() == 12852) {
						boss = 8502001;
						bossName = "$12400 $26980";
					} else if (getMapId() == 12853) {
						boss = 8502002;
						bossName = "$12401 $26980";
					} else if (getMapId() == 12854) {
						boss = 8502003;
						bossName = "$12402 $26980";
					} else if (getMapId() == 12855) {
						boss = 8502004;
						bossName = "$12403 $26980";
					} else if (getMapId() == 12856) {
						boss = 8502005;
						bossName = "$12404 $26980";
					} else if (getMapId() == 12857) {
						boss = 8502006;
						bossName = "$12405 $26980";
					} else if (getMapId() == 12858) {
						boss = 8502007;
						bossName = "$12406 $26980";
					} else if (getMapId() == 12859) {
						boss = 8502008;
						bossName = "$12407 $26980";
					} else if (getMapId() == 12860) {
						boss = 8502009;
						bossName = "$12408 $26980";
					} else if (getMapId() == 12861) {
						boss = 8502010;
						bossName = "$19939 $26980";
					}

					if (boss != 0) {
						L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("\\fY" + bossName + ":\\aH'" + name + "'\\fY인간 주제에 나의 봉인을 풀어 주었구나..너에게 죽음을 선물하지.."));
						L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY" + bossName + ":\\aH'" + name + "'\\fY인간 주제에 나의 봉인을 풀어 주었구나..너에게 죽음을 선물하지.."));
	...
	class Death implements Runnable {
		...
		@Override
		public void run() {
			MJObjectEventProvider.provider().monsterEventFactory().fireMonsterKill(_lastAttacker, L1MonsterInstance.this);
			...
			setKarma(0);
			startDeleteTimer();
			DominanceMonster(_lastAttacker);
			calcCombo(_lastAttacker);

			if (_lastAttacker != null) {
				if (_lastAttacker instanceof L1PcInstance) {
					L1PcInstance attacker_pc = (L1PcInstance) _lastAttacker;
					if (FatigueProperty.getInstance().use_fatigue()) {
						Account account = attacker_pc.getAccount();
						if (account != null && attacker_pc.getAI() == null) {
							account.inc_fatigue_point(attacker_pc);
						}
					}
				}
			}
			...
			if(_lastAttacker instanceof L1PcInstance) {
				L1PcInstance lat = (L1PcInstance)_lastAttacker;
				List<CustomQuest> cq_list = ServerCustomQuestTable.getInstance().getCustomQuestListByMap(getMapId());
				if(cq_list != null && cq_list.size() > 0) {
					for(CustomQuest cq : cq_list)
						cq.result(lat);
				}
			}

			// TODO Count_map + spawnlist_door : 문지기 관련
	...
	private void distributeExpDropKarma(L1Character lastAttacker) {
		...
		if (pc != null && !pc.noPlayerCK && !pc.noPlayerck2 && !pc.isDead()) {
			...
			Integer ainhasadBonus = AinhasadBonusMonsterTable.getInstance().getAinhasadBonus(getNpcTemplate().get_npcId());
			if (ainhasadBonus != null) {
				giveAinhasadBonus(lastAttacker, targetList, ainhasadBonus);
			}
			...
			long total = exp * level;


			/*
			 * int min_level = 127;
			 *
			 * if (pc.isInParty()) { for (L1PcInstance member : pc.getParty().getMembers()) { min_level = Math.min(member.getLevel(), min_level); } level = min_level; }
			 */

			if (level < 55)
				level = 55;

			int levelgapm = level - getLevel();
			if (levelgapm >= 5 && levelgapm <= 9) {
				total = (int) ((exp * level) * 0.90);
			} else if (levelgapm >= 10 && levelgapm <= 14) {
				total = (int) ((exp * level) * 0.85);
			} else if (levelgapm >= 15 && levelgapm <= 19) {
				total = (int) ((exp * level) * 0.80);
			} else if (levelgapm >= 20) {
				total = (int) ((exp * level) * 0.70);
			}

			int levelgapt = getLevel() - level;
			if (levelgapt >= 5 && levelgapt <= 9) {
				total = (int) ((exp * level) * 1.1);
			} else if (levelgapt >= 10 && levelgapt <= 14) {
				total = (int) ((exp * level) * 1.15);
			} else if (levelgapt >= 15 && levelgapt <= 19) {
				total = (int) ((exp * level) * 1.20);
			} else if (levelgapt >= 20) {
				total = (int) ((exp * level) * 1.30);
			}

			if (pc.isGm()) {
				pc.sendPackets("\\f3[" + getName() + "]의 경험치는 (\\f2" + total + "\\f3) 입니다.\\f4(서버배율 미포함)");
			}
			...
			CalcExp.calcExp(pc, getId(), targetList, hateList, total);
			...
			if (hateList.size() != 0) {
				...
				long exp = get_exp();
				int level = lastAttacker.getLevel();
				if (exp > 127) {
					level = 1;
				}
				long total = exp * level;
				 ...
				 * int min_level = 127;
				 *
				 * if (pc.isInParty()) { for (L1PcInstance member : pc.getParty().getMembers()) { min_level = Math.min(member.getLevel(), min_level); }
				 *
				 * level = min_level; }
				 */
...
//		System.out.println(String.format("아이템 지급 횟수 : %d, 몬스터 킬수 : %d, 경험치 지급 횟수 : %d", pc.get_dead_count(), pc.getMonsterkill(), pc.get_exp_count()));
	}
	...
	private void giveKarma(L1PcInstance pc) {
		...
		if (karma != 0) {
			...
			pc.addKarma((int) (karma * Config.ServerRates.RateKarma));
...
//		if (getUbSealCount() != 0) {
//			L1Colosseum ub = ColosseumTable.getInstance().getUb(getUbId());
//			if (ub != null) {
//				for (L1PcInstance pc : ub.getMembersArray()) {
//
//					int exp = 50000;
//					int settingEXP = (int) Config.ServerRates.RateXp;
//					double exppenalty = ExpTable.getPenaltyRate(pc.getLevel());
//					double addyExp = getUbSealCount() * exp * settingEXP * exppenalty;
//
//					if (pc != null && !pc.isDead() && !pc.isGhost()) {
//						if (getNpcId() == 62012) {
//							pc.add_exp((int) addyExp);
//							pc.sendPackets(new S_SystemMessage("1군 보스 클리어 보상: 경험치 획득"));
//						}
//						if (getNpcId() == 62024) {
//							pc.add_exp((int) addyExp);
//							pc.sendPackets(new S_SystemMessage("2군 보스 클리어 보상: 경험치 획득"));
//						}
//						if (getNpcId() == 62037) {
//							pc.add_exp((int) addyExp);
//							pc.sendPackets(new S_SystemMessage("최종 보스 클리어 보상: 경험치 획득"));
//						}
//					}
//				}
//			}
		...
		if (getUbSealCount() != 0) {
			...
			if (ub != null) {
				for (L1PcInstance pc : ub.getMembersArray()) {
					...
					double exppenalty = ExpTable.getPenaltyRate(pc.getLevel());
					double addyExp = getUbSealCount() * exp * settingEXP * exppenalty;
					if (pc != null && !pc.isDead() && !pc.isGhost() && pc.getNetConnection() != null && !pc.noPlayerCK) {
						L1Item item = ItemTable.getInstance().getTemplate(L1Colosseum.Info.infinity_sign_id);
						pc.getInventory().storeItem(L1Colosseum.Info.infinity_sign_id, L1Colosseum.Info.infinity_sign_count);
						pc.sendPackets(new S_SystemMessage(item.getName() + "을(를) "+L1Colosseum.Info.infinity_sign_count+"개를 획득 하셨습니다"));
	...
	private void calcCombo(L1Character lastAttacker) {
		if ((lastAttacker instanceof L1PcInstance)) {
			...
			if (!pc.hasSkillEffect(L1SkillId.COMBO_BUFF)) {
				...
				pc.sendPackets(new S_PacketBox(204, 11));
			}
		}
	}
				...
				if (doll_item != null) {
					...
					for (int d = 0; d < Event_dolls.length; d++) {
						...
						if (doll_item.getItemId() == Integer.parseInt(Event_dolls[d])) {
							int rnd = _random.nextInt(1000000 - 1) + 1;
							if (rnd <= bInfo.calc_probability(pc, d)) {
								Broadcaster.broadcastPacket(this, new S_SkillSound(getId(), 230));
