	...
	public boolean noTarget() {
				...
				switch (npc.getNpcTemplate().get_npcId()) {
					...
					case 71062:
						if (getLocation().getTileLineDistance(_master.getLocation()) < 3) {
							pc = (L1PcInstance) _master;
							if ((pc.getX() >= 32448 && pc.getX() <= 32452) // 角落莫斯附近坐標
					...
					case 71075:
						...
						if (getLocation().getTileLineDistance(_master.getLocation()) < 3) {
							pc = (L1PcInstance) _master;
							if ((pc.getX() >= 32731 && pc.getX() <= 32735) // 蜥蜴人長老附近
					...
					case 70957:
						if (getLocation().getTileLineDistance(_master.getLocation()) < 3) {
							pc = (L1PcInstance) _master;
							if ((pc.getX() >= 32917 && pc.getX() <= 32921) // 巴修附近
									// 坐標
		...
		if (_master.isDead() || getLocation().getTileLineDistance(_master.getLocation()) > 10) {
			...
			if (getLocation().getTileLineDistance(_master.getLocation()) > 2) {
				setSleepTime(setDirectionMoveSpeed(moveDirection(_master.getMapId(), _master.getX(), _master.getY())));
			}
		}
		return false;
	}
	...
	public void spawn(int npcId, int X, int Y, int H, short Map) {
		...
		if (l1npc != null) {
			...
			try {
				...
				Constructor<?> _constructor = Class.forName((new StringBuilder()).append("l1j.server.server.model.Instance. ").append(implementationName).append("Instance").toString())
						.getConstructors()[0];
				...
				newnpc.startChat(L1NpcInstance.CHAT_TIMING_APPEARANCE); // 開始聊天
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
