...
import l1j.server.server.server.datatables.UBSpawnTable;
...
public class L1UltimateBattle {
	...
	private l1j.server.server.model.L1Location _location;
	...
	private void sendRoundMessage(int curRound) {
		switch (_ubId) {
			...
			case 5:
				if (curRound == 1) {
					sendMessage("競技場管理員: 第一軍投入!"); // 開始
				} else if (curRound == 2) {
					sendMessage("競技場管理員: 第二軍投入!"); // 開始
				} else if (curRound == 3) {
					sendMessage("競技場管理員: 第三軍投入!"); // 開始
				} else if (curRound == 4) {
					sendMessage("競技場管理員: 最終戰開始! 限制時間為5分鐘"); // 開始
				}
				break;
			case 4:
				if (curRound == 1) {
	...
	private void spawnSupplies(int curRound) {// 與地面物品掉落相關
		switch (_ubId) {
			case 1:
			case 2:
				if (curRound == 1) {
					...
					sendMessage("競技場管理員: 第一軍隊已經全部投入。");
					sendMessage("競技場管理員: 1分鐘後將開始第二軍隊的投入。");
				} else if (curRound == 2) {
					...
					spawnGroundItem(40079, 1, 10);
					// spawnGroundItem(3000156, 1, 5);
					sendMessage("競技場管理員: 第二軍隊已經全部投入。");
					sendMessage("競技場管理員: 2分鐘後將開始第三軍隊的投入。");
				} else if (curRound == 3) {
					...
					spawnGroundItem(40079, 1, 10);
					// spawnGroundItem(3000156, 1, 5);
					sendMessage("競技場管理員: 第三軍隊已經全部投入。");
					sendMessage("競技場管理員: 6分鐘後將開始最終戰。");
				}
				break;
			case 3:
				if (curRound == 1) {
			...
			case 4:
				if (curRound == 1) {
					...
					spawnGroundItem(40317, 1, 5);
					spawnGroundItem(40079, 1, 10);
					////// spawnGroundItem(3000156, 1, 1);
					// spawnGroundItem(3000156, 1, 15);
					sendMessage("競技場管理員: 第一軍隊已經全部投入。");
					sendMessage("競技場管理員: 2分鐘後將開始第二軍隊的投入。");
				} else if (curRound == 2) {
	...
	class UbThread implements Runnable {
		private void countDown() throws InterruptedException {
			...
			sendMessage("競技場管理員: 怪物即將出現，祝你好運。"); // 10秒前
			Thread.sleep(5000);
		...
		private void waitForNextRound(int curRound) throws InterruptedException {
			final int WAIT_TIME_TABLE[] = { 7, 12, 36, 18 };// 시간
	...
	public void start() {
		switch (getUbId()) {
			// case 1: L1World.getInstance().broadcastServerMessage("由於大量內容，奇岩村的無限對戰已經中止。"); break;
			case 2:
				L1World.getInstance().broadcastServerMessage("稍後，威爾頓村的無限對戰即將開始。");
				break;
			case 3:
				L1World.getInstance().broadcastServerMessage("稍後，古魯丁村的無限對戰即將開始。");
				break;
			case 4:
				L1World.getInstance().broadcastServerMessage("稍後，說話之島的無限對戰即將開始。");
				break;
			case 5:
				L1World.getInstance().broadcastServerMessage("稍後，銀騎士村的無限對戰即將開始。");
				break;
			default:
				break;
		}
