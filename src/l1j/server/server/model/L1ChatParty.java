...
public class L1ChatParty {
	...
	public void addMember(L1PcInstance pc) {
		...
		if (_membersList.size() == Config.ServerAdSetting.PARTYCHATCOUNT && !_leader.isGm() || _membersList.contains(pc)) {
			return;
		}
		...
		for (L1PcInstance member : _membersList) {
			member.sendPackets(message, false);
		}
	}
	...
	public boolean isVacancy() {
		return _membersList.size() < Config.ServerAdSetting.PARTYCHATCOUNT;
	}

	public int getVacancy() {
		return Config.ServerAdSetting.PARTYCHATCOUNT - _membersList.size();
	}
