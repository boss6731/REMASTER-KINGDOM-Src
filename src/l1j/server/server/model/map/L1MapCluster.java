...
public class L1MapCluster {
	...
	public int calculateClusterX(int x) {
		...
		if(clusterX >= _clusterX)clusterX = _clusterX - 1;
		return clusterX;
	}

	public int calculateClusterY(int y) {
		int clusterY = Math.max(y - _startY, 0) / CLUSTER_SIZE;
		if(clusterY >= _clusterY)clusterY = _clusterY - 1;
		return clusterY;
	}
	...
	public ArrayList<L1Object> getVisiblePoint(L1Location loc, int radius) {
		...
		for(int clusterX = startX; clusterX < startX + 3; ++clusterX){
			if(clusterX < 0 || clusterX >= _clusterX)continue;
			for(int clusterY = startY; clusterY < startY + 3; ++clusterY){
				if(clusterY < 0 || clusterY >= _clusterY)continue;
				for(L1Object target : _objects[clusterX][clusterY].values()){
					if(mapId != target.getMapId())continue;
					if(loc.getTileLineDistance(target.getLocation()) <= radius)result.add(target);
				}
			}
		}
		return result;
	...
	public ArrayList<L1Object> getVisibleObjects(L1Object object, int radius) {
		...
		for(int clusterX = startX; clusterX < startX + 3; ++clusterX){
			...
			for(int clusterY = startY; clusterY < startY + 3; ++clusterY){
				if(clusterY < 0 || clusterY >= _clusterY)continue;
				for(L1Object target : _objects[clusterX][clusterY].values()){
					if(target.equals(object))continue;
					if(mapId != target.getMapId())continue;
					if(radius == -1){
						if(pt.isInScreen(target.getLocation()))result.add(target);
					}else if(radius == 0){
						if(pt.isSamePoint(target.getLocation()))result.add(target);
					}else{
						if(pt.getTileLineDistance(target.getLocation()) <= radius)result.add(target);
					}
				}
			}
		}
	...
	public ArrayList<L1PcInstance> getVisiblePlayer(L1Object object, int radius) {
		...
		for(int clusterX = startX; clusterX < startX + 3; ++clusterX) {
			...
			for(int clusterY = startY; clusterY < startY + 3; ++clusterY) {
				if(clusterY < 0 || clusterY >= _clusterY)continue;
				for(L1Object target : _objects[clusterX][clusterY].values()) {
					if(!(target instanceof L1PcInstance))continue;
					L1PcInstance targetPc = (L1PcInstance) target;
					if(targetPc.equals(object))continue;
					if(mapId != targetPc.getMapId())continue;
					if(radius == -1){
						if(pt.isInScreen(target.getLocation()))result.add(targetPc);
					}else if(radius == 0){
						if(pt.isSamePoint(target.getLocation()))result.add(targetPc);
					}else{
						if(pt.getTileLineDistance(target.getLocation()) <= radius)result.add(targetPc);
					}
				}
			}
		}
		...
		for(int clusterX = startX; clusterX < startX + 3; ++clusterX){
			...
			for(int clusterY = startY; clusterY < startY + 3; ++clusterY){
				...
				for(L1Object targetObject : _objects[clusterX][clusterY].values()){
					if(!(targetObject instanceof L1PcInstance))continue;
					L1PcInstance element = (L1PcInstance) (targetObject);
					if(element.equals(object))continue;
					if(Config.ServerAdSetting.PC_RECOGNIZE_RANGE == -1){
						if(objectPt.isInScreen(element.getLocation())){
							if(!targetPt.isInScreen(element.getLocation()))result.add(element);
						}
					}else{
						if(objectPt.getTileLineDistance(element.getLocation()) <= Config.ServerAdSetting.PC_RECOGNIZE_RANGE){
							if(targetPt.getTileLineDistance(element.getLocation()) > Config.ServerAdSetting.PC_RECOGNIZE_RANGE)result.add(element);
						}
					}
				}
			}
