	...
	private void loadList(Map<Integer, ArrayList<L1PvpDmg>> list) {
		...
		try {
			...
			while (rs.next()) {
				...
				double dmg_rate = rs.getInt("傷害比率") * 0.01;
				double magic_dmg_rate = rs.getInt("魔法傷害比率") * 0.01;

				ed.setTargetType(rs.getInt("目標職業類型"));
				ed.setDmg(rs.getInt("額外傷害"));
				ed.setHit(rs.getInt("額外命中"));
				ed.setMagicDmg(rs.getInt("魔法額外傷害"));
				ed.setMagicHit(rs.getInt("魔法額外命中"));
