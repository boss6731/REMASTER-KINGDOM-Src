...
public class NpcTable {
	...
	private final HashMap<Integer, L1Npc> _npcs = new HashMap<Integer, L1Npc>(5120);
	...
	private void loadNpcBorn() {
		...
		try {
			...
			while (rs.next()) {
				...
				if (npc == null) {
					System.out.println(String.format("[npc_born 加載失敗] 無法找到NPC ID：%d", npcid));
					continue;
				}
				npc.createBorn(rs);
			}
...
//				npc.setInfoNumber(infoNumber);
				npc.set_class_id(infoNumber);
//				npc.set_class_id(rs.getInt("class_id"));
				// 應用NPC原地傳送
				...
				npc.setboss(rs.getString("boss").equalsIgnoreCase("true") ? true : false);
	...
	private int getNpcClassId(L1Npc npc) {
		if(npc.get_IsTU())                                    return 25; // 轉死靈怪物時，返回骷髏classid（由PSS識別）
//        else if(npc.getImpl().equals(DOPPEL_IMPL_STRING))    return 560; // 多重怪物時，返回多重classid（由PSS識別）
		return 0;
	}
	...
	public L1NpcInstance newNpcInstance(int id) throws ClassNotFoundException, IllegalAccessException, InstantiationException, InvocationTargetException, IllegalArgumentException {
		...
		if (npcTemp == null) {
			throw new IllegalArgumentException(String.format("NpcTemplate：%d 未找到", id));
		}
	...
	public static Map<String, Integer> buildFamily() {
		Map<String, Integer> result = new HashMap<String, Integer>(256);
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
