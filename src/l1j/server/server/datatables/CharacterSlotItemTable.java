...
public class CharacterSlotItemTable {
		...
		try {
			...
			if (pc.getSlotItems(2) != null) {
				for (int i = 0; i < pc.getSlotItems(2).size(); i++) {
					pstm = con.prepareStatement("INSERT INTO character_slot_items SET item_objid=?, char_id=? ,slot_number=?");
					pstm.setInt(1, pc.getSlotItems(2).get(i));
					pstm.setInt(2, pc.getId());
					pstm.setInt(3, 2);
					pstm.execute();
					pstm.close();
			...
			if (pc.getSlotItems(3) != null) {
				for (int i = 0; i < pc.getSlotItems(3).size(); i++) {
					pstm = con.prepareStatement("INSERT INTO character_slot_items SET item_objid=?, char_id=? ,slot_number=?");
					pstm.setInt(1, pc.getSlotItems(3).get(i));
					pstm.setInt(2, pc.getId());
					pstm.setInt(3, 3);
					pstm.execute();
					pstm.close();
	...
	public void selectCharSlotcolor(L1PcInstance pc) {
	...
	public void update_CharSlotcolor(L1PcInstance pc, int slotnum, L1InventorySlot slot){
		try{
			Updator.exec(String.format("insert into %s set char_id=?, slotNum=?, slot_color=?, slot_name=? on duplicate key update slot_color=?, slot_name=?", "character_slot_color"), new Handler() {
				@Override
				public void handle(PreparedStatement pstm) throws Exception {
					int idx = 0;
