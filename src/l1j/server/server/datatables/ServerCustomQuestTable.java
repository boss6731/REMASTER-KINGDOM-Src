...
public class ServerCustomQuestTable {
	...
	private List<CustomQuest> _list = new ArrayList<CustomQuest>();
	...
	public void reload() {
		List<CustomQuest> list = new ArrayList<CustomQuest>();
		tableLoad(list);
		_list = list;
	}
	...
	private void tableLoad(List<CustomQuest> list) {
		...
		try {
			...
			while (rs.next()) {
				...
				List<Integer> apply_map_ids = new ArrayList<Integer>();
				try {
	...
	public CustomQuest getCustomQuest(int questid) {
		return _list.stream().filter(datas -> datas.getQuestId() == questid).findFirst().orElse(null);
	}
	
	public CustomQuest getCustomQuestByMap(int mapid) {
		return _list.stream().filter(datas -> datas.getQuestApplyMapIds().contains((Object) mapid)).findFirst().orElse(null);
	}
	
	public List<CustomQuest> getCustomQuestListByMap(int mapid) {
		List<CustomQuest> new_list =new ArrayList<CustomQuest>();
		
		for(CustomQuest cq : _list) {
			if(cq.getQuestApplyMapIds().contains((Object) mapid)) {
				new_list.add(cq);
			}
		}
