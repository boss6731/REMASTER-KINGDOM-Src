...
public class NpcInfoTable {
	...
	public NpcInfoData getNpcInfo(int npcId){
		return _dataMap.containsKey(npcId) ? _dataMap.get(npcId) : null;
	}
	...
	private FastMap<Integer, NpcInfoData> load() {
		...
		try {
			...
			while (rs.next()) {
				...
				if(spawnMsg != null)spawnMsgArray = spawnMsg.split(StringUtil.CommaString);
				
				String[] dieMsgArray = null;
				String dieMsg = rs.getString("dieMsg");
				if(dieMsg != null)dieMsgArray = dieMsg.split(StringUtil.CommaString);
	...
	public class NpcInfoData {
		...
		public NpcInfoData(int _npcId, boolean _recall, int _spawnActionId,
				boolean _reward, String _rewardRange, int _rewardItemId,
				int _rewardItemCount, int _rewardEinhasad, int _rewardGfx,
				String _msgRange, String[] _spawnMsg, String[] _dieMsg, boolean _dieMsgPcList, boolean _autoLoot,
				int _transformChance, int _transformId, int _transformGfxId, ScriptInfo _scriptInfo) {
			super();
			this._npcId = _npcId;
