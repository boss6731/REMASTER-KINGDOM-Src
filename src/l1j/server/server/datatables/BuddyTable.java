...
public class BuddyTable {
	...
	private Map<Integer, ArrayList<L1Buddy>> _buddys = new HashMap<Integer, ArrayList<L1Buddy>>();
	...
	public void reload() {
		Map<Integer, ArrayList<L1Buddy>> buddys = new HashMap<Integer, ArrayList<L1Buddy>>();
		loadBuddy(buddys);
		_buddys = buddys;
	}

	public ArrayList<L1Buddy> getBuddyList(int objid) {
		return _buddys.get((Object) objid);
	}
	
	public L1Buddy getBuddy(int ojbid, String name) {
		ArrayList<L1Buddy> buddy_list = _buddys.get((Object) ojbid);
		if(buddy_list == null)
			return null;
		...
		for (int i = 0; i < buddy_list.size(); i++) {
			...
			if (buddy != null) {
				if (buddy.getCharName().equalsIgnoreCase(name)) {
					return buddy;
				}
			}
		}
	...
	private void loadBuddy(Map<Integer, ArrayList<L1Buddy>> buddys) {
		...
		try {
			...
			while (rs.next()) {
				...
				buddy_list = buddys.get(owner_obj_id);
				if (buddy_list == null) {
					buddy_list = new ArrayList<L1Buddy>();
					buddys.put(owner_obj_id, buddy_list);
				}
	...
	public void addAndSetBuddy(int charId, String name, String memo) {
		ArrayList<L1Buddy> buddy_list = _buddys.get((Object) charId);
		if (buddy_list == null) {
			buddy_list = new ArrayList<L1Buddy>();
			_buddys.put(charId, buddy_list);
		}
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_buddys (char_id, buddy_name, buddy_memo) VALUES (?, ?, ?) "
					+ "ON DUPLICATE KEY UPDATE char_id=?,buddy_name=?,buddy_memo=?");
