	...
	public static void save(L1PcInstance pc) {
		...
		try {
			...
			for (Iterator<Integer> iterator = keys.iterator(); iterator.hasNext();) {
				...
				cm = pc.getCustomQuestList().get(questid);
				String sql = "INSERT INTO character_custom_quest (char_id,char_name,quest_id,quest_state,success_count,quest_type)"
						+ "VALUES (?, ?, ?, ?, ?, ?) "
						+ "ON DUPLICATE KEY UPDATE char_id=?,char_name=?,quest_id=?,quest_state=?,success_count=?,quest_type=?";

				SQLUtil.execute(con, sql, new Object[] { 
						pc.getId(), pc.getName(), cm.getQuestId(), cm.getQuestState(), cm.getSuccessCount(), cm.getQuestType().toInt(),
						pc.getId(), pc.getName(), cm.getQuestId(), cm.getQuestState(), cm.getSuccessCount(), cm.getQuestType().toInt()
				});
			}
