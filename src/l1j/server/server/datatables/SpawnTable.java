	...
	public void reload(int mpaid) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (template1 == null) {
					// 警告：ID 為 npcTemplateId 的怪物數據在 NPC 表中缺失
					_log.warning("ID為：" + npcTemplateId + " 的怪物數據在 NPC 表中缺失");
					System.out.println("ID為：" + npcTemplateId + " 的怪物數據在 NPC 表中缺失");
					spawnDat = null;
				} else {
					...
					if (count > 1 && spawnDat.getLocX1() == 0) {
						// 對於多個固定生成點的情況，將其改為範圍生成，範圍為個體數量 * 6。
						// 但範圍不超過30。
