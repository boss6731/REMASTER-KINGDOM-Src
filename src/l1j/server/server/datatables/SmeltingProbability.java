...
import l1j.server.server.server.datatables.MJSmeltingProbabilityBox;
...
public class SmeltingProbability{
	...
	private HashMap<Integer, AlchemyProbStruct> load(){
		...
		Selector.exec("select * from tb_smelting_probability", new FullSelectorHandler(){
			@Override
	...
	public Integer select_id(int alchemy_id){
		return MJSmeltingProbabilityBox.getInstance().nextCurrentAlchemyId(alchemy_id);
