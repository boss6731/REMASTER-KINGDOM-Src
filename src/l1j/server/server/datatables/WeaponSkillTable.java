...
public class WeaponSkillTable {
	...
	private void fillWeaponSkillTable(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			weaponSkill = new L1WeaponSkill(weaponId, probability, fixDamage, randomDamage, area, skillId, skillTime,
					effectId, effectTarget, isArrowType, attr);
			_weaponIdIndex.put(weaponId, weaponSkill);
		}
		_log.config("武器技能列表 " + _weaponIdIndex.size() + " 條加載");
	}
