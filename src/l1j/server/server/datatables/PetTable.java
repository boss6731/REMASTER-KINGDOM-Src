...
public class PetTable {
	...
	public static boolean isNameExists(String nameCaseInsensitive) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			/*
			 * 查找相同的名字。MySQL默認是大小寫不敏感的，
			 * 所以通常不需要LOWER，但為了防止被改為二進制的情況。
			 */
			pstm = con
					.prepareStatement("SELECT item_obj_id FROM pets WHERE LOWER(name)=?");
			pstm.setString(1, nameLower);
			rs = pstm.executeQuery();
			if (!rs.next()) { // 沒有找到相同的名字
				result = false;
			}
			else if (l1j.server.server.datatables.PetTypeTable.getInstance().isNameDefault(nameLower)) { // 如果是默認名字，則認為不重複
				result = false;
			}
		} catch (Exception e) {
			e.printStackTrace();
