...
public class EnchantResultTable {
	...
	private HashMap<Integer, L1EnchantResult> _list = new HashMap<Integer, L1EnchantResult>();
	private EnchantResultTable() {
		Selector.exec("SELECT * FROM enchant_result", new FullSelectorHandler() {

			@Override
			public void result(ResultSet rs) throws Exception {
				while (rs.next()) {
					int itemId = rs.getInt("item_id");
					_list.put(itemId, new L1EnchantResult(rs));
				}
			}
		});
	}
	...
	public void send(L1PcInstance pc, SC_ENCHANT_RESULT.eResult result, int enchantGap, L1ItemInstance item) {
		L1EnchantResult er = _list.get(item.getItemId());
	...
	private class L1EnchantResult {
		...
		public L1EnchantResult(ResultSet rs) {
			try {
				isBm = rs.getBoolean("is_bm");
				effectId = Integer.parseInt(rs.getString("effect"));
				isHigh = rs.getBoolean("is_high");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
