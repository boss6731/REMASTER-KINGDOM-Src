...
public class CraftLimitTimeTrigger {
	...
	public static void do_load(){
		HashMap<Integer, CraftLimitTimeTrigger> triggers = new HashMap<Integer, CraftLimitTimeTrigger>();
		Selector.exec("select * from craft_limit_trigger", new FullSelectorHandler(){
			@Override
			public void result(ResultSet rs) throws Exception {
				while(rs.next()){
					CraftLimitTimeTrigger o = newInstance(rs);
					triggers.put(o.get_craft_id(), o);
				}
			}
		});
		m_triggers = triggers;
	...
	public static CraftLimitTimeTrigger get_trigger(int craft_id) {
		return m_triggers.get(craft_id);
	}
	...
	private int m_inherit_item_id;
	private long m_limit_millis;
	private CraftLimitTimeTrigger(){}
	...
	public long get_limit_millis(){
		return m_limit_millis;
	}
	public int get_inherit_item_id() {
		return m_inherit_item_id;
	}
}
