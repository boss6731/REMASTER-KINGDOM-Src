...
public class ClanStorageTable {
	...
	private Map<String, ArrayList<String>> _storage_list = new HashMap<String, ArrayList<String>>();
	...
	private void loadClanStorage() {
		...
		try {
			...
			while(rs.next()) {
				...
				_storage_list.put(clanname, listarray);
			}
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
	...
	public ArrayList<String> get_StorageList(L1PcInstance pc){
		...
		if (_storage_list.get(clanname) != null) {
			return _storage_list.get(clanname);
		}
		return null;
	}
	public boolean is_ClanStorageUse(L1PcInstance pc, String name) {
		...
		if (_storage_list.get(clanname) == null) {
			...
			if (_storage_list.get(clanname).contains(pc.getName())) {
				return true;
			} else {
				return false;
			}
	...
	public void add_Storage_list(L1PcInstance pc, String name) {
		if (_storage_list.get(pc.getClanname()) == null) {
			ArrayList<String> allowname = new ArrayList<String>();
			allowname.add(pc.getClan().getLeaderName());
			_storage_list.put(pc.getClanname(), allowname);
			insert_Storage_List(pc);
		}
		
		_storage_list.get(pc.getClanname()).add(name);
		update_Storage_List(pc);
	}
	
	public void delete_Storage_List(L1PcInstance pc, String name) {
		if (_storage_list.get(pc.getClanname()) != null) {
			if (is_ClanStorageUse(pc, name)) {
				while(_storage_list.get(pc.getClanname()).remove(String.valueOf(name))) {}
				for (String s: _storage_list.get(pc.getClanname())) {
	...
	public void update_Storage_List(L1PcInstance pc) {
		StringBuilder list = new StringBuilder();
		String comma = "";
		for (String g: _storage_list.get(pc.getClanname())) {
			list.append(comma);
			list.append(g);
			comma = ",";
		}
	...
	public void insert_Storage_List(L1PcInstance pc) {
