...
public class SkillsTable {
	...
	private void FillSkillsTable(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			l1skills.setDurationShowType(eDurationShowType.fromString(rs.getString("duration_show_type")));
			...
			l1skills.setDebuff(rs.getString("debuff").equalsIgnoreCase("true") ? true : false);
		...
		_log.config("技能 " + _skills.size() + "個 加載");
	}
