...
public class ArmorSetTable {
	...
	private List<L1ArmorSetImpl> m_armor_sets;
	...
	private void fillTable(ResultSet rs) throws SQLException {
		List<L1ArmorSetImpl> armor_sets = new ArrayList<L1ArmorSetImpl>();
		L1ArmorSets as  = null;
		while (rs.next()) {
			as = new L1ArmorSets();
			...
			L1ArmorSetImpl impl = L1ArmorSetImpl.newInstance(as);
			armor_sets.add(impl);
		}
		m_armor_sets = armor_sets;
	}

	public Collection<L1ArmorSetImpl> values(){
		return m_armor_sets;
	}
	
	public L1ArmorSetImpl find(int main_id) {
		for(L1ArmorSetImpl impl : values()) {
