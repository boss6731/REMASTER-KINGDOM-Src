...
public class CharacterFreeShieldTable {
	...
	public Map<Integer, L1FreeShield> _free_shield_info = new HashMap<Integer, L1FreeShield>();
	...
	private void loadCharacterFreeShield() {
		...
		try {
			...
			while(rs.next()) {
				...
				_free_shield_info.put(freeshield.get_Account_Id(), freeshield);
			}
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
	...
	private void InsertCharacterFreeShield(L1PcInstance pc) {
/*		L1FreeShield shield = new L1FreeShield();
		...
		_free_shield_info.put(pc.getAccount().getAccountId(), shield);*/
	...
	private void InsertCharacterFreeShieldLogin(int login, String name) {
		/*		L1FreeShield shield = new L1FreeShield();
				shield.set_Account_Id(pc.getAccount().getAccountId());
				shield.set_Account_name(pc.getAccountName());
				shield.set_Pc_Gaho(0);
	...
	public void updateFreeShileld(L1PcInstance pc) {
		...
		Updator.exec("update character_free_shield set pc_gaho=?, pc_gaho_use=?, free_gaho=?, free_gaho_use=? where account_id=?", new Handler() {
			...
			public void handle(PreparedStatement pstm) throws Exception {
				int idx = 0;
				pstm.setInt(++idx, getPcGaho(pc));
				pstm.setInt(++idx, getPcGahoUse(pc));
				pstm.setInt(++idx, getFreeGaho(pc));
				pstm.setInt(++idx, getFreeGahoUse(pc));
	...
	public void UpdateFreeShieldInfoLogin(int login, String name) {
		...
		shield.set_Event_Gaho(0);
		shield.set_Event_Gaho_use(0);
		_free_shield_info.put(login, shield);
	...
	public void getTam(L1PcInstance pc, int tamcount) {
		...
		if (tamcount >= 1 && tamcount <3) {
			if (_free_shield_info.get(pc.getAccount().getAccountId()).get_Free_Gaho_use() == 0) {
				_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho(1);
			} else {
				_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho(1-_free_shield_info.get(pc.getAccount().getAccountId()).get_Free_Gaho_use());
			}
		} else if (tamcount >= 3) {
			if (_free_shield_info.get(pc.getAccount().getAccountId()).get_Free_Gaho_use() == 0) {
				_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho(2);
			} else {
				_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho(2-_free_shield_info.get(pc.getAccount().getAccountId()).get_Free_Gaho_use());
			}
		}
	...
	public void getPCBuff(L1PcInstance pc) {
		...
		if (getPcGahoUse(pc) == 3) {
			_free_shield_info.get(pc.getAccount().getAccountId()).set_Pc_Gaho(0);
		} else {
			_free_shield_info.get(pc.getAccount().getAccountId()).set_Pc_Gaho(3 - getPcGahoUse(pc));
			_free_shield_info.get(pc.getAccount().getAccountId()).set_Pc_Gaho_use(getPcGahoUse(pc));
		}
		updateFreeShileld(pc);
	}
	...
	public int getPcGaho(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gaho = _free_shield_info.get(accountid).get_Pc_Gaho();
		return gaho;
	}
	public int getPcGahoUse(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gahouse = _free_shield_info.get(accountid).get_Pc_Gaho_use();
		return gahouse;
	}
	...
	public void usePcGaho(L1PcInstance pc) {
		...
		_free_shield_info.get(accountid).set_Pc_Gaho(gahocount);
		_free_shield_info.get(accountid).set_Pc_Gaho_use(gahousecount);
	...
	public int getFreeGaho(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gaho = _free_shield_info.get(accountid).get_Free_Gaho();
		return gaho;
	}
	public int getFreeGahoUse(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gahouse = _free_shield_info.get(accountid).get_Free_Gaho_use();
		return gahouse;
	}
	...
	public void useFreeGaho(L1PcInstance pc) {
		...
		_free_shield_info.get(accountid).set_Free_Gaho(gahocount);
		_free_shield_info.get(accountid).set_Free_Gaho_use(gahousecount);
	...
	public int getEventGaho(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gaho = _free_shield_info.get(accountid).get_Event_Gaho();
		return gaho;
	}
	public int getEventGahoUse(L1PcInstance pc) {
		int accountid = pc.getAccount().getAccountId();
		int gahouse = _free_shield_info.get(accountid).get_Event_Gaho_use();
		return gahouse;
	}
	...
	public void useEventGaho(L1PcInstance pc) {
		...
		_free_shield_info.get(accountid).set_Event_Gaho(gahocount);
		_free_shield_info.get(accountid).set_Event_Gaho_use(gahousecount);
	...
	public void resetGaho(L1PcInstance pc) {
		Updator.exec("update character_free_shield set pc_gaho=?, pc_gaho_use=?, free_gaho=?, free_gaho_use=? where account_id=?", new Handler() {
			@Override
			public void handle(PreparedStatement pstm) throws Exception {
				int pccount = 0;
				...
				if (_free_shield_info.get(pc.getAccount().getAccountId()) != null){
					_free_shield_info.get(pc.getAccount().getAccountId()).set_Pc_Gaho(pccount);
					_free_shield_info.get(pc.getAccount().getAccountId()).set_Pc_Gaho_use(0);
					_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho(freegahocount);
					_free_shield_info.get(pc.getAccount().getAccountId()).set_Free_Gaho_use(0);
				} else {
					L1FreeShield shield = new L1FreeShield();
					shield.set_Account_Id(pc.getAccount().getAccountId());
					shield.set_Account_name(pc.getAccountName());
	...
	public L1FreeShield getFreeShieldLogin(int login, String name) {
		if (_free_shield_info.get(login)== null) {
			UpdateFreeShieldInfoLogin(login, name);
		}
		if (_free_shield_info.containsKey(login)) {
			return _free_shield_info.get(login);
		}
...
//        System.out.println("連接確認 20");
		int accountid = pc.getAccount().getAccountId();
//        System.out.println("連接確認 21");
		if (_free_shield_info.get(accountid)== null) {
			UpdateFreeShieldInfo(pc);
		}
		if (_free_shield_info.containsKey(accountid)) {
			return _free_shield_info.get(accountid);
		}
		return null;
	}
