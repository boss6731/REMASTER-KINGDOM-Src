...
public class LightSpawnTable {
	...
	public void FillLightSpawnTable() {
		...
		try {
			...
			do {
				...
				L1Npc l1npc = l1j.server.server.datatables.NpcTable.getInstance().getTemplate(rs.getInt(2));
				if (l1npc != null) {
	...
	public void storeSpawn(L1PcInstance pc, L1Npc npc) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
			.prepareStatement("INSERT INTO spawnlist_light SET npcid=?,locx=?,locy=?,mapid=?");
