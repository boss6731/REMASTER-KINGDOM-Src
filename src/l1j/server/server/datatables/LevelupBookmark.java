...
public class LevelupBookmark {
	...
	private HashMap<Integer, ArrayList<L1BookMark>> do_load(){
		final HashMap<Integer, ArrayList<L1BookMark>> bookmarks = new HashMap<Integer, ArrayList<L1BookMark>>();
		Selector.exec("select * from levelup_addteleport order by id asc", new FullSelectorHandler() {
			@Override
			public void result(ResultSet rs) throws Exception {
				while(rs.next()) {
					int level = rs.getInt("level");
					ArrayList<L1BookMark> list = bookmarks.get(level);
					if(list == null) {
						list = new ArrayList<L1BookMark>();
						bookmarks.put(level, list);
					}		
	...
	public void on_level(final L1PcInstance pc) {
		ArrayList<L1BookMark> list = m_bookmarks.get(pc.getHighLevel());
		if(list == null) {
			return;
		}
