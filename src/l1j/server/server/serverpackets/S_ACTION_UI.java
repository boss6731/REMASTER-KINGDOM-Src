...
public class S_ACTION_UI extends l1j.server.server.serverpackets.ServerBasePacket {
	...
	private static final String TEST_IN_PROGRESS = "" + "0a e3 01 0a 55 "
...
//		         boolean bloodpledge = false;
		...
		extra += pc.getAinExpBonus() * 100;
			...
			/** 類型 **/
		    writeC(0x38);
			// if (blessOfAin < 1/* && pc.PC room_buff */){
//              writeBit(0x02); // 艾因哈薩德的庇護: pc
//		     } else 
		    if (blessOfAin < 1 && pc.hasSkillEffect(L1SkillId.EINHASAD_GRACE)){
		     writeBit(0x03); // 艾因哈薩德的庇護
		    } else {
		     writeBit(0x01);
		    }
		if ((blessOfAin > 0 && blessOfAin < 80000000) && !pc._dragonbless){
		         pc.sendPackets(new l1j.server.server.serverpackets.S_NewSkillIcon(L1SkillId.DRAGON_BLESS, true, -1).toString());
