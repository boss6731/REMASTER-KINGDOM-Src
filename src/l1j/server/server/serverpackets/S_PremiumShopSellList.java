...
public class S_PremiumShopSellList extends ServerBasePacket {
	...
	public S_PremiumShopSellList(int objId, L1PcInstance pc) {
		...
		try {
			...
			System.out.println("點擊商店（NPC）時：NPCID（無物品/異常）： " + npcId);
		}
		...
		for (int i = 0; i < shopItems.size(); i++) {
			...
			try {
				...
				System.out.println("點擊商店 (NPC) 時：NPCID（物品不存在/異常）：" + npcId);
			}
			writeD(price);
				 ...
				 if (status != null) {
					 ...
					 if (item.getBless() >= 128)
		               ...
		               for (byte b : status) {
					 	...
					 	System.out.println("項目錯誤信息通知 : " + dummy.getItemId());
		            }
			}
		}
		/** 商店探險家、貝里等圖像封包（桌號） **/
		...
		/*} else if (npcId == 7310119 || npcId == 7310120 || npcId == 7310126 || npcId == 7310124 || npcId == 520) {
			writeH(0x4215);*/
			/*} else if (npcId == 7310128) { // --夜明珠
			writeH(0x41D8);*/
			/*} else if (npcId == 91245) {// 潘多拉的證書
			writeH(0x3F23);*/
			/*} else if (npcId == 70017 || npcId == 8502040 || npcId == 8502063) {// 奧林的石榴石
			writeH(0x4115);*/
			/*} else if (npcId == 7320250) {// 哈丁的石榴石
			writeH(0x45AF);*/
			/*} else if (npcId == 521) {// 魯道夫的鈴鐺
			writeH(17619);*/
			/*
			 * } else if (npcId == 7320055) {// N幣
			 * writeH(-2);
			 */
			/*} else if (npcId == 8500140) {// 根特的印章
			writeH(17368);*/
