...
public class S_TamWindow extends ServerBasePacket {
	...
	public static final int ACCOUNT_TAM = 0xCD; // 帳戶探查資訊窗口
		...
		try {
			...
			while (rs.next()) {
				...
				writeC(_Name.getBytes().length + 14 + objidcount/* addlen */
						+ tamcount); // 封包總長度
				writeC(0x08); //
				writeC(0x00); // 變化（伺服器更改）
				writeC(0x10); //
				byteWrite(_objid); // 探查時增加。
				//write4bit(_objid); // 測試
				...
				if (time == 0) {
					...
					byteWrite(time / 1000); // 探查時增加。
					//write4bit(time/1000);
				}
				writeC(0x20);
				writeC(tamwaitcount(_objid));
				writeC(0x2a);//
				writeC(_Name.getBytes().length); // 名字長度
				writeByte(_Name.getBytes()); // 名字後面無 0 的名字。
				writeC(0x30); //
				writeC(_level); // 等級
				writeC(0x38); //
				writeC(_class); // 職業編號
				writeC(0x40); //
				writeC(_sex); // _sex); // 男性女性？
			}
			writeC(0x10);
			writeC(0x03);
			writeC(0x18);
	...
	private int tamwaitcount(int obj) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM tam WHERE objid = ?"); // 角色
																				// 從表格中
																				// 只選擇
																				// 王族
			pstm.setInt(1, obj);
