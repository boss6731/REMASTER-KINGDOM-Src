...
public class S_PetPack extends ServerBasePacket {
	...
	private void buildPacket(L1PetInstance pet, L1PcInstance pc) {
		...
		writeH(pet.getY());
		writeD(pet.getId());
		writeH(pet.getCurrentSpriteId()); // 在 List.spr 中的 SpriteID
		writeC(pet.getStatus()); // 在 List.spr 中的模式
		writeC(pet.getHeading());
		writeC(pet.getLight().getChaLightSize()); // (亮度) - 0~15
		writeC(pet.getMoveSpeed()); // 速度 - 0:正常, 1:快,
		// 2:慢
		writeD(pet.get_exp());
