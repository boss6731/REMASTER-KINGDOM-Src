...
public class S_TestPacket extends ServerBasePacket {
	...
	public static final int SERVER_ANNOUNCEMENT = 103;
	...
	public S_TestPacket(int type) {
		...
		case b:
		  writeC(0x00);
		  writeC(0x08);
		  String 訊息 = "00 08 " + "b8 62 " // b4 62 = 中間的君主臉 , 01 = 沒有圖像
		  + "10 " // ?
		  + "c6 49 " // 訊息代碼
		  + "1a 03 " // ?
		  + "00 ff ff " // 字體顏色 (參照html顏色表)
		  + "20 14";
		  StringTokenizer st = new StringTokenizer(訊息.toString());
		  while (st.hasMoreTokens()) {
		    writeC(Integer.parseInt(st.nextToken(), 16));
		  }
		  writeH(0x00);
