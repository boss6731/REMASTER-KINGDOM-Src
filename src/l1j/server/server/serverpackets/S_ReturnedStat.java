...
public class S_ReturnedStat extends ServerBasePacket {
	...
	public static final int Unknown_LOGIN2 = 0x44;//문제
	...
	public static final int POWERBOOK_SEARCH = 0x019;
...
//	public static final int NEW_ADDSTAT = 76;
	...
	public S_ReturnedStat(l1j.server.server.server.model.Instance.L1PcInstance pc, int start) {
...
//			break;
	...
	public static final int OPEN_SLOT_SHOULD = 0x40;         // 肩甲
	public static final int OPEN_SLOT_BADGE = 0x80;          // 徽章
	public static final int OPEN_SLOT_LRING95 = 0x100;       // 測試
	public static final int OPEN_SLOT_RRING100 = 0x200;      // 測試
	public static final int OPEN_SLOT_LEARRING101 = 0x400;   // 測試
	public static final int OPEN_SLOT_REARRING103 = 0x800;   // 測試
		...
		case RING_RUNE_SLOT:
			...
			if (subType == SUBTYPE_RING) { // 戒指槽位
				if (value == 2)
					...
					value = 3;
				writeC(value);
			} else if (subType == SUBTYPE_RUNE) { // 符文槽位
				writeC(1); // 1~3
			}
			writeD(0);
			writeD(0);
...
//		writeH(0x00);
	...
	/*public S_ReturnedStat(L1PcInstance pc, int type, String action) {
	  ...
	  public S_ReturnedStat(L1PcInstance pc, int type, int c) {// 精煉藥劑附加效果
	        writeC(Opcodes.S_VOICE_CHAT);
	        writeC(type);
			switch (type) {
			case END:
...
//			break;
		case LOGIN:
			/*
			 * pc.getAbility返回的最小屬性值陣列順序 0:力量/1:敏捷/2:體質/3:智慧/4:魅力/5:智力
			 */
			/*writeC(0x07);
			writeC(0x00);
			writeC(0x02);
	...
	public S_ReturnedStat(L1PcInstance pc, int c, boolean f) {// 精煉藥劑附加效果
		writeC(Opcodes.S_VOICE_CHAT);
		writeC(END);
		writeC(c);
	}
