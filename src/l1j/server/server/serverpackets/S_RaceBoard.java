	...
	private void buildPacket(int number, boolean is_viewed_gm) {
		...
		if(is_viewed_gm){
			...
			for( int i = 0; i < 5; ++i ) {
				...
				writeS(String.format("%s/%s/%.2f%%/%.2f배/%d", BugRaceController.getInstance()._littleBugBear[i].getName(), BugRaceController.getInstance()._bugCondition[i], rate, ration, move_speed));
				writeS("");
				writeS("");
			}
		}else{
			for( int i = 0; i < 5; ++i ) {
				writeS(BugRaceController.getInstance()._littleBugBear[i].getName()); // 小熊貝爾名稱
				writeS(BugRaceController.getInstance()._bugCondition[i]); // 狀態
				writeS(Double.toString(BugRaceController.getInstance()._winRate[i]) + "%"); // 勝率實際
// 				writeS(Double.toString(BugRaceController.getInstance()._winViewRate[i]) + "%"); // 勝率統合
			}
		}
	}
