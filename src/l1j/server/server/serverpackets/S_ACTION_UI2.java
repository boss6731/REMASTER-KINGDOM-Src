...
public class S_ACTION_UI2 extends l1j.server.server.serverpackets.ServerBasePacket {
		...
		case CLAN_JOIN_WAIT:
			...
			if (size > 0) {
				...
				for (L1PcInstance pc : list) {
					...
					// byte[] memo = pc.getTitle().getBytes();//臨時稱呼
					// writeC(memo.length);
					// writeByte(memo);
					writeC(0x30);
					writeC(L1World.getInstance().getPlayer(pc.getName()) != null ? 1: 0);// 連線中
					writeC(0x38);
					writeC(pc.getType());// 類別
					i++;
					if (i == 2)
						break;
				}
	...
	private static final String AinhasadBlessing1 = "00 08 01 10 82 22 18";
	private static final String AinhasadBlessing2 = "20 0e 28 aa 4b 30 80 44 38 01 40 c9 36 48 "
			+ "00 50 00 58 01 60 00 68 00 70 00 78 2a 80 01 00 00 00";

	public S_ACTION_UI2(String code, long time) {
		writeC(Opcodes.S_EXTENDED_PROTOBUF);
		writeC(ICON_BUFF);

		String buffPacket = "";
		if (code.equals("艾因哈薩德的庇佑")) {
			buffPacket = AinhasadBlessing1;
		}

		StringTokenizer st = new StringTokenizer(buffPacket.toString());
		...
		if (code.equals("艾因哈薩德的庇佑")) {
			buffPacket = AinhasadBlessing2;
		}
		st = new StringTokenizer(buffPacket.toString());
		while (st.hasMoreTokens()) {
			writeC(Integer.parseInt(st.nextToken(), 16));
		}
	}
