	...
	private void buildPacket(int objectId, String house_number) {
		...
		try {
			...
			while (rs.next()) {
				...
				writeD(objectId);
				writeS("agsel");
				writeS(house_number); // 領地的編號
				writeH(9); // 以下字符行的數量
				writeS(rs.getString(2)); // 領地的名稱
				writeS(rs.getString(6)); // 領地的位置
				writeS(String.valueOf(rs.getString(3))); // 領地的面積
				writeS(rs.getString(7)); // 前任擁有者
				writeS(rs.getString(9)); // 當前的競標者
				writeS(String.valueOf(rs.getInt(5))); // 當前的競標價格
				cal = timestampToCalendar((Timestamp) rs.getObject(4));
				int month = cal.get(Calendar.MONTH) + 1;
				int day = cal.get(Calendar.DATE);
				int hour = cal.get(Calendar.HOUR_OF_DAY);
				writeS(String.valueOf(month)); // 截止月份
				writeS(String.valueOf(day)); // 截止日期
				writeS(String.valueOf(hour)); // 截止時間
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
