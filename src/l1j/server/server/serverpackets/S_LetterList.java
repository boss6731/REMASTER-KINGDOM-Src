...
public class S_LetterList extends l1j.server.server.serverpackets.ServerBasePacket {
    ...
    private void buildPacket(L1PcInstance pc, int type, int count) { // 問題出在這部分
        Connection con = null;
        ...
        try {
            ...
            while (rs.next()) {
                writeD(rs.getInt(1));
                writeC(rs.getInt(9));
                writeD((rs.getTimestamp(5).getTime() / 1000L));
                writeC(0);
                writeS(rs.getString(3));
                writeSS(rs.getString(7));
            }
        } catch (Exception e) {
            e.printStackTrace();// 移除此日誌就不會出現
        } finally {
            SQLUtil.close(rs);
            SQLUtil.close(rs1);
            SQLUtil.close(pstm);
    ...
    public S_LetterList(int type, boolean value) {
        writeC(Opcodes. S_MAIL_INFO);
        writeC(type); 
        writeC(value ? 1 : 0);
    }
