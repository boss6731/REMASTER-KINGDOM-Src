...
public class S_AddItem extends l1j.server.server.serverpackets.ServerBasePacket {
	...
	public S_AddItem(L1PcInstance pc, L1ItemInstance item) {
		...
		if (item.getItem().isCantDelete()) bit += 4; // 刪除不可能
		if (item.getItem().get_safeenchant() < 0) bit += 8; // 強化不可能
//  	if (item.getItem().getWareHouse() > 0 && !item.getItem().getItem().isTradable()) bit += 16; // 倉庫保管可能
		...
		writeD(0);
		writeC(item.getBless() >= 128 ? 3 : item.getItem().isTradable() ? 7 : 2);// 製作1102新增
		writeD(0);
...
//  	if (item.getItem().getWareHouse() > 0 && !item.getItem().getItem().isTradable()) bit += 16; // 倉庫保管可能
		...
		writeD(item.getId());
		if(item.getItem().getUseType() == 73){// 放置娃娃時的背包效果封包
			writeD(0x20);
		...
		writeD(0);
		writeC(item.getBless() >= 128 ? 3 : item.getItem().isTradable() ? 7 : 2);//新增製作1102
		writeD(0);
		if(item.getItem().getType2() == 1)
			writeC(item.getAttrEnchantBit(item.getAttrEnchantLevel()));
		else
