...
public class PacketHandler {
	...
	public void handlePacket(byte abyte0[], L1PcInstance object) throws Exception {
		int i = abyte0[0] & 0xff;
			...
			//	System.out.println("[C opocde] = " + i + "[Length] = " + abyte0.length);//從主伺服器提取操作碼
...
//				System.out.println("封包檢查1");
			...
			case C_DELETE_CHARACTER:
				if(_client.getAccount().getCPW() != null){
					...
					_client.sendPacket(new S_CharPass(S_CharPass._passwordInputWindow));
					return;
				}
			...
			case C_FAR_ATTACK: 
				...
				break;
				// 當角色的快捷鍵或列表狀態在遊戲中變更時，
				// 將會附帶快捷鍵或列表狀態從客戶端傳送過來。
				// 傳送的時機是在客戶端結束時。
			case C_BAN_MEMBER: 
				C_BanClan.createC_BanClan(abyte0, _client);
				break;
			case C_PLATE: 
