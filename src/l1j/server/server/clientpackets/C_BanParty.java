...
public class C_BanParty extends l1j.server.server.clientpackets.ClientBasePacket {
	...
	public C_BanParty(byte decrypt[], GameClient client) throws Exception {
		...
		if (!player.getParty().isLeader(player)) {
			// 不是隊伍領導的情況下
			player.sendPackets(String.valueOf(new S_ServerMessage(427))); // 只有隊伍領導才能進行驅逐。
			return;
		}
		...
		// 未被發現
		player.sendPackets(String.valueOf(new S_ServerMessage(426, s))); // %0 不是隊伍成員。
	}
