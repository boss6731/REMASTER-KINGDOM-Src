...
// 			System.out.println("載入時DB中的HP值: "+currentHp);
			pc.setCurrentHp(currentHp);
// 			System.out.println("從DB載入後的HP: "+pc.getCurrentHp());
			pc.setDead(false);
			pc.setStatus(0);
			...
			pc.setAinState(rs.getInt("AinState")); // 生存冷卻時間狀態
			pc.setSurvivalGauge(rs.getLong("SurvivalGauge")); // 生存計量
			pc.setMark_count(rs.getInt("Mark_Count"));
			pc.setAge(rs.getInt("Age")); // 設定年齡
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO characters SET AccessLevel=?,account_name=?,objid=?,char_name=?"
					...
					+ ",Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,notes=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,GloryEarthAttr=?"
					+ ",PKcount=?,ExpRes=?,PartnerID=?,OnlineStatus=?,HomeTownID=?,Contribution=?"
					...
					+ ",TamEndTime=?,SpecialSize=?,FishingShopBuyTime_1=?,slot_number=?,elf_attrinit_count=?,clan_contribution=?, class_rank_bless_time=?");
			...
			pstm.setInt(++i, pc.getAinState()); // 生存冷卻時間
			pstm.setLong(++i, pc.getSurvivalGauge()); // 生存計量表
			pstm.setInt(++i, pc.getBirthDay());
			pstm.setInt(++i, pc.getMark_count());
			pstm.setInt(++i, pc.getAge()); // 設定年齡
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE characters SET AccessLevel=?,level=?,HighLevel=?,Exp=?,MaxHp=?"
					...
					+ ",notes=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,GloryEarthAttr=?,PKcount=?,ExpRes=?,PartnerID=?"
					...
					+ ",Mark_Count=?,Age=?,AddDamage=?,AddDamageRate=?,AddReduction=?,AddReductionRate=?"
					+ ", TamEndTime=?,SpecialSize=?,FishingShopBuyTime_1=?,slot_number=?,elf_attrinit_count=?, last_topBless=?, clan_contribution=?"
					+ ", class_rank_bless_time=? WHERE objid=?");
			pstm.setShort(++i, pc.getAccessLevel());
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
			pstm.setLong(++i, pc.get_exp());
			...
			System.out.println(pc.getName() + " 角色異常處理確認請求（請轉交給負責人）");
			e.printStackTrace();
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
