package l1j.server.server.server.storage.mysql;
...
public class MySqlCharactersItemStorage extends CharactersItemStorage {
	...
	public ArrayList<L1ItemInstance> loadItems(int objId) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				item.setSupportItem(rs.getInt("is_support_item") != 0 ? true : false);
	...
	public void storeItem(int objId, L1ItemInstance item) throws Exception {
		...
		try {
			int idx = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_items SET id = ?, item_id = ?, char_id = ?, item_name = ?, "
					+ "count = ?, is_equipped = 0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, "
					+ "last_used = ?, bless = ?, attr_enchantlvl = ?, end_time =?,buy_time=?, package = ?, bless_level = ?, item_level = ?, "
					+ "Hotel_Town = ?, is_support_item=?, Carving = ?, doll_bonus_level = ?, doll_bonus_value = ?,  cant_unseal=? "
					+ "on duplicate key update item_id = ?, char_id = ?, item_name = ?, count = ?, is_equipped = 0, "
					+ "enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, "
					+ "bless = ?, attr_enchantlvl = ?, end_time =?,buy_time=?, package = ?, bless_level = ?, item_level = ?, "
					+ "Hotel_Town=?, is_support_item=?, Carving = ?, doll_bonus_level = ?, doll_bonus_value = ?, bless_type=?, bless_type_value=?, "
					+ "smelting_slot_value=?, smelting_itemid_1=?, smelting_itemid_2=?, smelting_kind_1=?, smelting_kind_2=?, halpas_time=?, cant_unseal=?");
			...
			e.printStackTrace();
			System.out.println("資料庫物品儲存錯誤 物品名稱: " + item.getName() + " 擁有者: " + item.getItemOwner().getName());
			throw e;
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
	...
	public void updateItemAll(L1ItemInstance item) throws Exception {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE character_items SET " + "item_id = ?" + ",count = ?" + ",durability = ?" + ",charge_count = ?" + ",remaining_time = ?" + ",enchantlvl = ?"
					+ ",is_equipped = ?" + ",is_id = ?" + ",last_used = ?" + ",bless = ?" + ",attr_enchantlvl = ?" + ",special_enchant = ?" + ",end_time = ?" + ",bless_level = ?" + ",item_level = ?"
					+ ",Hotel_Town = ?, is_support_item = ?," + "Carving = ?, doll_bonus_level = ?, doll_bonus_value=?,bless_type = ?, bless_type_value = ?"
					+ ",smelting_slot_value=? ,smelting_itemid_1=? ,smelting_itemid_2=? ,smelting_kind_1=? ,smelting_kind_2=?, halpas_time=?, cant_unseal=? WHERE id=?");
