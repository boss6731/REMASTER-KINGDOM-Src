	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		try {
			...
			while (st.hasMoreTokens()) {
				...
				if (itCount == 0
						&& (tempVar.equals("鎧甲") || tempVar.equals("武器")
						|| tempVar.equals("etc")
						|| tempVar.equals("變身") || tempVar
						.equals("NPC"))) {
					simpleS = false;
					type = tempVar;
				} else {
			...
			pc.sendPackets(String.valueOf(new S_SystemMessage(".find [鎧甲,武器,etc,變身,NPC] 請輸入")));
		}
	}
	...
	private void find_object(L1PcInstance pc, String type, String real_name_id_view) {
		...
		try {
			...
			if (!error) {
				...
				pc.sendPackets(String.valueOf(new S_SystemMessage("正在搜索的 '" + real_name_id_view.replace("%", " ") + "' 是 " + type + " 名稱...")));
				while (rs.next()) {
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					if (type.equals("鎧甲") || type.equals("武器")) {
						boolean etc = type.equals("etc");
						{
							bless = rs.getInt(3);
							if (bless == 1) {
								blessed = "";
							...
							pc.sendPackets(String.valueOf(new S_SystemMessage(blessed + "編號: " + str1 + ", " + str2)));
						} else{
							pc.sendPackets(String.valueOf(new S_SystemMessage("編號: " + str1 + ", " + str2)));
						}
						count++;
					}
					pc.sendPackets(String.valueOf(new S_SystemMessage("找到 " + count + " 行 " + type + " 類型的項目。")));
				}
			} catch(Exception e){
				e.printStackTrace();
			} finally{
		...
		private void find_object (L1PcInstance pc, String real_name_id_view){
			...
			try {
				...
				pc.sendPackets(String.valueOf(new S_SystemMessage("在行中搜索的對象名稱: '" + real_name_id_view.replace("%", " ") + "'")));

				statement = con.prepareStatement("SELECT item_id,real_name_id_view,bless FROM armor WHERE real_name_id_view Like '%" + real_name_id_view + "%'");
				rs = statement.executeQuery();
				while (rs.next()) {
					if (count1 == 0) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("鎧甲:")));
					}
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					bless = rs.getInt(3);
			...
			try {
				...
				while (rs.next()) {
					if (count2 == 0) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("武器:")));
					}
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					bless = rs.getInt(3);
			...
			try {
				...
				while (rs.next()) {
					if (count3 == 0) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("etc:")));
					}
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					bless = rs.getInt(3);
			...
			try {
				...
				while (rs.next()) {
					if (count4 == 0) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("變身:")));
					}
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					pc.sendPackets(String.valueOf(new S_SystemMessage("編號: " + str1 + ", " + str2)));
			...
			try {
				...
				while (rs.next()) {
					if (count5 == 0) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("NPC:")));
					}
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					pc.sendPackets(String.valueOf(new S_SystemMessage("編號: " + str1 + ", " + str2)));
			...
			try {
				pc.sendPackets(String.valueOf(new S_SystemMessage("搜尋結果:")));
				String found = "";
				if (count1 > 0) {
					found += "盔甲: " + count1 + "、";
				}
				...
				pc.sendPackets(String.valueOf(new S_SystemMessage(found)));
			} catch (Exception e) {
			}
		}
	}
