...
public class L1ChatNG implements l1j.server.server.command.executor.L1CommandExecutor {
...
//			String reason = st.nextToken();
			...
			if (tg != null) {
				...
				pc.sendPackets(String.valueOf(new S_SystemMessage(name + " 聊天禁止 : " + String.valueOf(time) + "分 適用完成。")));
// 				tg.sendPackets(new S_ServerMessage(286, String.valueOf(time))); // 因為不適合遊戲的行為，因此將在未來%0分鐘內禁止聊天。
			} else {
				pc.sendPackets(String.valueOf(new S_SystemMessage("該角色未登入。")));
			}
		} catch (Exception e) {
			pc.sendPackets(String.valueOf(new S_SystemMessage(cmdName + " [角色名] [分鐘] 輸入。")));
		}
	}
