...
public class L1LevelPresent implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String arg) {
		...
		try {
			...
			if (temp == null) {
				pc.sendPackets(String.valueOf(new S_SystemMessage("不存在的物品ID。")));
				return;
			}

			present(minlvl, maxlvl, itemid, enchant, count);
			pc.sendPackets(String.valueOf(new S_SystemMessage(temp.getName() + "已贈送" + count + "個。(Lv" + minlvl + "~" + maxlvl + ")")));
		} catch (Exception e) {
			pc.sendPackets(String.valueOf(new S_SystemMessage("請輸入指令格式：.等級禮物 [最低等級] [最高等級] [物品ID] [強化] [數量]。")));
		}
	}
	...
	private static void present(ArrayList<String> accountList, int itemid, int enchant, int count) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception(".present 處理過程中發生錯誤。");
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
