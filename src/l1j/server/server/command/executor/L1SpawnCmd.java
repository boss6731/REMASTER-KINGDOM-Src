...
public class L1SpawnCmd implements L1CommandExecutor {
	...
	private void sendErrorMessage(L1PcInstance pc, String cmdName) {
		...
		pc.sendPackets(String.valueOf(new S_SystemMessage(errorMsg)));
	}
		...
		GeneralThreadPool.getInstance().execute(new Runnable() {
			...
			public void run() {
				try {
					...
					if (npc == null) {
						pc.sendPackets(String.valueOf(new S_SystemMessage("未發現該 NPC。")));
						return;
					}
					...
					String msg = String.format("%s(%d) (%d) 已召喚。 (範圍:%d)", npc.get_name(), npcid, count, randomrange);
					pc.sendPackets(msg);
					...
					e.printStackTrace();
					pc.sendPackets(String.valueOf(new S_SystemMessage(cmdName + " 內部錯誤。")));
				}
			}
		});
	}
