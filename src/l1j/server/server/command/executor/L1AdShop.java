...
public class L1AdShop implements l1j.server.server.command.executor.L1CommandExecutor {
	...
	public void execute(L1Object pc, String cmdName, String arg) {
		try {
			...
			if (CharacterTable.getInstance().isContainNameList(name) || L1World.getInstance().getPlayer(name) != null) {
				pc.sendPackets(String.valueOf(new S_SystemMessage("此角色名稱已存在")));
				return;
			}
			...
			pc.sendPackets(String.valueOf(new S_SystemMessage((new StringBuilder()).append("請輸入：.開始交易 [角色名稱]").toString())));
		}
	}
