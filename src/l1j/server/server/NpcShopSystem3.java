			...
			try {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					Broadcaster.broadcastPacket(npc,
							new S_DoActionShop(npc.getId(), ActionCodes.ACTION_Shop, shop.getShopName()));
					// Broadcaster.broadcastPacket(npc, new S_DoActionShop(npc.getId(),
					// ActionCodes.ACTION_Shop, shop.getShopName().getBytes("MS949")));
					_shops.add(npc);
					Thread.sleep(10);
				}
				// list.clear();
				...
				GeneralThreadPool.getInstance().scheduleAtFixedRate(NpcShopSystem3.getInstance(), 0, 3600000);
			}
            return new l1j.server.server.model.Instance.L1PcInstance[0];
        }
	}

	/** 2016.11.24 MJ 應用中心價格 **/
