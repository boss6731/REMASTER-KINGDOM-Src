...
public class DeadLockDetector implements Runnable {
		...
		try {
			...
			if (threadIds != null) {
				if (pc != null && pc.getNetConnection() != null)
					pc.sendPackets(new S_SystemMessage("偵測到死鎖！ - 參考伺服器 Cmd"));
				else
				...
				if (pc != null && pc.getNetConnection() != null)
					pc.sendPackets(new S_SystemMessage("沒有死鎖"));
				else
	...
	private void printMonitorInfo(ThreadInfo threadInfo,
		...
		sb.append(INDENT + "鎖定的監視器: " + monitorInfos.length + "個\n");
	...
	private void printLockInfo(LockInfo[] lockInfos) {
		sb.append(INDENT + "鎖定的同步對象: " + lockInfos.length + "個\n");
