...
public class BinaryOutputStream extends OutputStream {
	...
	public int writeLenght(int obj) {
		...
		if(obj < 0){
			BigInteger b = new BigInteger("18446744073709551615");
			while(BigInteger.valueOf(obj).and(b).shiftRight((length + 1) * 7).longValue() > 0)length++;
			length++;
		}else{
			if(obj <= 127)length = 1;
			...
			else if((long) obj <= 34359738367L)length = 5;
		}
		return length;
	}
