...
public class FaceToFace {
	...
	public static L1PcInstance faceToFace(L1PcInstance pc) {
		...
		if (players.size() == 0) { // 1格範圍內沒有玩家的情況下
			pc.sendPackets(new S_ServerMessage(93)); // 1 那裡沒有人。
			return null;
		}
		for (L1PcInstance target : players) {
			...
			if (pcHeading == 0 && pcX == targetX && pcY == (targetY + 1)) {
				if (targetHeading == 4) {
					...
					pc.sendPackets(new S_ServerMessage(91, target.getName())); // 1%0 沒有在看你。
					return null;
				}
