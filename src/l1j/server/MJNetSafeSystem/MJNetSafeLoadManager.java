...
//                L1World.getInstance().broadcastPacketToAll(new ServerBasePacket[] {new S_ServerMessage(3362), new S_SystemMessage("伺服器版本已更新。請重新啟動啟動器並務必重新下載正式伺服器補丁。")}, true);
					break;
				default:
					throw new Exception("");
			}
...
//			NS_CLIENT_VERSION = Long.parseLong(settings.getProperty(column, "2209131701"))
//            System.out.println("客戶端版本: "+Integer.toUnsignedString(NS_CLIENT_VERSION));
//			NS_CLIENT_VERSION = Long.parseLong(settings.getProperty(column, "2010271702"));
			...
			//	long value = Long.parseLong(settings.getProperty(column, "0x7a834df5").replace("0x", ""), 16);
			//	long value = Long.parseLong(settings.getProperty(column, "0x5455b0fd").replace("0x", ""), 16);
			//	long value = Long.parseLong(settings.getProperty(column, "0x1a4e04da").replace("0x", ""), 16); //20220810
//			long value = Long.parseLong(settings.getProperty(column, "0x142c974f").replace("0x", ""), 16); //20220817
