...
public abstract class Distributor {
	protected void toInvalidOp(GameClient clnt, int op, int length, String state, boolean isClose) throws Exception{
		...
		if(isClose){
			System.out.println(String.format("無效的操作碼(1). [%s] 操作碼 : %d, 長度 : %d -> 踢出。(%s)", clnt.getIp(), op, length, state));
			clnt.kick();
		}else {
			System.out.println(String.format("無效的操作碼(2). [%s] 操作碼 : %d, 長度 : %d -> (%s)", clnt.getIp(), op, length, state));
		}
	}
	...
	protected boolean isBanned(GameClient clnt){
		...
		if (IpTable.getInstance().isBannedIp(ip)) {
			System.out.println("\n┌──────────────────────────────────────┐");
			System.out.println(String.format("\t阻擋已被封鎖的IP連線! IP=%s", ip));
			System.out.println("└──────────────────────────────────────┘\n");
