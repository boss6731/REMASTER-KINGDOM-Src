...
public class MJHttpRequest implements HttpRequest{
	...
	private void parse_post_datas(){
		m_bit |= INIT_POST_DATA;
		m_post_datas = new HashMap<String, String>();
		HttpPostRequestDecoder decoder = new HttpPostRequestDecoder(m_factory, m_request);
		try{
			for(InterfaceHttpData data : decoder.getBodyHttpDatas()){
				...
				if(HttpDataType.Attribute == data_type){
					try{
						...
						m_post_datas.put(attribute.getName(), attribute.getValue());
					}catch(Exception e){
						e.printStackTrace();
						MJWSHandler.print(m_ctx, String.format("Throws BODY Attribute - %s", data.getHttpDataType().name()));
					}
				}else{
					MJWSHandler.print(m_ctx, String.format("invalid BODY data - %s : %s", data_type.name(), data));
				}
			}
		}finally{
			try{
