		...
		try {
			...
			for (Iterator<String> iterator = key.iterator(); iterator.hasNext();) {
				...
				if(tbi.category.equalsIgnoreCase("select_adena")) {
					tbi.item_count = Integer.valueOf((String) jsonObj.get("item_count")) * MJNCoinSettings.ADENA_GENERATE_UNIT;//물품등록 완료 누루면 빠져나가는 금액
					tbi.item_name = "아데나";
					tbi.item_enchant = 0;
					tbi.item_bless = "bless_normal";
					tbi.item_attr = "attr_none";
