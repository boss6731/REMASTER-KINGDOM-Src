...
public class MJTradeAdenaAverageResponse extends MJHttpResponse {
	...
	private void getAdenaAverageInfoInt(JsonObject json) {
		...
		try {
			...
			//TODO 修改物品交易中亞丁幣出售時的售價表示及限制的部分
			if(total_price != 0 && total_item_count > 0) {//주석
	...
	private void getAdenaAverageInfo(StringBuilder stringBuilder) {
		...
		try {
			...
			stringBuilder.append("<span class=\"exp_txt\"> * 實時平均售價統計<font style=\"color:#da2121;\"> "+MJNCoinSettings.ADENA_MARKET_STATISTICS+"一千萬 <font style=\"color:#9d9692;\">每金幣 <font style=\"color:#da2121;\">");
			stringBuilder.append(MJString.parse_money_string(result_price) + "元");
//			stringBuilder.append("<span class=\"exp_txt\"> * 實時靈魂寶珠收購統計 <font style=\"color:#da2121;\"> "+MJNCoinSettings.GM_Purchase_Count+"億 <font style=\"color:#9d9692;\">每金幣 <font style=\"color:#da2121;\">");
//			stringBuilder.append(MJString.parse_money_string(MJNCoinSettings.GM_Cash_Count) + "元");
			stringBuilder.append("<span class=\"exp_txt\"> * 輸入帳號密碼時<font style=\"color:#da2121;\"> [영문]<font style=\"color:#9d9692;\">請更改為…後輸入.<font style=\"color:#da2121;\">");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			SQLUtil.close(rs, pstm, con);
