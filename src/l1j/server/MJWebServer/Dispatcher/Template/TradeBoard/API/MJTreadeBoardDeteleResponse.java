		...
		if (_player == null) {
			...
			try {
				...
				if (rs.next()) {
					...
					_player.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\f3商品已成功取消銷售處理。"));
					_action = "delete_ok";
					_message = "該文章已被取消。";
				}
			} catch (Exception e) {
	...
	private boolean checkDeteleAccountSame(int board_id) {
		...
		if (_player == null) {
			...
			try {
				...
				rs = pstm.executeQuery();
				if (rs.next()) {
					if (rs.getString("state").equalsIgnoreCase("銷售")) {
						int insertItem_id = ItemTable.getInstance()
								.findItemIdByNameWithoutSpace(rs.getString("item_name"));
						commit(_player, insertItem_id, rs.getInt("item_count"), rs.getInt("item_enchant"),
								Integer.valueOf(getReName(rs.getString("item_attr"))), rs.getInt("item_attr_level"),
								rs.getInt("item_bless"));
						_player.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\f3商品已成功取消銷售處理。"));
