package l1j.server.MJWebServer.Dispatcher.Template.mychaTrad;
...
public class MJChaTradIndexResponse extends MJHttpResponse {
	private static String m_page_document;

	public MJChaTradIndexResponse(MJHttpRequest request) {
		super(request);
	}
	...
	public HttpResponse get_response() throws l1j.server.MJWebServer.Dispatcher.MJHttpClosedException {
		...
		if (Config.Web.appcenterCacheReset) {
			m_page_document = load_file_string("./appcenter/ChaTrad_index.html");
		...
		params.add(
				new MJKeyValuePair<String, String>("{LEVEL}", _user != null ? String.valueOf(_user.getLevel()) : "-"));
		...
		params.add(new MJKeyValuePair<String, String>("{PROFILE_URL}",
				"https://wstatic-cdn.plaync.com/resource/lineage/profile/char" + _user.getClassId() + "_"
						+ (_user.getGender() == 0 ? "m" : "f") + ".png"));
		params.add(new MJKeyValuePair<String, String>("{NCOIN}",
				_player != null ? MJString.parse_money_string(_player.getAccount().Ncoin_point) : "0"));
