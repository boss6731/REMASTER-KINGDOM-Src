...
public class MJCouponChargeInsertDatabase extends MJHttpResponse {
	...
	private void insertCouponInfo(String code) {
		try {
			...
			//_player.getInventory().storeItem(m_pInfo.get_itemid(), m_pInfo.get_count());//인벤으로 바로 지급
			SC_GOODS_INVEN_NOTI.do_send(_player);
			_player.send_effect(2048);
			_player.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,String.format("\\f3쿠폰 아이템이 (%s)개가 지급 되었습니다. 부가 아이템 창고 에서 수령 하시길 바랍니다.", new DecimalFormat("#,##0").format(m_pInfo.get_count()))));
			_player.sendPackets(String.format("쿠폰 아이템이 (%s)개가 지급되었습니다.", new DecimalFormat("#,##0").format(m_pInfo.get_count())));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
