...
public class CommandInventoryDelete implements MJMyPageGmCommand{
	@Override
	...
	private void onRemove(L1PcInstance pc, String parameter0) {
		if(MJString.isNullOrEmpty(parameter0)) {
			return;
		}
		try {
			int objectId = Integer.parseInt(parameter0);
			pc.getInventory().removeItem(objectId);
		}catch(Exception e) {
		}
	}
