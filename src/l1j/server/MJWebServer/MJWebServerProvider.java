...
public class MJWebServerProvider {
	...
	public void bind(int port, final l1j.server.MJWebServer.MJDefaultWebServer initializer){
		if(!alive()){
			return;
		}
		lock.fullyLock();
		try{
			if(openeds.containsKey(port)){
				throw new RuntimeException(String.format("already bind port %d", port));
			}
