...
public class Demon implements Runnable {
	...
	public void run() {
		...
		// System.out.println(""+ 上午下午 + " " + cal.get(時間) + "時" + cal.get(分) + "分" + "
		// ■■■■■■ 冰魔結束 " + _map+" ■■■■■■");
		System.out.println("" + 上午下午 + " " + cal.get(時間) + "時" + cal.get(分) + "分" + "   ■"+ BossNames[m_select_bosses_index] + "■ 結束地圖:" + _map + "■");
	}
	...
	public void Start() {
		...
		// System.out.println(""+ 上午下午 + " " + cal.get(時間) + "時" + cal.get(分) + "分" + "
		// ■■■■■■ 開始冰魔 " + _map+" ■■■■■■");
		System.out.println("" + 上午下午 + " " + cal.get(時間) + "時" + cal.get(分) + "分" + "   ■"+ BossNames[m_select_bosses_index] + "■ 開始地圖::" + _map + "■");
	}
	...
	private void Boss() {
		if (_list5.size() > 0) {
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
				if (obj instanceof L1PcInstance) {
					L1PcInstance pc = (L1PcInstance) obj;
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "請見見牆後面的馬賓。"));
				}
			}
	...
	private void End() {
		for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
			if (obj instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) obj;
				// L1Teleport.teleport(pc, 34068, 32311, (短)4, 4, true); //這是該部分將要落下的座標
				//
				//
				//
				//
	...
	private void SpawnMonster() {
		// NPC 生成
		spawn(32734, 32802, (short) _map, 4, 5086, 1, 2, 0); // 象牙塔間諜
		spawn(32860, 32920, (short) _map, 6, 5087, 1, 2, 0); // 斯溫
		...
		spawn(32845, 32920, (short) _map, 6, BossIds[m_select_bosses_index], 0, 2, 5); // 冰魔
	}

	public static void spawn(int x, int y, short MapId, int Heading, int npcId, int randomRange, int type,
			int roomnumber) {
		try {// 在類型部分，Wollae是2，而Ice Queen是1。
