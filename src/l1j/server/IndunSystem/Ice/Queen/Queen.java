...
public class Queen implements Runnable {
	...
	public void run() {
		...
		Queen_Delete();
		// System.out.println("" + ampm + " " + cal.get(Calendar.HOUR) + "時" + cal.get(Calendar.MINUTE) +
		// "分" + " ■■■■■■ 冰雪女王結束 " + _map + " ■■■■■■");
		System.out.println("" + ampm + " " + cal.get(Calendar.HOUR) + "時" + cal.get(Calendar.MINUTE) + "分" + " ■■■■■■ 冰雪女王結束 " + _map + " ■■■■■■");
	...
	public void Start() {
		...
		// System.out.println("" + AMPM + " " + cal.get(Calendar.HOUR) + "時" + cal.get(Calendar.MINUTE) +
		// "分" + " ■■■■■■ 冰雪女王開始 " + _map + " ■■■■■■");
		System.out.println("" + ampm + " " + cal.get(Calendar.HOUR) + "時" + cal.get(Calendar.MINUTE) + "分" + "   ■冰雪女王開始■地圖:" + _map + "■");
	}
		...
		if (Time % 60 == 0) {
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
				if (obj instanceof L1PcInstance) {
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, minute + "分鐘後將強制移動到村莊。"));
				}
			}
		}
	}
	...
	private void Boss() {
		if (_list5.size() > 0) {
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
				if (obj instanceof L1PcInstance) {
					L1PcInstance pc = (L1PcInstance) obj;
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "請在牆後面見到斯賓。"));
				}
			}
			openDoor(5151); // 第五扇門打開。.
			BossRoom = false;
		}
	}
	...
	private void SpawnMonster() {
		// NPC生成
		spawn(32734, 32802, (short) _map, 4, 5086, 1, 1, 0); // 象牙塔情報員
		spawn(32860, 32920, (short) _map, 6, 5087, 1, 1, 0); // 斯賓
