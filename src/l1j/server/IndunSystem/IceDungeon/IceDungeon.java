...
public class IceDungeon implements Runnable {
	...
	public void run() {
		...
		Dungeon_Delete();
		System.out.println("" + ampm + " " + cal.get(Calendar.HOUR) + "時" + cal.get(Calendar.MINUTE) + "分" + "   ■冰霜地牢結束■地圖:" + _map + "■");
	...
	public void Start() {
		...
		System.out.println("" + ampm + " " + cal.get(hour) + "時" + cal.get(minute) + "分" + "   ■冰霜地牢開始■地圖:" + _map + "■");
	}
		...
		if (Time % 60 == 0) {
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
				if (obj instanceof L1PcInstance) {
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, minute + "分鐘後會強制傳送到村莊。"));
				}
			}
		}
	}
		...
		if (_list5.size() > 0) {
			...
			for (L1Object obj : L1World.getInstance().getVisibleObjects(_map).values()) {
				if (obj instanceof L1PcInstance) {
					...
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "請去見在牆後面的斯賓。"));
				}
			}
			openDoor(5151); // 第五扇門開啟.
			BossRoom = false;
		}
	}
	...
	private void SpawnMonster() {
		...
		spawn(32860, 32920, (short) _map, 6, 5087, 1, 0); // 斯文
		...
		if (per >= 1 && per <= 3){
			spawn(32845, 32920, (short) _map, 6, 81201, 0, 5); // 冰惡魔
		}else if (per >= 4 && per <= 6){
			spawn(32843, 32916, (short) _map, 6, 5082, 0, 5); // 冰女王近衛兵(弓)
			spawn(32843, 32927, (short) _map, 6, 5082, 0, 5); // 冰女王近衛兵(弓)
			spawn(32845, 32920, (short) _map, 6, 45609, 0, 5); // 冰女王
		}else if (per >= 7 && per <= 9){
			spawn(32845, 32920, (short) _map, 6, 7320217, 0, 5); // 冰舞兔
		}
	}
	
	public static void spawn(int x, int y, short MapId, int Heading, int npcId, int randomRange, int roomnumber) {
		try {// 類型這部分本來是2，但是冰女王是1，如果按照類型去看
			L1NpcInstance npc = NpcTable.getInstance().newNpcInstance(npcId);
			if (npc == null)
				System.out.println(npcId + " : 沒有火焰之氣任務的 NPC 資訊。");
