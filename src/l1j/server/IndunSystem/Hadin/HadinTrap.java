...
public class HadinTrap implements Runnable {
	...
	public boolean LEVEL_1_TRAP_CK = false;// 第一間房有4張凳子
	public boolean LEVEL_2_TRAP_CK = false;// 1個雙人房腳凳
	public boolean LEVEL_3_TRAP_CK = false;// 出租房間有4個腳踏板
	public boolean LEVEL_4_TRAP_CK = false;// 最後一個房間腳手架
			...
			try {
				...
				for (L1Object ob : cklist) {
					...
					if (ob instanceof L1PcInstance) {
						...
						if (door.getNpcTemplate().get_npcId() == 900151
								|| door.getNpcTemplate().get_npcId() == 900152) {
				...
				// System.out.println("哈丁陷阱事件线程错误实例ID： "+_map +" -> "+e);
			} finally {
				try {
					Thread.sleep(2000);
				} catch (Exception e) {
	...
	private void TRAP_ON_CK() {
		...
		if (LEVEL_1_TRAP_1 && LEVEL_1_TRAP_2 && LEVEL_1_TRAP_3 && LEVEL_1_TRAP_4) {// 第一個房間有 4 個腳踏板
			LEVEL_1_TRAP_CK = true;
			door = doorList.get("骷髏門 2");
			door.open();
			BonginSendPacekt(7611);
		} else if (TRAP_1) {// 第二間客房，1 個腳踏板
			...
			door.open();
		} else if (LEVEL_2_TRAP_1 && LEVEL_2_TRAP_2 && LEVEL_2_TRAP_3 && LEVEL_2_TRAP_4) {// 出租房間有4個腳踏板，2個門打開
			...
			for (L1Object ob : cklist) {
				...
				if (ob instanceof L1PcInstance) {
					...
					pc.getInventory().storeItem(410108, 1); // 秘密袋
				}
			}
	...
	private void TRAP_ON(L1PcInstance pc) {
		...
		if (pc.getX() == 32666 && pc.getY() == 32817)
			...
			LEVEL_1_TRAP_4 = true;// 第一個房間有 4 個腳踏板

		else if (pc.getX() == 32684 && pc.getY() == 32816)
			TRAP_1 = true;// 第二間客房，1 個腳踏板
