...
public class HadinTrap implements Runnable {
	...
	public boolean LEVEL_1_TRAP_CK = false; // 第一道房間的 4 個地板陷阱
	public boolean LEVEL_2_TRAP_CK = false; // 第二道房間的 1 個地板陷阱
	public boolean LEVEL_3_TRAP_CK = false; // 第三道房間的 4 個地板陷阱
	public boolean LEVEL_4_TRAP_CK = false; // 最後一道房間的地板陷阱
	...
	private void TRAP_ON_CK(){
		...
		if (LEVEL_1_TRAP_1 && LEVEL_1_TRAP_2 && LEVEL_1_TRAP_3 && LEVEL_1_TRAP_4) { // 第一道房間的 4 個地板陷阱
			LEVEL_1_TRAP_CK = true;
			door = doorList.get("骷髏門 2");
			door.open();
			BonginSendPacekt(7611);
		} else if (TRAP_1) { // 第二道房間的 1 個地板陷阱
			LEVEL_2_TRAP_CK = true;
			door = doorList.get("骷髏門 4");
			door = doorList.get("骷髏門 5");
			door = doorList.get("骷髏門 6");
			door = doorList.get("骷髏門 8");
			door.open();
		} else if (LEVEL_2_TRAP_1 && LEVEL_2_TRAP_2 && LEVEL_2_TRAP_3 && LEVEL_2_TRAP_4) { // 第三道房間的 4 個地板陷阱，打開 2 扇門
			LEVEL_3_TRAP_CK = true;
			door = doorList.get("骷髏門 7");
			door = doorList.get("骷髏門 9");
			...
			for(L1Object ob : cklist){
				...
				if(ob instanceof L1PcInstance){
					L1PcInstance pc = (L1PcInstance) ob;
//					L1Teleport.teleport(pc, 32785, 32821, _map, 5, true);
					pc.start_teleport(32785, 32821, _map, 5, 18339, true, false);
				}
			}
			...
			for(L1Object ob : cklist){
				...
				if(ob instanceof L1PcInstance){
					...
					pc.getInventory().storeItem(410108, 1); // 秘密包裹
				}
			}
	...
	private void TRAP_ON(L1PcInstance pc){
		...
		else if (pc.getX() == 32668 && pc.getY() == 32819) LEVEL_1_TRAP_4 = true; // 第一道房間的 4 個地板陷阱

		else if (pc.getX() == 32684 && pc.getY() == 32816) TRAP_1 = true; // 第二道房間的 1 個地板陷阱
		...
		else if (pc.getX() == 32712 && pc.getY() == 32793) LEVEL_2_TRAP_4 = true; // 第三道房間的 4 個地板陷阱
