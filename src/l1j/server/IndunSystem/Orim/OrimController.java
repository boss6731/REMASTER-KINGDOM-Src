...
public class OrimController extends Thread {
	...
	private void MSGOrimAndPartyleader(int i) {
		...
		switch (i) {
			case 0:
				sendMessage("奧林：大家不要慌張，請聽我說。");
				talkPartyleader("大家不要慌張，請聽我說。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：我正離開島嶼前往大陸，去見師父。");
				talkPartyleader("奧林正離開島嶼前往大陸，去見師父。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：我現在正被黑魔法師奧丁追趕。");
				talkPartyleader("奧林現在正被黑魔法師奧丁追趕。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：如果不需要情況說明，請按 [alt + 1] 來跳過。");
				talkPartyleader("如果不需要情況說明，請按 [alt + 1] 來跳過。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
				}
			case 1:
				sendMessage("奧林：攻擊我們船的是名為奧丁的東西。");
				talkPartyleader("攻擊我們船的是名為奧丁的東西。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：首先，我會告訴你們如何使用艦炮。");
				talkPartyleader("首先，我會告訴你們如何使用艦炮。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：稍後地面會出現紅色圓圈，請站到中心位置。");
				talkPartyleader("稍後地面會出現紅色圓圈，請站到中心位置。");
				try {
					Thread.sleep(3000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：然後，我會站在大炮之間有相同圖案的踏板上。");
				talkPartyleader("然後，我會站在大炮之間有相同圖案的踏板上。");
				StarZone(1, 0);
				try {
					Thread.sleep(3000L);
				} catch (Exception e) {
				...
				sendMessage("奧林：接著，當我按下 [alt + 4] 時，大炮就會發射。");
				talkPartyleader("接著，當我按下 [alt + 4] 時，大炮就會發射。");
				try {
					Thread.sleep(10000L);
				} catch (Exception e) {
					// 處理例外情況
				...
				attackFormat();
				sendMessage("奧林：時間不多了，我們進行下一步。");
				talkPartyleader("時間不多了，我們進行下一步。");
				try {
					Thread.sleep(3000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：但是，由於敵人也會攻擊，我們也需要進行防禦。");
				talkPartyleader("但是，由於敵人也會攻擊，我們也需要進行防禦。");
				try {
					Thread.sleep(3000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：為了防禦，請進入黃色圓圈內。");
				talkPartyleader("為了防禦，請進入黃色圓圈內。");
				StarZone(0, 1);
				try {
					Thread.sleep(3000L);
				} catch (Exception e) {
				...
				sendMessage("奧林：我會在同一個位置用 [alt + 2] 施放防禦法術。");
				talkPartyleader("我會在同一個位置用 [alt + 2] 施放防禦法術。");
				try {
					Thread.sleep(12000L);
				} catch (Exception e) {
				}
	...
	private void MSGOrim(int i, int stageNum) {
		...
		switch (i) {
			case 0:
				sendMessage("奧林：現在，我們必須準備應對敵人的攻擊。請快點行動。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：船上有潛伏的怪物。請消滅它們。");
				break;
			case 1:
				sendMessage("奧林：感覺附近有什麼在徘徊。請小心。");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				...
				switch (stageNum) {
					...
					case 7:
					case 10:
						sendMessage("奧林：附近有一群鯊魚。請小心。");
						break;
					case 2:
					case 5:
					case 8:
					case 11:
						sendMessage("奧林：看來附近是海妖的棲息地。請小心。");
						break;
					case 3:
					case 6:
					case 9:
					case 12:
						sendMessage("奧林：那是海龍。不要因為好奇而靠近它們。");
				}

				break;
			case 2:
				if (stageNum == 1)
					sendMessage("奧林：敵人的船正在靠近！請準備！");
				else if (stageNum == 4)
					sendMessage("奧林：呃..! 這次來的船上感覺到奧丁的氣息！");
				else if (stageNum == 8)
					sendMessage("奧林：小心！這次來的船比以往任何時候都強大。");
				else {
					sendMessage("奧林：感覺到比之前更強的氣息！請小心！");
				}
				Effect(0);
				break;
			case 3:
				sendMessage("奧林：敵人的攻擊正在損壞船隻。多次損壞會導致船隻沉沒，請注意。");
				Effect(0);
				break;
			case 4:
				sendMessage("奧林：船隻即將相撞！小心不要掉入海中！");
				Effect(0);
				break;
			case 5:
				sendMessage("奧林：船隻相撞後，敵人會跳上來！請小心！");
				Effect(0);
				break;
			case 6:
				sendMessage("奧林：快逃！某處有困住你的物品！");
				break;
			case 7:
				sendMessage("奧林：船隻即將沉沒。無論如何都要保護船隻！");
				Effect(0);
				break;
			case 8:
				sendMessage("奧林：船隻正在沉沒！快逃！這樣下去不行！");
				Effect(1);
				break;
			case 9:
				sendMessage("奧林：幾乎全部被擊退了。請再加把勁！");
				break;
			case 10:
				sendMessage("奧林：闖入的敵人正在損壞我們的船隻！快點！");
				break;
			case 11:
				sendMessage("奧林：時間不多了。快點。");
				break;
			case 12:
				sendMessage("奧林：那是什麼可怕的東西！看起來很危險！");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
				}
				sendMessage("奧林：呃！連海洋生物也在攻擊我們！偏偏在這種時候！");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
				}
	...
	private void MSG1(int stageNum, int j) {
		...
		switch (j) {
			case 0:
				sendMessage1("戰鬥即將開始 [" + stageNum + "/12]");
				try {
					Thread.sleep(5000L);
				} catch (Exception e) {
					// 處理例外情況
			...
			case 1:
				sendMessage1("第一支部隊已經闖入。");
				break;
			case 2:
				sendMessage1("第二支部隊已經闖入。");
				break;
			case 3:
				if ((stageNum == 4) || (stageNum == 8) || (stageNum == 12))
					MSGOrim(9, 0);
				else
					sendMessage1("第三支部隊已經闖入。");
				break;
		}
	}
	...
	private void MSGOlden(int i) {
		...
		switch (i) {
			case 1:
				sendMessage2("奧丁：懶惰的傢伙們！快出去戰鬥！把那些敵人全部消滅掉！");
				break;
			case 2:
				sendMessage2("奧丁：抓住那個傢伙！");
				break;
		}
	}
	...
	private void End() {
		...
		InDunclear();
		clearPlayMember();
		L1World.getInstance().broadcastServerMessage("海音地下城已經重置。");
	}
}