 ...
 public class DominanceFloorLv9
   implements Runnable
 {
   ...
   public DominanceFloorLv9(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse, int effectid) {
     this._npcid = bossid;
     this._mapx = x;
     this._mapy = y;
     this._mapid = mapid;
         ...
         switch (this.stage) {
           case 1:
             for (L1PcInstance pc : PcCK()) {
               pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27659"));
             }
           ...
           case 2:
             if (this._END == true) {
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27667"));
               }
               Thread.sleep(7000L);
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27668"));
               }
               this.Running = false;
               break;
             }
