 ...
 public class DominanceFloorLv3
   implements Runnable
 {
   ...
   public DominanceFloorLv3(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse, int effectid) {
     this._npcid = bossid;
     this._mapx = x;
     this._mapy = y;
     this._mapid = mapid;
         ...
         switch (this.stage) {
           case 1:
             for (L1PcInstance pc : PcCK()) {
               pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27442"));
             }
           ...
           case 2:
             if (this._END == true) {
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27450"));
               }
               Thread.sleep(7000L);
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27451"));
               }
               this.Running = false;
               break;
             }
