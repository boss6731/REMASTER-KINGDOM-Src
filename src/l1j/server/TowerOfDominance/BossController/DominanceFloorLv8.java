...
public class DominanceFloorLv8 implements Runnable {
	...
	public DominanceFloorLv8(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse,
			int effectid) {
		_npcid = bossid;
		_mapx = x;
		_mapy = y;
	...
	public void run() {
		while (Running) {
			try {
				...
				switch (stage) {
				case EVENT:
					final L1NpcInstance[] knifes = new L1NpcInstance[] {
							spawn(32669, 32839, (short) _mapid, 0, 50000067, ActionCodes.ACTION_Appear),
							spawn(32673, 32843, (short) _mapid, 0, 50000067, ActionCodes.ACTION_Appear),
							spawn(32664, 32845, (short) _mapid, 0, 50000067, ActionCodes.ACTION_Appear),							
					};
					for (L1PcInstance pc : PcCK()) {
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27649"));
					}
					Thread.sleep(3000L);
				...
				case END:
					if (_END == true) {
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27657"));
						}
						Thread.sleep(7000);
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27658"));
						}
						Running = false;
						break;
					}
	...
	private static L1NpcInstance spawn(int x, int y, short MapId, int Heading, int npcId, int actioncode) {
