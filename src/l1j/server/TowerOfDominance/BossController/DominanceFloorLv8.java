...
/*     */ 
 ...
 public class DominanceFloorLv8
   implements Runnable
 {
   ...
   public DominanceFloorLv8(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse, int effectid) {
     this._npcid = bossid;
     this._mapx = x;
     this._mapy = y;
     this._mapid = mapid;
   ...
   public void run() {
     while (this.Running) {
       ...
       try { final L1NpcInstance[] knifes; TimeCheck();
         switch (this.stage) {



           case 1:
             knifes = new L1NpcInstance[] { spawn(32669, 32839, (short)this._mapid, 0, 50000067, 4), spawn(32673, 32843, (short)this._mapid, 0, 50000067, 4), spawn(32664, 32845, (short)this._mapid, 0, 50000067, 4) };

             for (L1PcInstance pc : PcCK()) {
               pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27649"));
             }
             Thread.sleep(3000L);
           ...
           case 2:
             if (this._END == true) {
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27657"));
               }
               Thread.sleep(7000L);
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27658"));
               }
               this.Running = false;
               break;
             }
