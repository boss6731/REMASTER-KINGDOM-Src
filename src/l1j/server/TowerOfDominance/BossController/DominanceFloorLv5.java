...
public class DominanceFloorLv5 implements Runnable {
	...
	public DominanceFloorLv5(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse,
			int effectid) {
		_npcid = bossid;
		_mapx = x;
		_mapy = y;
			...
			try {
				...
				case EVENT:
					for (L1PcInstance pc : PcCK()) {
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27462"));
					}
				...
				case END:
					if (_END == true) {
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27470"));
						}
						Thread.sleep(7000);
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27471"));
						}
						Running = false;
						break;
					}
