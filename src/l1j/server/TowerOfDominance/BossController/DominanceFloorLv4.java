			...
			try {
				...
				case EVENT:
					for (L1PcInstance pc : PcCK()) {
						pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27452"));
					}
				...
				case END:
					if (_END == true) {
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27460"));
						}
						Thread.sleep(7000);
						for (L1PcInstance pc : PcCK()) {
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fY$27461"));
						}
						Running = false;
						break;
					}
