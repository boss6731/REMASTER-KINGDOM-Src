 ...
 public class DominanceFloorLv10
   implements Runnable
 {
   ...
   public DominanceFloorLv10(int bossid, int x, int y, int mapid, boolean mentuse, String ment, boolean effectuse, int effectid) {
     this._npcid = bossid;
     this._mapx = x;
     this._mapy = y;
     this._mapid = mapid;
         ...
         switch (this.stage) {
           case 1:
             for (L1PcInstance pc : PcCK()) {
               pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27669"));
             }
           ...
           case 2:
             if (this._END == true) {
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27677"));
               }
               Thread.sleep(7000L);
               for (L1PcInstance pc : PcCK()) {
                 pc.sendPackets((ServerBasePacket)new S_PacketBox(84, "\\fY$27678"));
               }
               this.Running = false;
               break;
             }
