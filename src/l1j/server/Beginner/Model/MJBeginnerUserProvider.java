...
public class MJBeginnerUserProvider {
	...
	public void onProgressNoti(L1PcInstance pc) {
		...
		for(MJBeginnerUserProgress progress : user.values()) {
			...
			SC_QUEST_PROGRESS_UPDATE_NOTI not = SC_QUEST_PROGRESS_UPDATE_NOTI.newInstance();
			not.set_quest(progress.convertClientModel());
			// 新任務註釋
			pc.sendPackets(not, MJEProtoMessages.SC_QUEST_PROGRESS_UPDATE_NOTI);
