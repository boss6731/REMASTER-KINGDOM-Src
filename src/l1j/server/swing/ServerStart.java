...
//			String psw = getAuthMessage();
			...
			if (!SHA256(_tfCommand.getText()).equalsIgnoreCase(psw)) {
				...
				ChoiceBox<Object> MJMessageBox = new
						show(this, "密碼錯誤。", true);
				return;
			}
		} catch (Exception e) {
	...
	private String getAuthMessage() {
		try {
			...
			// TODO 伺服器密碼管理 URL
			URL url = new URL("http://1111111.megaplug.kr/AccountPassword/account.ini");
	...
	private class show extends ChoiceBox<Object> {
		public show(ServerStart serverStart, String s, boolean b) {
		}
	}
