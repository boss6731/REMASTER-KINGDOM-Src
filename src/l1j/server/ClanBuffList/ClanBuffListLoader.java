package l1j.server.server.ClanBuffList;

import l1j.server.MJDTSSystem.MJDTSElement;
...
public class ClanBuffListLoader {
	...
	private HashMap<Integer, ArrayList<l1j.server.ClanBuffList.ClanBuffListInfo>> _clan_buff_list;
	private HashMap<Integer, l1j.server.ClanBuffList.ClanBuffItemInfo> _clan_buff_items;
	...
	private void Infoload() {
		final HashMap<Integer, ArrayList<l1j.server.ClanBuffList.ClanBuffListInfo>> buff_list_map = new HashMap<Integer, ArrayList<ClanBuffListInfo>>();
		Selector.exec("select * from clan_buff_list", new FullSelectorHandler() {
			@Override
			public void result(ResultSet rs) throws Exception {
				while(rs.next()) {
					try {
						MJDTSElement ClanBuffListInfo = null;
						MJDTSElement Info = ClanBuffListInfo.newInstance(rs);
						if (Info == null)
							continue;

						ArrayList<l1j.server.ClanBuffList.ClanBuffListInfo> inputList = buff_list_map.get(Info.get_clan_id());
						if (inputList == null) {
	...
	private void Itemload() {
		HashMap<Integer, l1j.server.ClanBuffList.ClanBuffItemInfo> items = new HashMap<Integer, l1j.server.ClanBuffList.ClanBuffItemInfo>();
		Selector.exec("select * from clan_buff_items", new FullSelectorHandler(){
			@Override
			public void result(ResultSet rs) throws Exception {
				while(rs.next()){
					try{
						l1j.server.ClanBuffList.ClanBuffItemInfo Info = l1j.server.ClanBuffList.ClanBuffItemInfo.newInstance(rs);
	...
	public void addClanBuff(L1Clan clan, int SkillId, String Skill_name, Timestamp skilltime) {
		Updator.exec("insert into clan_buff_list set clan_id=?, clan_name=?, skill_id=?, skill_name=?, remaining_time=?", new Handler(){
			@Override
		...
		l1j.server.ClanBuffList.ClanBuffListInfo Info = new l1j.server.ClanBuffList.ClanBuffListInfo();
		...
		ArrayList<l1j.server.ClanBuffList.ClanBuffListInfo> inputList = _clan_buff_list.get(clan.getClanId());
		if (inputList == null) {
			inputList = new ArrayList<l1j.server.ClanBuffList.ClanBuffListInfo>();
	...
	public <ClanBuffListInfo> ArrayList<ClanBuffListInfo> getClanBuffList(L1Clan clan) {
		if (clan != null) {
			if (_clan_buff_list.containsKey(clan.getClanId())) {
				return (ArrayList<ClanBuffListInfo>) _clan_buff_list.get(clan.getClanId());
			}
		}
		return null;
	}
	
	public ArrayList<l1j.server.ClanBuffList.ClanBuffItemInfo> values(){
		return new ArrayList<l1j.server.ClanBuffList.ClanBuffItemInfo>(_clan_buff_items.values());
	}
	
	public <ClanBuffItemInfo> ClanBuffItemInfo getItemId(int itemid){
		ArrayList<ClanBuffItemInfo> items = (ArrayList<ClanBuffItemInfo>) values();
		if(items == null)
			return null;
	...
	public <ClanBuffItemInfo> boolean useItem(L1PcInstance pc, L1ItemInstance item){
		...
		for(ClanBuffListInfo buff_info : info_list) {
			if (skill_id == L1SkillId.CLAN_EXP_BUFF_1ST) {
				if (buff_info.get_skill_id() == L1SkillId.CLAN_EXP_BUFF_2ND) {
					...
					pc.sendPackets("無法重複應用相同的 1級 和 2級 增益效果。");
					return false;
				}
			}
...
//				return false;
	...
	public void skill_info(L1PcInstance pc, int skillid, long skill_time) {
		...
		case L1SkillId.CLAN_EXP_BUFF_1ST:
			if (pc.getClanRank() == L1Clan.KING || pc.getClanRank() == L1Clan.VICE_KING) {
			...
			} else if (pc.getClanRank() == L1Clan.NORMAL) {
				pc.add_item_exp_bonus(12); // 此部分應根據實際需要填寫
			}
				pc.add_item_exp_bonus(10);	
			}
			break;
		case L1SkillId.CLAN_EXP_BUFF_2ND:
			iif (pc.getClanRank() == L1Clan.KING || pc.getClanRank() == L1Clan.VICE_KING) {
		...
		if (pc.getClanRank() == L1Clan.KING || pc.getClanRank() == L1Clan.VICE_KING || pc.getClanRank() == L1Clan.ELITE || pc.getClanRank() == L1Clan.GUARDIAN) {
			// HP +50, 命中率 +2, 加成伤害 +1, 魔法抗性 +3, 魔法命中率 +1
		}
				pc.addMagicHit(1);
			} 
			//生命值+50，命中+2，附加傷害+1，魔力+3
			pc.addMaxHp(50);
			pc.addDmgup(1);
			pc.addBowDmgup(1);
			pc.addHitup(2);
		...
		case L1SkillId.CLAN_PVP_BUFF_2ND:
			if (pc.getClanRank() == L1Clan.KING || pc.getClanRank() == L1Clan.VICE_KING || pc.getClanRank() == L1Clan.ELITE || pc.getClanRank() == L1Clan.GUARDIAN) {
				// 生命值+50，命中率+2，額外傷害+1，魔法抗性+3，魔法命中率+1

				pc.addMagicHit(1);
				pc.addHitup(1);
				pc.addBowHitup(1);
			}
			//生命+100，命中+3，附加傷害+2，所有命中+1
			pc.addMaxHp(100);
			pc.addDmgup(2);
			pc.addBowDmgup(2);
			pc.addHitup(2);
