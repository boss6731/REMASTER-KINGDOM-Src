...
import MJNCoinSystem.AttributeKey;
...
public class MJDTSElement{
	...
	public static AttributeKey<MJClientEntranceService.EntranceClientModel> newInstance(String entranceAttribute) {
		return null;
	}
	...
	public void use(L1PcInstance pc, L1ItemInstance item){
		...
		if((_max_level > 0 && _max_level < lvl) || (_min_level > 0 && _min_level > lvl)){
			pc.sendPackets(String.format("%s 物品只能在 %d 級到 %d 級之間使用。", item.getName(), _min_level, _max_level));
			return;
		}

		if(_validation && !is_valid(pc)){
			pc.sendPackets("無法使用此物品。");
			return;
		}

		TeleporterActionListener listener = ActionListenerLoader.getInstance().findListener(ListenerFinderTable.getInstance().getListenerInfo(_map_id));
		if (listener != null) {
		...
		if(_use_count > 0){
			...
			if(!pc.getInventory().consumeItem(item, amount)){
				pc.sendPackets(String.format("%s 物品需要 %d 個才能使用。", item.getName(), amount));
				return;
			}
		}
