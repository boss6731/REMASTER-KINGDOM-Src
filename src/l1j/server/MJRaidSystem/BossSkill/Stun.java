...
/** shock stun. **/
public class Stun extends MJRaidBossSkill{
	private static final S_Paralysis s_paral = new S_Paralysis(S_Paralysis.TYPE_STUN, true);
	
	@Override
	public String toString(){
		return Stun.class.getName();
	}
	...
	private void processAction(L1NpcInstance own, L1PcInstance pc){
		...
		int time = calcStunTime();
		pc.setSkillEffect(L1SkillId.SHOCK_STUN, time);
		L1EffectSpawn.getInstance().spawnEffect(MJRaidLoadManager.MRS_BS_STUN_EFFECTID, time, pc.getX(), pc.getY(), pc.getMapId());
		pc.sendPackets(s_paral, false);
	...
	private int calcStunTime(){
		return _rnd.nextInt(MJRaidLoadManager.MRS_BS_STUN_MAX - MJRaidLoadManager.MRS_BS_STUN_MIN) + MJRaidLoadManager.MRS_BS_STUN_MIN;
	}
}
