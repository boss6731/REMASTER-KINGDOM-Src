...
public class ExpMergeController {
	...
	private final ExpMergeDatabaseService databaseService;
	...
	private ExpMergeController() {
		...
		MJMonitorCacheModel<ExpMergeSystemModel> model = MJMonitorCacheProvider.newJsonFileCacheModel("mj-merge-system-model", "./config/exp-merge.json", ExpMergeSystemModel.class, MJEncoding.MS949);
	...
	private class ExpMergeModelConverter implements MJMonitorCacheConverter<ExpMergeSystemModel>, l1j.server.ExpMerge.ExpMergeModelConverter {
		@Override
	...
	private void onMerge0(final L1PcInstance pc, final ExpMergeTargetModel targetModel) {
		...
		pc.sendPackets(String.format("\f3已獲得 %d 的經驗值。", appendExp));
	}
}
