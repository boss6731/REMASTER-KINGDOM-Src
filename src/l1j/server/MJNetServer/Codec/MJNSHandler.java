...
public class MJNSHandler extends ChannelInboundHandlerAdapter{
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		String host_address = inetAddr.getAddress().getHostAddress();
		if (MJNSDenialAddress.getInstance().is_denials_address(host_address)) {
			ctx.close();
			return;
		}

		/*if (MJHttpLoginManager.getInstance().get_login_info_from_address(host_address) == null) {
			print(host_address, port, "�� �׼��� ���� �����Ͽ� �����߽��ϴ�.");
			ctx.close();
			return;
		}*/

		if (MJHttpLoginManager.getInstance().remove_and_get_from_address(host_address) == null) {
			synchronized (m_counter_lock) {
				if (m_checker == null) {
		...
		if (port <= MJNetServerLoadManager.NETWORK_WELL_KNOWNPORT) {
			print(host_address, port, "����ڰ� WellKnown-Port�� ������ �õ��Ͽ� �����߽��ϴ�.");
		...
		if (_clnts.size() + 1 >= Config.Login.MaximumOnlineUsers) {
			print(host_address, port, String.format("���� �����ǰ� %d���� �ʰ��Ͽ� �ڵ� �����߽��ϴ�.(�����ο���)", Config.Login.MaximumOnlineUsers));
			ctx.close();
			return;
		}
		...
		if (!Config.shutdownCheck && ctx.channel().isActive()) {
			...
			if (nSize == -1) {
				print(host_address, port, String.format("��[Ȯ��]: %dȸ �̻��� ������ �õ��Ͽ� �����մϴ�.��", MJNetServerLoadManager.NETWORK_CLIENT_PERMISSION));
			...
			print("IP:" + host_address, port, String.format("(������) (�����ο�:%d/%d)", nSize, _clnts.size() + 1));
