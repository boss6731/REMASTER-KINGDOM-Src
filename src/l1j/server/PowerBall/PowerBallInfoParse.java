...
public class PowerBallInfoParse {
	...
	public boolean PowerBallInfo() {
		try {
			 ...
			 if (json == null) {
			 System.out.println("強力球信息重新加載(強力球信息)");
			 return false;
			 }
			 // 入口強力球期次 && 分配率信息(不實現分配率)
			 JSONObject infojson = readJsonFromUrl("http://ntry.com/data/json/games/dist.json");
			 if (infojson == null) {
			 System.out.println("強力球信息重新加載(期次信息)");
			 return false;
			 }
			 // 需要共享的部分可以創建一個類，讓控制器只訪問一次，這樣就不需要鎖。
	...
	public static JSONObject readJsonFromUrl(String url) throws IOException, JSONException {
		...
		try {
			...
			return json;
 			...
 			System.out.println("★在JSON中發生錯誤★");
 			}catch(UnknownHostException e) {
 			System.out.println("強力球：無法找到主機。");
		}catch(IOException e) {
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
