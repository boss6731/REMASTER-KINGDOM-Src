	...
	public static final ActionIntervalDecorator<L1PcInstance> PC_WALK_DECORATOR = new ActionIntervalDecorator<L1PcInstance>() {
		...
		public double decoration(L1PcInstance owner, double interval) {
			...
			switch (accelerate_check) {
				...
				case 3:
					...
					break;
				case 4:
					accelerate += MOVE_HASTE_ACCELERATION + MOVE_BRAVE_ACCELERATION + MOVE_THIRD_ACCELERATION + MOVE_FOUR_GEAR_ACCELERATION;
					break;
				default:
					break;
			}
			...
			if (owner.getMoveDelayRate() > 0) {
				...
				//complete_accelerate -= complete_accelerate * ((owner.getMoveDelayRate() / 100));
			}
		...
		public static final ActionIntervalDecorator<L1PcInstance> PC_ATT_DECORATOR = new ActionIntervalDecorator<L1PcInstance>() {
			...
			public double decoration(L1PcInstance owner, double interval) {
				...
				boolean dragon_pearl = false;
				if (owner != null) {
					if (owner.getWeapon() != null && (owner.getCurrentSpriteId() == 18551 || owner.getCurrentSpriteId() == 18555)) {
					...
					if (owner.hasSkillEffect(L1SkillId.STATUS_FRUIT)) {
						...
						accelerate_check++;
					else if (owner.hasSkillEffect(L1SkillId.MOVING_ACCELERATION) && owner.isPassive(MJPassiveID.MOVING_ACCELERATION_PASS.toInt()))
						accelerate_check++;
					else if (owner.hasSkillEffect(L1SkillId.HOLY_WALK) && owner.isPassive(MJPassiveID.HOLY_WALK_EVOLUTION.toInt()))
						accelerate_check++;
				...
				switch (accelerate_check) {
					...
					case 3:
						...
						break;
					case 4:
						accelerate += ATT_HASTE_ACCELERATION + ATT_BRAVE_ACCELERATION + ATT_THIRD_ACCELERATION + ATT_FOUR_GEAR_ACCELERATION;
						break;
					default:
						break;
				}
		...
		/**
		 * SP_HASTE_RETARDATION = 一段加速
		 * SP_BRAVE_RETARDATION = 二段加速
		 * SP_THIRD_RETARDATION = 三段加速
		 * SP_DRAGON_PEARL_RETARDATION = 單吃珍珠時
		 **/
			...
			public double decoration(L1PcInstance owner, double interval) {
				...
				switch (accelerate_check) {
					...
					case 3:
						...
						break;
					case 4:
						accelerate += SP_HASTE_ACCELERATION + SP_BRAVE_ACCELERATION + SP_THIRD_ACCELERATION + SP_FOUR_GEAR_ACCELERATION;
						break;
					default:
						break;
				}
