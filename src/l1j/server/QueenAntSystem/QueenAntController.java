					...
					case STATUS_START:
						if (remain_time == 0) {
							for (L1PcInstance pc : PcCK(true)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "모래 바람 사이로 여왕 개미 은신처가 보이기 시작합니다."));
							}
...
//					Time = 3010;// 에르자베 바로 소환(테스트용:주석바람)
						GeneralThreadPool.getInstance().schedule(this, 1000L);
						return null;
					case STATUS_1ST_EVENT:
						if (Time == 3000) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "지금, 여왕의 방이 열렸습니다."));
								pc.sendPackets("\\f3지금, 여왕의 방이 열렸습니다. 2분뒤 입구가 사라집니다.");
							}
							for (int i = 0; i < 10; i++) {
					...
					case STATUS_2ND_EVENT:
						...
						if (remain_time == 300) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "용사님...저를 구해주세요...빛을 따라서 제 방으로..."));
								pc.sendPackets("용사님...저를 구해주세요...빛을 따라서 제 방으로...");
							}
						}
						...
						if (remain_time == 240) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "당신만이 절 속박하는 구속을 풀어주실 수 있습니다.."));
								pc.sendPackets("당신만이 절 속박하는 구속을 풀어주실 수 있습니다..");
							}
						}
						if (remain_time == 180) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "빛나는 바닥돌이 저를 구할 수 있는 열쇠입니다.. 그곳으로..."));
								pc.sendPackets("빛나는 바닥돌이 저를 구할 수 있는 열쇠입니다.. 그곳으로...");
							}
						...
						if (remain_time == 300) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "좀 더 힘을 모아주세요... 제발..."));
								pc.sendPackets("좀 더 힘을 모아주세요... 제발...");
							}
						...
						if (remain_time == 240) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "請再多給我一些力量...拜託了..."));
								pc.sendPackets("請再多給我一些力量...拜託了...");
							}
						...
						if (remain_time == 60) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "啊啊..現在幾乎都完成了..."));
								pc.sendPackets("啊啊..現在幾乎都完成了...");
							}
						...
						if (remain_time < 0) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "..多虧了你，束縛我的枷鎖已經解開了..現在..."));
								pc.sendPackets("..多虧了你，束縛我的枷鎖已經解開了..現在...");
							}
					...
					case STATUS_3RD_EVENT:
						if (remain_time == 0) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "..是用餐時間了..!!"));
								pc.sendPackets("..是用餐時間了..!!");
							}
	...
	public void Start() {
		if (Running) {
			...
			for (L1PcInstance pc : PcCK(true)) {
				if (pc.isGm()) {
					pc.sendPackets("\f3[僅限管理員公告]: 蟻后的隱藏處系統正在關閉。1分鐘後請在活動警報表中增加1並重新加載設置。");
				}
			}
		} else {
			Time = 3600;
	...
	private void TimeCheck() {
		...
		if (_END == true) {
			try {
				for (L1PcInstance pc : PcCK(true)) {
					pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "蟻后的隱藏處正隨著沙塵暴消失。"));
				}
			...
			if (object instanceof L1MonsterInstance) {
				...
				if (npc == _boss_id) {
					if (mob != null && mob.isDead()) {
						if (remain_time == 0) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "已經擊敗蟻后。1分鐘後蟻后的房間將會倒塌。"));
								pc.sendPackets("已經擊敗蟻后。1分鐘後蟻后的房間將會倒塌。");
							}
						}
						if (remain_time == 30) {
							for (L1PcInstance pc : PcCK(false)) {
						...
						if (remain_time == 50) {
							for (L1PcInstance pc : PcCK(false)) {
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "蟻后的房間正在倒塌。為了安全，我們將返回駐地。"));
								pc.sendPackets("蟻后的房間正在倒塌。為了安全，我們將返回駐地。");
							}
						if (remain_time > 60) {
		...
		public void run() {
			...
			try {
				for (int width = 96; width >= 0; width -= _z_space) {
					...
					for (L1PcInstance pc : PcCK(false)) {
						if (!_END)
							pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "觀察到蟻后的意志。邊界已經變化。"));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
				...
				for (L1Object object : L1World.getInstance().getVisibleObjects(_mapid).values()) {
					if (object instanceof L1PcInstance) {
						...
						if (pc.getLocation().getTileLineDistance(new Point(effect.getLocation())) < 5 && !pc.isDead()) {
							if (pc != null) {
								pc.receiveDamage(effect, Config.ServerAdSetting.QueenAnt_Magnetic_Dmg);//磁場傷害
