...
public class InfinityBattle extends InfinityBattleInfo implements Runnable{
	...
	public void Start() {
		...
		if (!Config.Login.StandbyServer) {
			L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("可以進入激戰的競技場了。"));
			L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "可以進入激戰的競技場了。"));
		}
	}
				...
				switch (getMiniGameStatus()) {
					case REST:
						if (Remain_time == 60) {
							...
							L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "4分鐘後將開始激戰的競技場比賽。"));
						}
						if (Remain_time == 120) {
							L1World.getInstance().broadcastPacketToAll(new S_ServerMessage(7536));
							L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "3分鐘後將開始激戰的競技場比賽。"));
						}
						if (Remain_time == 180) {
							L1World.getInstance().broadcastPacketToAll(new S_ServerMessage(7537));
							L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "2分鐘後將開始激戰的競技場比賽。"));
						}
						if (Remain_time == 240) {
							L1World.getInstance().broadcastPacketToAll(new S_ServerMessage(7538));
							L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "1分鐘後將開始激戰的競技場比賽。"));

						}
						if (Remain_time == 290) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7494);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "10秒後將開始比賽。"));
							}
						}
						if (Remain_time == 300) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7495);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "第一回合開始~!"));
							}
					...
					case ROUND_1:
						...
						if (Remain_time == 590) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7498);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "10秒後將開始第二回合比賽。"));
							}
						}
						...
						if (Remain_time > 600) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7499);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "第二回合開始~!"));
							}
					...
					case ROUND_2:
						...
						if (Remain_time == 480) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7502);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "2分鐘後將開始最終回合。"));
							}
						}
						if (Remain_time > 600) {
							for (L1PcInstance pc : Members_pc(4)) {
								pc.sendPackets(7503);
								pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "最終戰開始！限制時間為10分鐘。"));
							}
	...
	public void chart_info_send() {
		...
		for (L1PcInstance pc : Members_pc(4)) {
			if (pc.getMapId() == INFINITI_BATTLE_MAPID) {
				SC_INFINITY_BATTLE_BOARD_INFO_NOTI.team_send(pc, (int)team1_percent, (int)team2_percent, (int)team3_percent, (int)team4_percent);
			}
		}
	}
	...
	private void TimeCheck() {
		if (Time == 300) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7504);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "5分鐘後將結束決戰的競技場。"));
			}
		}
		if (Time == 240) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7505);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "距離比賽結束還有4分鐘。"));
			}
		}
		if (Time == 180) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7506);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "距離比賽結束還有3分鐘。"));
			}
		}
		if (Time == 120) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7507);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "距離比賽結束還有2分鐘。"));
			}
		}
		if (Time == 60) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7508);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "距離比賽結束還有1分鐘。"));
			}
		}
		if (Time == 30) {
			for (L1PcInstance pc : Members_pc(4)) {
				pc.sendPackets(7509);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "30秒後比賽結束。"));
			}
		}
	...
	public void addTeamMembers(L1PcInstance pc) {
		...
		if (_1st_team_members.size() <= _2nd_team_members.size() && _1st_team_members.size() <= _3rd_team_members.size() && _1st_team_members.size() <= _4th_team_members.size()) {
			addFirstTeamMember(pc);
			MJPoint pt = MJPoint.newInstance(32680, 33273, 14, (short) INFINITI_BATTLE_MAPID, 50);
			pc.start_teleport(pt.x, pt.y, pt.mapId, pc.getHeading(), 18339, true);
			pc.setInfinityTeamId(1);
		} else if (_1st_team_members.size() > _2nd_team_members.size() && _2nd_team_members.size() <= _3rd_team_members.size() && _2nd_team_members.size() <= _4th_team_members.size()) {
			addSecondTeamMember(pc);
			MJPoint pt = MJPoint.newInstance(32580, 33380, 14, (short) INFINITI_BATTLE_MAPID, 50);
			pc.start_teleport(pt.x, pt.y, pt.mapId, pc.getHeading(), 18339, true);
			pc.setInfinityTeamId(2);
		} else if (_2nd_team_members.size() > _3rd_team_members.size() && _2nd_team_members.size() > _3rd_team_members.size() && _3rd_team_members.size() <= _4th_team_members.size()) {
							...
							if (_team1_popsition < 7) {
								if (_team1_popsition == 6) {
									if (_team1_Spwan_Remain_time == 1) {
										for (L1PcInstance pc : Members_pc(0)) {
											pc.sendPackets(7496);
											pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "怪物攻略完成！第一回合的BOSS將很快出現。"));
										}
									}
								}
								if (_team1_Spwan_Remain_time > 5) {
									_team1_popsition++;
									_team1_monster.addAll(InfinityBattleSpawn.getInstance().Spawn_List(INFINITI_BATTLE_MAPID, team, _team1_popsition));
									_team1_Spwan_Remain_time = 0;
								}
								...
								if (_door_open_count < _current_team_size - 1) {
									if (_team1_Spwan_Remain_time == 1) {
										for (L1PcInstance pc : Members_pc(0)) {
											pc.sendPackets(7497);
											pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "第一回合結束。需要移動到下一個區域。"));
										}
							...
							if (_team1_popsition < 10) {
								if (_team1_popsition == 9) {
									if (_team1_Spwan_Remain_time == 1) {
										for (L1PcInstance pc : Members_pc(0)) {
											pc.sendPackets(7500);
											pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "怪物攻略完成！第二回合的BOSS將很快出現。"));
										}
									}
								}
								if (_team1_Spwan_Remain_time > 5) {
								...
								if (_door_open_count < 1) {
									if (_team1_Spwan_Remain_time == 1) {
										for (L1PcInstance pc : Members_pc(0)) {
											pc.sendPackets(7501);
											pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "第二回合結束。需要移動到下一個區域。"));
										}
						...
						if (_team2_popsition >= 1 && _team2_popsition <= 7 && ready == Status.ROUND_1) {
							if (_team2_popsition < 7) {
								...
								if (_team2_Spwan_Remain_time > 5) {
									_team2_popsition++;
									_team2_monster.addAll(InfinityBattleSpawn.getInstance().Spawn_List(INFINITI_BATTLE_MAPID, team, _team2_popsition));
									_team2_Spwan_Remain_time = 0;
								}
						...
						if (_team3_popsition >= 1 && _team3_popsition <= 7 && ready == Status.ROUND_1) {
							if (_team3_popsition < 7) {
								...
								if (_team3_Spwan_Remain_time > 5) {
									_team3_popsition++;
									_team3_monster.addAll(InfinityBattleSpawn.getInstance().Spawn_List(INFINITI_BATTLE_MAPID, team, _team3_popsition));
									_team3_Spwan_Remain_time = 0;
								}
						...
						if (_team4_popsition >= 1 && _team4_popsition <= 7 && ready == Status.ROUND_1) {
							if (_team4_popsition < 7) {
								...
								if (_team4_Spwan_Remain_time > 5) {
									_team4_popsition++;
									_team4_monster.addAll(InfinityBattleSpawn.getInstance().Spawn_List(INFINITI_BATTLE_MAPID, team, _team4_popsition));
									_team4_Spwan_Remain_time = 0;
								}
