			...
			if ((_fsFlg & EventSystemTimeController.FS_START) > 0) {
				if (_EventInfo.get_event_name() != null) {
					...
					L1World.getInstance().broadcastPacketToAll(
							new S_PacketBox(S_PacketBox.GREEN_MESSAGE, String.format("\f2稍後 %s 活動即將開始。", _EventInfo.get_event_name())), false
					);
					L1World.getInstance().broadcastPacketToAll(
							new S_SystemMessage(String.format("\f2稍後 %s 活動即將開始。", _EventInfo.get_event_name())), false
					);
				} else {
					EventSystemTimeController.getInstance().setFlag(_EventInfo);
				}
				...
				EventSystemTimeController.getInstance().delFlag(_EventInfo);
					L1World.getInstance().broadcastPacketToAll(
							new S_PacketBox(S_PacketBox.GREEN_MESSAGE, String.format("\f3稍後 %s 活動即將結束。", _EventInfo.get_event_name())), false
					);
					L1World.getInstance().broadcastPacketToAll(
							new S_SystemMessage(String.format("\f3稍後 %s 活動即將結束。", _EventInfo.get_event_name())), false
					);
