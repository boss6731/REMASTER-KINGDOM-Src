...
public class EventSystemInfo {
	...
	public int calc_probability(int i, L1PcInstance pc) {
		...
		int probability = 0;
		String[] drop_min_chance = (String[]) MJArrangeParser.parsing(get_drop_min_chance(), ",", MJArrangeParseeFactory.createStringArrange()).result();
		String[] drop_max_chance = (String[]) MJArrangeParser.parsing(get_drop_max_chance(), ",", MJArrangeParseeFactory.createStringArrange()).result();

		if (drop_min_chance[i] == drop_max_chance[i])
			probability = Integer.parseInt(drop_max_chance[i]);
		else
			probability = random.nextInt(Integer.parseInt(drop_max_chance[i]) - Integer.parseInt(drop_min_chance[i]) + Integer.parseInt(drop_max_chance[i]));

		return probability;
	}
	
	public int calc_count(int i, L1PcInstance pc) {
		String[] drop_min_count = (String[]) MJArrangeParser.parsing(get_min_count(), ",", MJArrangeParseeFactory.createStringArrange()).result();
		String[] drop_max_count = (String[]) MJArrangeParser.parsing(get_max_count(), ",", MJArrangeParseeFactory.createStringArrange()).result();
		int count 	= 0;
