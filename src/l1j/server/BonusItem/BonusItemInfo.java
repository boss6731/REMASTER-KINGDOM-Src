...
public class BonusItemInfo {
	...
	public int calc_probability(L1PcInstance pc) {
		...
		if (_is_loggin) {
			if (pc.isGm()) {
				System.out.println(String.format("獎勵物品: +%d %s : 掉落物品:%s, 掉落機率:(%d)", _item_enchant, _item_name, _bonus_name, probability));
			}
		}
	...
	public int calc_count(L1PcInstance pc) {
		...
		if (_is_loggin) {
			if (pc.isGm()) {
				System.out.println(String.format("獎勵物品: +%d %s : 掉落物品:%s, 掉落數量:(%d)", _item_enchant, _item_name, _bonus_name, count));
			}
		}
		return count;
	}
	...
	public boolean isMap(L1PcInstance pc) {
		String[] bonusmap = (String[])MJArrangeParser.parsing(get_mapids(), ",", MJArrangeParseeFactory.createStringArrange()).result();
