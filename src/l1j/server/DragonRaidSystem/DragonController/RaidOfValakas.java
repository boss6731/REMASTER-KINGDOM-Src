...
public class RaidOfValakas implements Runnable {
	DragonRaidSystemInfo dinfo = DragonRaidSystemLoader.find(4);
	...
	private int teleporter_id = 73201224;
			...
			try {
				...
				switch (stage) {
					case EVENT:
						...
						TownEffectCheck();
						if (CurrentTime == 30) {
							spawn(dinfo.getMapX(), dinfo.getMapY(), (short) dinfo.getMapId(), 0, dinfo.getDragonId(), ActionCodes.ACTION_Appear, 3600 * 1000);
							stage = 2;
							CurrentTime = 0;
						}
					case END:
	...
	public void Start() {
		reset();
		L1World.getInstance().broadcastPacketToAll(SC_NOTIFICATION_MESSAGE_NOT.CenterMessage("在某處感受到龍的力量。", 5), true);
		spawn(33727, 32244, (short) 15440, 0, teleporter_id, ActionCodes.ACTION_Appear, 3600 * 1000);
	...
	private void TimeCheck() {
		...
		if (Time == 300) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "5分鐘後突襲將結束。", 5);
			}
		}
		if (Time == 180) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "3分鐘後突襲將結束。", 5);
			}
		}
		if (Time == 60) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "1分鐘後突襲將結束。", 5);
			}
		}
		if (Time == 10) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "10秒後突襲將結束。", 5);
			}
		}
		if (Time == 5) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "5秒後突襲將結束。", 5);
			}
		}
	...
	private void TownEffectCheck() {
		...
		if (Town_Effect == 0)
			spawn(33719+rnd.nextInt(5), 32493+rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 5)
			spawn(33707-rnd.nextInt(5), 32496-rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 10)
			spawn(33716+rnd.nextInt(5), 32502+rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 15)
			spawn(33709-rnd.nextInt(5), 32505-rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 20)
			spawn(33707+rnd.nextInt(5), 32496+rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 25)
			spawn(33719-rnd.nextInt(5), 32493-rnd.nextInt(5), (short) 4, 0, 73201232, ActionCodes.ACTION_Appear, 4500);
	...
	private static void spawn(int x, int y, short MapId, int Heading, int npcId, int actioncode, int timeMillisToDelete) {
		try {
