...
public class RaidOfAntaras implements Runnable {
	...
	public void Start() {
		reset();
		L1World.getInstance().broadcastPacketToAll(SC_NOTIFICATION_MESSAGE_NOT.CenterMessage("我在某處感受到了龍的力量.", 5), true);
	...
	private void TimeCheck() {
		...
		if (Time == 10) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "突襲10秒後結束.", 5);
			}
		}
		if (Time == 5) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "突襲5秒後結束.", 5);
			}
		}
