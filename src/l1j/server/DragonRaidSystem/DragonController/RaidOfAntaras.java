...
public class RaidOfAntaras implements Runnable {
	DragonRaidSystemInfo dinfo = DragonRaidSystemLoader.find(1);
	...
	private int teleporter_id = 73201221;
	...
	public void Start() {
		...
		spawn(32732, 32802, (short) 37, 0, teleporter_id, ActionCodes.ACTION_Appear, 3600 * 1000);
	...
	private void TimeCheck() {
		...
		if (Time == 10) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "10秒鐘後突襲將結束。", 5);
			}
		}
		if (Time == 5) {
			for (L1PcInstance pc : PcCK()) {
				SC_NOTIFICATION_MESSAGE_NOT.CenterMessage(pc, "5秒鐘後突襲將結束。", 5);
			}
		}
	...
	private void TownEffectCheck() {
		if (Town_Effect == 0)
			spawn(33430, 32807, (short) 4, 0, 73201225, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 10)
			spawn(33436, 32813, (short) 4, 0, 73201225, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 20)
			spawn(33430, 32819, (short) 4, 0, 73201225, ActionCodes.ACTION_Appear, 4500);
		if (Town_Effect == 30) {
			spawn(33424, 32813, (short) 4, 0, 73201225, ActionCodes.ACTION_Appear, 4500);
			Town_Effect = 0;
		}
		Town_Effect++;
	}
