		...
		for (int i = 1; i <= 1; i++) {
			...
			if (ub.checkUbTime() && !ub.isActive()) {
				if (L1Colosseum.Info.colouse) {
					...
					L1World.getInstance().broadcastPacketToAll(SC_NOTIFICATION_MESSAGE.make_stream("\f3稍後，奇岩村將開始競技場活動。", MJSimpleRgb.green(), 1), true);
					L1World.getInstance().broadcastServerMessage("\f3稍後，奇岩村將開始競技場活動。");
				}
				_list.put(i, ub);
				ub.start();
			}
