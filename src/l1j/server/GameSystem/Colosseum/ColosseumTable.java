...
public class ColosseumTable {
	...
	private void loadTable() {
		...
		try {
			...
			System.out.println("ubsettings couldnt be initialized:" + e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		...
		try {
			...
			System.out.println("ub_managers couldnt be initialized:" + e);
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		...
		try {
			...
			System.out.println("ub_times couldnt be initialized:" + e);
		} finally {
			SQLUtil.close(rs, pstm, con);
		}
		_log.config("UB列表 " + _ub.size() + " 條已載入");
	}
