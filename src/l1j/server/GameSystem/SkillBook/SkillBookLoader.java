...
//						skills.put(sInfo.getSkillId(), sInfo);
	...
	private static final String[] CLASS_NAMES = new String[] {
		"王族", "騎士", "妖精", "法師", "黑暗妖精", "龍騎士", "幻術師", "戰士", "劍士", "黃金槍騎"
	};
	...
	public boolean masterSkill(L1PcInstance pc, SkillBookInfo sInfo, boolean temp) {
		...
		if (sInfo.isNormalMagic()) {
			switch (pc.getClassNumber()) {
				case 0, 4, 8, 5, 9 -> level = level / 8 * 15;
				case 1, 7 -> level = level / 8 * 50;
				case 2 -> level = level / 8 * 10;
				default -> {
				}
			}
			...
			if (pc.getClassNumber() != sInfo.getClassType()) {
				pc.sendPackets(String.format("該技能只有 %s 可以學習。", CLASS_NAMES[sInfo.getClassType()]));
				return false;
			}
		}
		...
		if (temp) {
			...
			SC_AVAILABLE_SPELL_NOTI not = SC_AVAILABLE_SPELL_NOTI.newInstance();
			not.appendNewSpell(sInfo.getSkillId(), true);
			pc.sendPackets(not, MJEProtoMessages.SC_AVAILABLE_SPELL_NOTI, true);
	...
	private void checkSkillStage(L1PcInstance pc) {
		...
		int skillStage = switch (classType) {
			...
			case 9 -> // 黃金槍騎
					2;
			default -> throw new IllegalArgumentException(String.format("invalid class type: %d", classType));
		};
	}
