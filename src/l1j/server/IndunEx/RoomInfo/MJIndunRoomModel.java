...
/**
 * 地牢房間資訊的模型
 * 地牢房間流程 {@link MJIndunRoomController} -> MJIndunRoomModel -> {@link MJIndunRoomView}
 * MJIndunRoomController 僅通知模型過濾後的房間變更數據。
 * MJIndunRoomModel 僅接收正常流程的控制通知並通告給視圖。
 * MJIndunRoomView 僅實現以廣播為主的純視圖功能。
 * @author mjsoft
 */
...
//		rInfo.set_member_count_min(MJIndunRoomController.MIN_PLAYER);
	...
	void onClear() {
		//System.out.println("MJIndunRoomModel::onClear");
	...
	void onKick(L1PcInstance kicker){
		...
		if(member_model != null && members.size() > 0){
			...
			kicker.sendPackets("您已被踢出，請加入其他房間。");
			onexit(kicker);
		}
		MJIndunRoomView.notifyRoomInfoChanged(this);
		doSafetyMove(kicker);
		...
		if(member_model != null) {
			...
			if (check) {
				for(MJIndunRoomMemberModel members : get_members()) {
					members.member.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "房主已離開，請加入其他房間。"));
					members.member.sendPackets("房主已離開，請加入其他房間。");
...
//		return IntRange.includes(max_player, MJIndunRoomController.MIN_PLAYER, MJIndunRoomController.MAX_PLAYER);
	}
	...
	boolean check_kind(){
		return map_kind.equals(eArenaMapKind.OrimLab_Minor) || map_kind.equals(eArenaMapKind.OrimLab_Normal)|| map_kind.equals(eArenaMapKind.CROCODILE) || map_kind.equals(eArenaMapKind.FANTASY)|| map_kind.equals(eArenaMapKind.AURAKIA)/*|| map_kind.equals(eArenaMapKind._INSTANCE_DUNGEON_END)*/;
	}
