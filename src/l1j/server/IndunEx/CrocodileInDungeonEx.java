...
public class CrocodileInDungeonEx implements Runnable {
	...
	private static final int BOSS4_POINT = 80; // 召喚夢幻魅魔所需點數
	...
	public CrocodileInDungeonEx(int mapid, int price, MJIndunRoomModel model) {
		...
		for (MJIndunRoomMemberModel member_model : model.get_members()) {
			...
			for (int i = 420100; i <= 420111; i++){
				...
				member_model.member.getInventory().consumeItem(i); // 奧拉奇亞的箭矢刪除
			}
		...
		Gerang = L1SpawnUtil.spawn4(33385, 33192, (short) mapid, 4, 7800100, 0, 0, 0);//게렝
...
//				System.out.println("step: "+step+"sub_step2: "+sub_step2);
					...
					if (sub_step2 == 0){
						...
						if (boss_1 != null && boss_1.isDead()) {
							int number = CommonUtil.random(33);
							current_point += number + 1;
							GREEN_MSG("去找東邊盡頭的燈塔吧。通過那裡可以到達地下水道。");
							GREEN_MSG("水道的盡頭與鱷魚島相連。");
							GREEN_MSG("如果你不打算停下腳步，我不會刻意阻攔你...但要小心。");
							sub_step2 = 3;
						}
					} else if(sub_step2 == 3){
...
//					System.out.println(boss_2.isDead()+"+"+boss_2);
						if (boss_2 != null && boss_2.isDead()) {
							...
							GREEN_MSG("數十年間，我們甚至都不願接近的地方，你竟然打算前往。");
							GREEN_MSG("這裡是被封印的怪物鱷魚王羅瑟斯所在之地。");
							GREEN_MSG("願神的祝福與你同在...");
							sub_step3 = 1;
						}
					} else if (sub_step3 == 1){
...
//					System.out.println(boss_3.isDead()+"+"+boss_3);
				...
				case 5: //END
					if (sub_step5 == 0){
						...
						QUEST_MSG(surf, dialogs);
						GREEN_MSG("辛苦了。多虧了你，暫時會和平一段時間。");
						GREEN_MSG("現在，請回到你的世界去吧。");
						sub_step5 = 1;
						GeneralThreadPool.getInstance().schedule(this, 1000);
						return new L1PcInstance[0];
					} else if (sub_step5 == 1){
				...
				case 7: // 時間結束
					if (sub_step7 == 0) {
						...
						GREEN_MSG("糟了，你在這裡停留的時間已經結束了。");
						sub_step7 = 2;
					} else if (sub_step7 == 2) {
						GREEN_MSG("很遺憾，但你必須回到你原來的地方。");
						sub_step7 = 3;
					} else if (sub_step7 == 3) {
						quit();
					}
				...
				case 8: // 當全員死亡時
					if (sub_step8 == 0) {
						...
						GREEN_MSG("唉... 因為我的貪念，竟然連你也失去了。");
						sub_step8 = 2;
					} else if (sub_step8 == 2) {
						GREEN_MSG("回到你的世界，安息吧。");
						sub_step8 = 3;
					} else if (sub_step8 == 3) {
						quit();
					}
