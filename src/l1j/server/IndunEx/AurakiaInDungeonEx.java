	...
	public AurakiaInDungeonEx(int mapid, int price, MJIndunRoomModel model) {
		...
		for (MJIndunRoomMemberModel member_model : model.get_members()) {
			...
			for (int i = 420100; i <= 420111; i++){
				...
				member_model.member.getInventory().consumeItem(i); // 刪除阿烏拉基亞的箭
			}

			if (member_model.member.getInventory().checkItem(420122)){
				member_model.member.getInventory().consumeItem(420122); // 刪除阿烏拉基亞的箭
			}
...
//				home_teleport();
	...
	public void run() {
		...
		try {
			...
			switch (step) {
			case 0:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step);
				if (sub_step == 0){
					sub_step = 1;
				} else if (sub_step == 1) {
...
//					eyeGate.deleteMe();
				...
				return new L1PcInstance[0];
			case 1:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step1);
//				System.out.println(spawnCnt+"+"+firstMonsterList);
				if (sub_step1 == 0){
					...
					GREEN_MSG("黑暗監視者，處理那些傢伙");
//                    System.out.println("召喚監視者");
					darkWatch = (L1MonsterInstance)L1SpawnUtil.spawnnpc(32851, 32826, (short) _mapnum, 7800305, 0, 0, 0);
					for (L1PcInstance pc : getPlayMemberArray()){
						pc.sendPackets(SC_SCENE_NOTI.make_stream(true, System.currentTimeMillis(), 73600000, darkWatch.getX(), darkWatch.getY()), true);
					}
...
//					System.out.println(darkWatch+"+"+darkWatch.isDead());
				...
				return new L1PcInstance[0];
			case 2:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step2);
				if (sub_step2 == 0){
					spawnCnt = 0;
					sub_step2 = 1;
				} else if (sub_step2 == 1){
					GREEN_MSG("讓你見識黑暗監視者的真面目");
					sub_step2 = 2;
				} else if (sub_step2 == 2){
					darkTransWatch = (L1MonsterInstance)L1SpawnUtil.spawnnpc(32851, 32826, (short) _mapnum, 7800306, 0, 0, 0);
					for (L1PcInstance pc : getPlayMemberArray()){
						pc.sendPackets(SC_SCENE_NOTI.make_stream(true, System.currentTimeMillis(), 73600000, darkTransWatch.getX(), darkTransWatch.getY()), true);
					}
					GREEN_MSG("沒想到你會看到我的真面目");
					sub_step2 = 3;
				} else if (sub_step2 ==3){
				...
				return new L1PcInstance[0];
			case 3:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step3);
				if(sub_step3 == 0){
					stoneActive(true);
					arrestedAurakia = (L1MonsterInstance)L1SpawnUtil.spawnnpc(32851, 32826, (short) _mapnum, 7800300, 0, 0, 0);
					for (L1PcInstance pc : getPlayMemberArray()){
						pc.sendPackets(SC_SCENE_NOTI.make_stream(true, System.currentTimeMillis(), 73600004, arrestedAurakia.getX() + 5, arrestedAurakia.getY() - 5), true);
					}
					arrestedAttackCnt = 0;
					step = 4;
				}
				...
				return new L1PcInstance[0];
			case 4:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step4);
				if (sub_step4 == 0){
					if (stone_1.getOpenStatus() == ActionCodes.ACTION_Close && stone_2.getOpenStatus() == ActionCodes.ACTION_Close
						&& stone_3.getOpenStatus() == ActionCodes.ACTION_Close && stone_4.getOpenStatus() == ActionCodes.ACTION_Close){
						stoneActive(false);
						sub_step4 = 1;
					}
				}
					...
					for (L1PcInstance pc: getPlayMemberArray()){
						pc.getInventory().storeItem(420122, 1, 0, deltime); //副本剩餘時間後刪除 (應對斷線)
					}
					GREEN_MSG("在背包中生成奧拉奇亞的箭矢。");
					GREEN_MSG("請用奧拉奇亞的箭矢解除奧拉奇亞的束縛。");
					sub_step4 = 2;
				...
				return new L1PcInstance[0];
			case 5:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step5);
				if (sub_step5 == 0) {
					sub_step5 = 1;
					GeneralThreadPool.getInstance().schedule(this, 5000);
					return new L1PcInstance[0];
				} else if (sub_step5 == 1){
					GREEN_MSG("不行...奧拉奇亞的束縛被解除了...");
					sub_step5 = 2;
				...
				break;
			case 6:
//                System.out.println("階段: " + step + " 額外階段: " + sub_step6);
				if (sub_step6 == 0 ){
					GREEN_MSG("10秒後將移動到村莊。");
					sub_step6 =1;
					GeneralThreadPool.getInstance().schedule(this, 5000);
	...
	private void circleCheck(){
		...
		for (L1PcInstance pc : getPlayMemberArray()) {
			if (pc.getX() == circle_1.getX() && pc.getY() == circle_1.getY()) {
				...
				pc.setAurakiaCircle(true);
			} else pc.setAurakiaCircle(pc.getX() == circle_5.getX() && pc.getY() == circle_5.getY());
		}
	}
	...
	private void arrestedAction(int scriptId){
		if(scriptId > 0){
			arrestedLevel++;
			for (L1PcInstance pc : getPlayMemberArray()){
				pc.sendPackets(SC_SCENE_NOTI.make_stream(true, System.currentTimeMillis(), scriptId, arrestedAurakia.getX() + 5, arrestedAurakia.getY() - 5), true);// 구속구 이미지 출력	
			}
	...
	private <FastTable> void monsterGateAction(){
		FastTable monList;
		monList = null;
//		System.out.println(gatecnt+"+"+gate1Active+"+"+gate2Active+"+"+gate3Active+"+"+gate4Active);
	
		if (gate1Active || gate2Active || gate3Active || gate4Active){
			if (gate1Active){// 1시
		...
		if(step == 1 && firstMonsterList != null){
			firstMonsterList.addAll((Collection<? extends L1NpcInstance>) monList);
		}
		if (gatecnt >= 4){
			if (gatecnt % 4 == 0){
				spawnCnt++;
...
//			System.out.println(gateNumber);
			switch(gateNumber){
			case 0:
					monsterGate_1 = (L1NpcInstance) L1SpawnUtil.spawnnpc(32863, 32829, (short) _mapnum, 7800308, 0, 10 * 1000, 0);// 몬스터 게이트 1시
					gate1Active = true;
				break;
			case 1:
					monsterGate_2 = (L1NpcInstance) L1SpawnUtil.spawnnpc(32848, 32840, (short) _mapnum, 7800309, 0, 10 * 1000, 0);// 몬스터 게이트 4시
					gate2Active = true;
				break;
			case 2:
					monsterGate_3 = (L1NpcInstance) L1SpawnUtil.spawnnpc(32837, 32829, (short) _mapnum, 7800310, 0, 10 * 1000, 0);// 몬스터 게이트 7시
					gate3Active = true;
				break;
			case 3:
					monsterGate_4 = (L1NpcInstance) L1SpawnUtil.spawnnpc(32848, 32814, (short) _mapnum, 7800311, 0, 10 * 1000, 0);// 몬스터 게이트 11시
					gate4Active = true;
				break;
			default:
				break;
	...
	private void spawn_door(){
		...
		eyeGate			= (L1DoorInstance) L1SpawnUtil.spawnnpc(32812, 32830, (short) _mapnum, 7800315, 0, 0, 0);// 눈알 게이트
		monsterGate_1	= (L1NpcInstance) L1SpawnUtil.spawnnpc(32863, 32829, (short) _mapnum, 7800308, 0, time * 1000, 0);// 몬스터 게이트 1시
		gate1Active = true;
		monsterGate_2	= (L1NpcInstance) L1SpawnUtil.spawnnpc(32848, 32840, (short) _mapnum, 7800309, 0, time * 1000, 0);// 몬스터 게이트 4시
		gate2Active = true;
		monsterGate_3	= (L1NpcInstance) L1SpawnUtil.spawnnpc(32837, 32829, (short) _mapnum, 7800310, 0, time * 1000, 0);// 몬스터 게이트 7시
		gate3Active = true;
		monsterGate_4	= (L1NpcInstance) L1SpawnUtil.spawnnpc(32848, 32814, (short) _mapnum, 7800311, 0, time * 1000, 0);// 몬스터 게이트 11시
		gate4Active = true;
		stone_1			= (L1DoorInstance) L1SpawnUtil.spawnnpc(32862, 32817, (short) _mapnum, 7800316, 0, 0, 0);//석상 12시
		stone_2			= (L1DoorInstance) L1SpawnUtil.spawnnpc(32859, 32835, (short) _mapnum, 7800317, 0, 0, 0);//석상 3시
		stone_3			= (L1DoorInstance) L1SpawnUtil.spawnnpc(32840, 32837, (short) _mapnum, 7800318, 0, 0, 0);//석상 6시
		stone_4			= (L1DoorInstance) L1SpawnUtil.spawnnpc(32842, 32817, (short) _mapnum, 7800319, 0, 0, 0);//석상 9시 
	}
	...
	private void arrestedDestroy() {
		for (L1PcInstance pc : getPlayMemberArray()) {
			...
			// pc.sendPackets(S_GfxMessage.AURAKIA_END_MENT);// 文字
			pc.getInventory().consumeItem(420122); // 奧拉奇亞箭矢移除
			if(pc.isDead()) {
				continue; // 獎勵排除
			}
			pc.getInventory().storeItem(1000007, 1); // 龍之高級鑽石 1個
			// pc.einGetExcute(300);// 艾因哈薩德獎勵
			pc.getInventory().storeItem(420120, 1); // 奧拉奇亞的禮物1個
		}
	...
	public static void useArrow(L1PcInstance pc, L1MonsterInstance target) {
		...
		if (time1 - oldtime <= 2000) { // 再使用时间 2秒
			return;
		}
		if (target == null) {
			return;
		...
		Broadcaster.broadcastPacket(pc, (new S_UseAttackSkill(pc, target.getId(), 20336, target.getX(), target.getY(), 0, false)));
