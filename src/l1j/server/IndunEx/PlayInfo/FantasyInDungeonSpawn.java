...
public class FantasyInDungeonSpawn {
	private static FantasyInDungeonSpawn _instance;

	public static FantasyInDungeonSpawn getInstance() {
		if (_instance == null) {
			_instance = new FantasyInDungeonSpawn();
		}
		return _instance;
	}

	private FantasyInDungeonSpawn() {
	}
	...
	public FastTable<L1NpcInstance> fillSpawnTable(int mapid, int type) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawnlist_fantasy_indun");
