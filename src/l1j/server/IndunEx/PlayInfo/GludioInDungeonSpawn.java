...
public class GludioInDungeonSpawn {
	private static GludioInDungeonSpawn _instance;

	public static GludioInDungeonSpawn getInstance() {
		if (_instance == null) {
			_instance = new GludioInDungeonSpawn();
		}
		return _instance;
	}

	private GludioInDungeonSpawn() {
	}
	...
	public FastTable<L1NpcInstance> fillSpawnTable(int mapid, int type) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawnlist_gludio_indun");
