...
public class CrocodileInDungeonSpawn {
	private static CrocodileInDungeonSpawn _instance;

	public static CrocodileInDungeonSpawn getInstance() {
		if (_instance == null) {
			_instance = new CrocodileInDungeonSpawn();
		}
		return _instance;
	}

	private CrocodileInDungeonSpawn() {
	}
	...
	public FastTable<L1NpcInstance> fillSpawnTable(int mapid, int type) {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM spawnlist_crocodile_indun");
