	...
	public GludioInDungeonEx(int mapid, int price, MJIndunRoomModel model, boolean floor) {
		...
		for (MJIndunRoomMemberModel member_model : model.get_members()) {
			...
			for (int i = 420100; i <= 420111; i++){
				...
				member_model.member.getInventory().consumeItem(i);// 刪除奧拉基亞之箭
			}
			...
			switch (step) {
				case 0:
					if (sub_step == 0) {
						...
						GREEN_MSG("時間超過或水晶球被破壞則任務失敗。");
					} else if (sub_step == 4) {
						startAllBuffThread(); // 加入增益
						GREEN_MSG("奧林給予你一點力量的幫助。");
					} else if (sub_step == 5) {
						GREEN_MSG("時間超過或水晶球被破壞則任務失敗。");
					} else if (sub_step == 6) {
						time = gametime;
