...
public class NpcActionListener implements ActionListener{
	...
	public final String to_action(L1PcInstance pc, L1Object target){
		...
		try {
			...
			if (isNeedPcBuff()) {
				if (pc.getAccount().getBuffPcRoom() != null) {
					if (pc.getAccount().getBuffPcRoom().getTime() / 1000 < sysTime) return resultNoPcBuff(pc);
				} else {
			...
			if(need_item_id > 0 && need_item_amount > 0 && !pc.getInventory().consumeItem(need_item_id, need_item_amount))
				...
				if (item == null) {
					// 如果是支配之塔NPC，檢查是否有幻象護符。
					if (m_tower_npcs.contains(_npc_id)) {
						if (!pc.getInventory().containsAtOne(new int[] { 560028, 4100135 })) {
							return resultShortItem(pc); // 返回 result_short_item(pc);
						}
						// 一般NPC處理
					} else {
						return resultShortItem(pc); // 返回 result_short_item(pc);
	...
	private String resultShortItem(L1PcInstance pc) {
		return null;
	}

	private String resultNoPcBuff(L1PcInstance pc) {
		return null;
	}

	private boolean isNeedPcBuff() {
		return false;
	}
