...
public class TeleporterActionListener extends NpcActionListener{
	...
	public String result_un_opened(L1PcInstance pc) {
		//        pc.sendPackets(String.format("%s目前尚未開放。", get_destination_name()));
		// 無法進入: 目前尚未開放。
		pc.sendPackets(1229);
	...
	public String result_short_level(L1PcInstance pc) {
		//        pc.sendPackets(String.format("%s需達到 %d 級才能進入。", get_destination_name(), get_need_level()));
		// 無法進入: 等級不足
		pc.sendPackets(5359);
		pc.sendPackets(SC_NOTIFICATION_MESSAGE.make_stream("入場最低等級不符。", MJSimpleRgb.green(), 5));
	...
	public String result_limit_level(L1PcInstance pc) {
		...
		// 無法進入: 等級不足
		pc.sendPackets(5359);
		pc.sendPackets(SC_NOTIFICATION_MESSAGE.make_stream("入場最高等級不符。", MJSimpleRgb.green(), 5));
	...
	public String result_short_item(L1PcInstance pc){
		...
		if(needAmount > 0)
			pc.sendPackets(new S_ServerMessage(5359));
//            pc.sendPackets(String.format("%s %d 個不足。", item.getName(), get_need_item_amount()));
		else
			pc.sendPackets(String.format("%s不存在。", item.getName()));
	...
	public String result_success(L1PcInstance pc){
		...
		switch(get_npc_id()) {
			case 8502072:
			SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.do_send(pc, pt.x, pt.y, pt.mapId, SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_INTER_TEST);
			break;
		case 70086:
			SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.do_send(pc, pt.x, pt.y, pt.mapId, SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_FISLAND);
			break;
...
//			System.out.println(pt.x+"+"+pt.y+"+"+pt.mapId);
			SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.do_send(pc, pt.x, pt.y, pt.mapId, SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_INTER_MAGIC_DOLL_RACE);
			break;
		case 8502038:
			SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.do_send(pc, pt.x, pt.y, pt.mapId, SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_INTER_BACK);
			break;
		default:
//			SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.do_send(pc, pt.x, pt.y, pt.mapId, SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_INTER_BACK_RESTART);
			pc.start_teleport(pt.x, pt.y, pt.mapId, pc.getHeading(), 18339, true, false);
			break;
		}
