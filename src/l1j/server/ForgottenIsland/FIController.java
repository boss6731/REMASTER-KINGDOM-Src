	...
	public void NightSpawn() {
		...
		if (Info.get_type() == 1) {
			for (L1PcInstance pc : L1World.getInstance().getVisiblePlayers(Info.get_map_id())) {
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "從某處感受到陰森的氣息。"));
				
				if (pc.isDead())
					continue;
				
				String[] npcids = (String[]) MJArrangeParser.parsing(Info.get_npc_id(), ",", MJArrangeParseeFactory.createStringArrange()).result();
				String[] count = (String[]) MJArrangeParser.parsing(Info.get_count(), ",", MJArrangeParseeFactory.createStringArrange()).result();
				int size = 0;
			...
			for (L1PcInstance pc : L1World.getInstance().getVisiblePlayers(Info.get_map_id())) {
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "某處傳來陰森的氣息。"));
			}
			...
			for (String ids : npcids) {
				...
				for (int i = 0; i < npc_count; i++) {
					...
					if (l1npc != null) {
						try {
							int mapid = Info.get_map_id();
							int x = MapsTable.getInstance().getStartX(mapid) + ((MapsTable.getInstance().getEndX(mapid) - MapsTable.getInstance().getStartX(mapid)) / 2);
							int y = MapsTable.getInstance().getStartY(mapid) + ((MapsTable.getInstance().getEndY(mapid) - MapsTable.getInstance().getStartY(mapid)) / 2);
	...
	public void cloud_spawn(){
		...
		case 23:
			for (L1PcInstance pc : L1World.getInstance().getVisiblePlayers(pInfo.get_map_id())) {
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "島上的某處感受到痛苦的氣息。"));
			}
			...
			try {
				...
				for (int width = _range; width >= 0; width -= _z_space) {
					...
					for (int i = 0; i < width * 4; i += _x_space) {
						...
						for (int j = 0; j < 4; j++) {
							L1EffectInstance effect = L1EffectSpawn.getInstance().spawnEffect(8503185, remain_time, cpos[j][0] + 5, cpos[j][1] - 5, (short) _map_id);
							effect_list.add(effect);
						}
					}
				}
				
				L1EffectInstance position1 = L1EffectSpawn.getInstance().spawnEffect(8503185, remain_time, _x - 4, _y + 4, (short) _map_id);
				effect_list.add(position1);
				L1EffectInstance position2 = L1EffectSpawn.getInstance().spawnEffect(8503185, remain_time, _x + 4, _y - 4, (short) _map_id);
				effect_list.add(position2);
				L1EffectInstance position3 = L1EffectSpawn.getInstance().spawnEffect(8503185, remain_time, _x + 4, _y + 4, (short) _map_id);
				effect_list.add(position3);
				L1EffectInstance position4 = L1EffectSpawn.getInstance().spawnEffect(8503185, remain_time, _x - 4, _y - 4, (short) _map_id);
				...
				for (L1Object object : L1World.getInstance().getVisibleObjects(_mapid).values()) {
					if (object instanceof L1PcInstance) {
						...
						if (pc.getLocation().getTileLineDistance(new Point(effect.getLocation())) < 5 && !pc.isDead()) {
							if (pc != null) {
								pc.receiveDamage(effect, Config.ServerAdSetting.CLOUD_POISON_DAMAGE / 2);//磁場傷害
