...
public class BQSLoadManager implements MJCommand{
	...
	private MJCommandTree createCommand(){
		return new MJCommandTree(".圖鑑系統", "執行怪物圖鑑/每週任務相關命令。", null)
	...
	private MJCommandTree createCriteriaCommand(){
		return new MJCommandTree("圖鑑", "執行與怪物圖鑑相關的命令。", null)
				.add_command(new MJCommandTree("重新載入", "重新載入怪物圖鑑內的各種數據。", null)
						.add_command(new MJCommandTree("獎勵", "", null){
							...
							protected void to_handle_command(MJCommandArgs args) throws Exception{
								...
								args.notify("已重新載入 tb_mbook_reward_info/tb_mbook_reward_items 表。");
							}
						})
						.add_command(new MJCommandTree("圖鑑信息", "", null){
							...
							protected void to_handle_command(MJCommandArgs args) throws Exception{
								...
								args.notify("已重新載入 tb_mbook_information 表。");
							}
						})
				)
				.add_command(new MJCommandTree("角色初始化", "根據傳來的角色名稱初始化該角色的怪物圖鑑。", new String[]{"角色名稱"}) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						...
						GeneralThreadPool.getInstance().execute(new Runnable(){
							...
							public void run(){
								try{
									...
									pc.sendPackets("圖鑑信息已由管理員初始化。怪物已更新，但為了獲得獎勵，請重新登錄。");
									gm.sendPackets(String.format("已初始化 %s 的怪物圖鑑信息。", pc.getName()));
								}catch(Exception e){
									e.printStackTrace();
								}
								return null;
				...
				.add_command(new MJCommandTree("全部初始化", "初始化當前所有在線角色的圖鑑。", null) {
					protected void to_handle_command(MJCommandArgs args) throws Exception{
						L1PcInstance gm = args.getOwner();
						GeneralThreadPool.getInstance().execute(new Runnable(){
							@Override
							public void run(){
								...
								S_SystemMessage message = new S_SystemMessage("怪物圖鑑信息已由管理員初始化。怪物已更新，但為了獲得獎勵，請重新登錄。");
								int amount = 0;
								try{
									...
									gm.sendPackets(String.format("已初始化 %d 名角色的圖鑑信息。", amount));
								}catch(Exception e){
									e.printStackTrace();
								}finally{
									message.clear();
	...
	private MJCommandTree createV2Command(){
		return new MJCommandTree("週任務", "執行關於週任務的相關命令。", null)
				.add_command(new MJCommandTree("重新載入", "重新載入週任務中的各種數據。", null)
						.add_command(new MJCommandTree("獎勵", "", null){
							@Override
							protected void to_handle_command(MJCommandArgs args) throws Exception {
								BQSWQRewardsLoader.reload();
								args.notify("已重新載入 tb_mbook_wq_reward_info/tb_mbook_wq_reward_items 表。");
							}
						})
						.add_command(new MJCommandTree("週資訊", "", null){
							@Override
							protected void to_handle_command(MJCommandArgs args) throws Exception {
				...
				.add_command(new MJCommandTree("角色初始化", "根據傳來的角色名稱初始化該角色的週任務。", new String[]{"角色名稱"}) {
					@Override
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						String character_name = args.nextString();
						L1PcInstance pc = L1World.getInstance().getPlayer(character_name);
				...
				.add_command(new MJCommandTree("全部初始化", "初始化當前所有在線角色的週任務。", null) {
					protected void to_handle_command(MJCommandArgs args) throws Exception {
						L1PcInstance gm = args.getOwner();
						GeneralThreadPool.getInstance().execute(new Runnable() {
							@Override
							public void run() {
								gm.sendPackets(String.format("已初始化 %d 名角色的週任務。", BQSWQDecksLoader.getInstance().notifyUpdated()));
								return null;
							}
						});
					}
	...
	private MJCommandTree createSpawnCommand(){
		return new MJCommandTree("召喚", "基於圖鑑ID召喚怪物。", new String[]{"圖鑑ID", "數量"}) {
			@Override
			protected void to_handle_command(MJCommandArgs args) throws Exception {
				...
				args.notify(String.format("已召喚 %d 隻 %s(%d)。", count, bqsInfo.getNpcName(), criteria_id));
			}
		};
	}

	private MJCommandTree createCharacterUpdateCommand(){
		return new MJCommandTree("角色更新", "將傳來的圖鑑ID對應的圖鑑信息更新到指定角色的圖鑑中。", new String[]{"角色名稱", "圖鑑ID", "要更新的數量"}) {
			@Override
			protected void to_handle_command(MJCommandArgs args) throws Exception{
				String character_name = args.nextString();
				int criteria_id = args.nextInt();
				...
				GeneralThreadPool.getInstance().execute(new Runnable(){
					...
					public void run(){
						...
						try{
							...
							if(gm != null)
								gm.sendPackets(String.format("已將角色 %s 的圖鑑 %d 編號增加了 %d。", character_name, criteria_id, count));
						}
						return null;
					}
				});
	...
	private MJCommandTree createReloadConfigCommand(){
		return new MJCommandTree("重新加載配置", "重新加載圖鑑系統使用的配置。", null) {
			...
			protected void to_handle_command(MJCommandArgs args) throws Exception {
				...
				args.notify("mj_bookquestsystem.properties 已重新加載。");
			}
		};
	}

	private MJCommandTree createInitializedCommand(){
		return new MJCommandTree("系統初始化", "所有圖鑑信息將被初始化。", null) {
			@Override
			protected void to_handle_command(MJCommandArgs args) throws Exception {
				try {
					L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "因系統更新，圖鑑系統將暫時中斷。"));
					loadConfig();
					BQS_IS_ONUPDATE_BOOKS = false;
					BQSIdInfo.do_load();
					BQSInformationLoader.reload();
					...
					BQS_IS_ONUPDATE_BOOKS = true;
					L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "圖鑑系統已重新啟用。為了順利進行遊戲，請重新啟動遊戲。"));
				}
			}
		};
	}
		...
		return new MJCommandTree("設置", "將數據庫與 mj-tools… 同步。", null) {
			...
			protected void to_handle_command(MJCommandArgs args) throws Exception {
				try {
					...
					args.notify("數據庫同步已完成。");
					System.out.println("數據庫同步已完成。");
				} catch(Exception e) {
					e.printStackTrace();
				}
			}
	...
	private static void settingsAchievementTeleportCommand(MJCommandArgs args) {
		Updator.exec("update tb_mbook_information set tel_mapId=?, tel_x=?, tel_y=?, tel_need_item_id=?, tel_need_item_amount=?", new Handler() {
			@Override
			public void handle(PreparedStatement pstm) throws Exception {
				int idx = 0;
				pstm.setInt(++idx, 0);
		...
		Selector.exec("select * from `nature-achievement_teleport` order by achievement_id", new FullSelectorHandler(){
			...
			public void result(ResultSet rs) throws Exception {
				while(rs.next()) {
					...
					if(criteriaId <= 0) {
						System.out.println(String.format("無法找到對應的 criteriaId: %d。(settingsAchievementTeleportCommand)", achievementId));
						continue;
					}

					Updator.exec("update tb_mbook_information set tel_mapId=?, tel_x=?, tel_y=?, tel_need_item_id=?, tel_need_item_amount=? where criteria_id=?", new Handler() {
						@Override
						public void handle(PreparedStatement pstm) throws Exception {
							...
							if(item == null) {
								System.out.println(String.format("無法找到對應的消耗品項目: %d(%d)。已強制替換為祝福順序。(settingsAchievementTeleportCommand)", achievementId, costDescId));
							}else {
		...
		Selector.exec("select * from `nature-achievement-criteria` order by achievement_id", new FullSelectorHandler(){
			...
			public void result(ResultSet rs) throws Exception {
				while(rs.next()) {
					...
					final int criteriaId = achievementIdToCriteriaId(achievementId);
					if(criteriaId <= 0) {
						System.out.println(String.format("無法找到對應的 criteriaId: %d。", achievementId));
						continue;
					}
					...
					if(step >= 1 && step <= 3) {
						...
						Updator.exec(String.format("update tb_mbook_information set %s=? where criteria_id=?", column), new Handler() {
							@Override
							public void handle(PreparedStatement pstm) throws Exception {
						...
						System.out.println(String.format("ID 為 %d 的 criteria 數據過多。(步驟: %d)", achievementId, step));
						continue;
					}
				}
			}
