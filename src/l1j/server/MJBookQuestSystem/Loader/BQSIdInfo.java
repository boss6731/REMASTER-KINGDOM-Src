...
public class BQSIdInfo {
	...
	public static void do_load(){
		...
		Selector.exec("select achievement_id,criteria_id from tb_mbook_achievements order by achievement_id asc", new FullSelectorHandler(){
			@Override
	...
	public static BQSIdInfo from_criteria_id(final int criteria_id, int achievement_level){
		...
		if(ids == null){
			...
			System.out.println(String.format("not found BQSIdInfo...criteria_id : %d", criteria_id));
			return null;
		}
		if(ids.size() < achievement_level){
			System.out.println(String.format("not found BQSIdInfo achievement_level...criteria_id : %d, achievement_level : %d", criteria_id, achievement_level));
			return null;
		}
