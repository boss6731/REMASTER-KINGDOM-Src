...
//		destination_heal -= (get_user_decrease_percent(total_potion_info, owner, destination_heal) + total_potion_info.decrease_value);
		...
		destination_heal -= (get_user_decrease_percent(total_potion_info, owner, destination_heal) + total_potion_info.decrease_value);
		if (Config.LogStatus.USEPOTIONEFFECTLOGGIN) {
			System.out.println(String.format("물약 회복량:%.2f%%, 물약 회복값:%.2f, 물약회복감소효과상쇄:%.2f%%, 물약회복감소량:%.2f%%, 물약회복감소값:%.2f", (total_potion_info.increase_percent * 100),
					total_potion_info.increase_value, total_potion_info.decrease_defense_percent, total_potion_info.decrease_percent, total_potion_info.decrease_value));
			System.out.println(String.format("순수물약회복량:%.2f, 효과적용후회복량:%.2f, 보정회복량:%.2f", heal, destination_heal, Math.max((heal * 0.10), destination_heal)));
		}
		return Math.max((heal * 0.10), destination_heal);
