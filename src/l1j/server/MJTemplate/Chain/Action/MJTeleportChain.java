...
public class MJTeleportChain extends MJAbstractActionChain<MJITeleportHandler> {
	...
	public static final int ME_MARK_ID = 0; // 101 자신의 마크(우리팀)(본섭 패치되서 가입된 혈마크 표기)
	
	public static final int CLAN_MARK_ID = 104; //104 다른 혈 모두 표시 (다른혈은 해당마크 표기 가능)

	private static final int[] MARK_ID = new int[] { 106 }; // 무혈들 (가능)
			...
			if (SC_WORLD_PUT_OBJECT_NOTI.IS_PRESENTATION_MARK) {
				...
				case 12862: {
					...
					if (cInfo == null) {
						...
						// 여기 밑에가 문제임...(해결: 같은혈 마크처리,무혈 마크처리(주석시) (문제: 혈=혈 마크 처리)
						// 여기 밑에는 혈이없을때 처리(무혈)
