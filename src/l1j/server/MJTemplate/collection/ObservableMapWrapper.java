package l1j.server.MJTemplate.collection;

import java.util.Collection;
import java.util.Iterator;
import java.util.List;
import java.util.Map;
import java.util.Set;

import l1j.server.MJTemplate.MJValidate;
import l1j.server.MJTemplate.collection.MapEventListener.ChangeNode;

class ObservableMapWrapper<K, V> implements ObservableMap<K, V>{
	protected final Map<K, V> backingMap;
	protected final List<MapEventListener<K, V>> listeners;
	private ObservableKeySet keyset;
	private ObservableValues values;
	private ObservableEntrySet entryset;
	ObservableMapWrapper(Map<K, V> backingMap, List<MapEventListener<K, V>> listeners){
		this.backingMap = backingMap;
		this.listeners = listeners;
	}
	
	protected void firePut(K key, V newValue, V oldValue){
		firePutInternal(key, newValue, oldValue);
	}
	
	protected final void firePutInternal(K key, V newValue, V oldValue){
		if(listeners.size() <= 0){
			return;
		}
		
		ChangeNode<K, V> newNode = new ChangeNode<>(key, newValue);
		ChangeNode<K ,V> oldNode = new ChangeNode<>(key, oldValue);
		for(MapEventListener<K, V> listener : listeners){
			listener.onPut(this, newNode, oldNode);
		}
	}
	
	protected void firePutAll(Map<? extends K, ? extends V> map){
		firePutAllInternal(map);
	}
	
	protected final void firePutAllInternal(Map<? extends K, ? extends V> map){
		if(listeners.size() <= 0){
			return;
		}
		
		for(MapEventListener<K, V> listener : listeners){
			listener.onPutAll(this, map);
		}
	}
	
	protected void fireRemove(K key, V removeValue){
		fireRemoveInternal(key, removeValue);
	}
	
	protected final void fireRemoveInternal(K key, V removeValue){
		if(listeners.size() <= 0){
			return;
		}
		
		ChangeNode<K, V> removeNode = new ChangeNode<>(key, removeValue);
		for(MapEventListener<K, V> listener : listeners){
			listener.onRemove(this, removeNode);
		}
	}
	
	protected void fireClear(){
		fireClearInternal();
	}
	
	protected final void fireClearInternal(){
		if(listeners.size() <= 0){
			return;
		}
		for(MapEventListener<K, V> listener : listeners){
			listener.onClear(this);
		}
	}
	
	@Override
	public void addListener(MapEventListener<K, V> listener){
		MJValidate.checkNull(listener, "listener is null");
		listeners.add(listener);
	}
	
	@Override
	public void removeListener(MapEventListener<K, V> listener){
		MJValidate.checkNull(listener, "listener is null");
		listeners.remove(listener);
	}

	@Override
	public int size() {
		return backingMap.size();
	}

	@Override
	public boolean isEmpty() {
		return backingMap.isEmpty();
	}

	@Override
	public boolean containsKey(Object key) {
		return backingMap.containsKey(key);
	}

	@Override
	public boolean containsValue(Object value) {
		return backingMap.containsValue(value);
	}

	@Override
	public V get(Object key) {
		return backingMap.get(key);
	}

	@Override
	public V put(K key, V value) {
		MJValidate.checkNull(key, "key is null");
		MJValidate.checkNull(value, "value is null");
		
		V old = backingMap.put(key, value);
		firePut(key, value, old);
		return old;
	}

	@SuppressWarnings("unchecked")
	@Override
	public V remove(Object key) {
		MJValidate.checkNull(key, "key is null");
		V remove = backingMap.remove(key);
		if(remove == null){
			return null;
		}
		fireRemove((K)key, remove);
		return remove;
	}

	@Override
	public void putAll(Map<? extends K, ? extends V> map) {
		MJValidate.checkNull(map, "map is null");
		backingMap.putAll(map);
		firePutAll(map);
	}

	@Override
	public void clear() {
		if(backingMap.size() > 0){
			backingMap.clear();
			fireClear();
		}
	}

	@Override
	public Set<K> keySet() {
		return keyset == null ? (keyset = new ObservableKeySet()) : keyset;
	}

	@Override
	public Collection<V> values() {
		return values == null ? (values = new ObservableValues()) : values;
	}

	@Override
	public Set<java.util.Map.Entry<K, V>> entrySet() {
		return entryset == null ? (entryset = new ObservableEntrySet()) : entryset;
	}
	
	private class ObservableKeySet implements Set<K>{

		@Override
		public int size() {
			return backingMap.size();
		}

		@Override
		public boolean isEmpty() {
			return backingMap.isEmpty();
		}

		@Override
		public boolean contains(Object o) {
			return backingMap.keySet().contains(o);
		}

		@Override
		public Iterator<K> iterator() {
			return new Iterator<K>(){
				private Iterator<Entry<K, V>> entryIt = backingMap.entrySet().iterator();
				private K lastKey;
				private V lastValue;
				@Override
				public boolean hasNext() {
					return entryIt.hasNext();
				}
				@Override
				public K next() {
					Entry<K, V> last = entryIt.next();
					lastKey = last.getKey();
					lastValue = last.getValue();
					return lastKey;
				}
				
				@Override
				public void remove(){
					entryIt.remove();
					fireRemove(lastKey, lastValue);
				}
			};
		}

		@Override
		public Object[] toArray() {
			return backingMap.keySet().toArray();
		}

		@Override
		public <T> T[] toArray(T[] a) {
			return backingMap.keySet().toArray(a);
		}

		@Override
		public boolean add(K e) {
			throw new UnsupportedOperationException("Not supported.");
		}

		@Override
		public boolean remove(Object o) {
			return ObservableMapWrapper.this.remove(o) != null;
		}

		@Override
		public boolean containsAll(Collection<?> c) {
			return backingMap.keySet().containsAll(c);
		}

		@Override
		public boolean addAll(Collection<? extends K> c) {
			throw new UnsupportedOperationException("Not supported.");
		}

		@Override
		public boolean retainAll(Collection<?> c) {
			return removeRetain(c, false);
		}

		@Override
		public boolean removeAll(Collection<?> c) {
			return removeRetain(c, true);
		}

		@Override
		public void clear() {
			ObservableMapWrapper.this.clear();
		}

		@Override
		public String toString(){
			return backingMap.keySet().toString();
		}
		
		@Override
		public boolean equals(Object obj){
			return backingMap.keySet().equals(obj);
		}
		
		@Override
		public int hashCode(){
			return backingMap.keySet().hashCode();
		}
		
		private boolean removeRetain(Collection<?> c, boolean remove){
			boolean removed = false;
			for(Iterator<Entry<K, V>> i = backingMap.entrySet().iterator(); i.hasNext();){
				Entry<K, V> e = i.next();
				if(remove == c.contains(e.getKey())){
					removed = true;
					K key = e.getKey();
					V value = e.getValue();
					i.remove();
					fireRemove(key, value);
				}
			}
			return removed;
		}
	}
	
	private class ObservableValues implements Collection<V>{

		@Override
		public int size() {
			return backingMap.size();
		}

		@Override
		public boolean isEmpty() {
			return backingMap.isEmpty();
		}

		@Override
		public boolean contains(Object o) {
			return backingMap.values().contains(o);
		}

		@Override
		public Iterator<V> iterator() {
			return new Iterator<V>(){
				private Iterator<Entry<K, V>> entryIt = backingMap.entrySet().iterator();
				private K lastKey;
				private V lastValue;
				
				@Override
				public boolean hasNext() {
					return entryIt.hasNext();
				}

				@Override
				public V next() {
					Entry<K, V> last = entryIt.next();
					lastKey = last.getKey();
					lastValue = last.getValue();
					return lastValue;
				}
				
				@Override
				public void remove(){
					entryIt.remove();
					fireRemove(lastKey, lastValue);
				}
			};
		}


		@Override
		public Object[] toArray() {
			return backingMap.values().toArray();
		}

		@Override
		public <T> T[] toArray(T[] a) {
			return backingMap.values().toArray(a);
		}

		@Override
		public boolean add(V e) {
            throw new UnsupportedOperationException("Not supported.");
		}

		@Override
		public boolean remove(Object o) {
			for(Iterator<V> i = iterator(); i.hasNext();) {
                if (i.next().equals(o)) {
                    i.remove();
                    return true;
                }
            }
            return false;
		}

		@Override
		public boolean containsAll(Collection<?> c) {
			return backingMap.values().containsAll(c);
		}

		@Override
		public boolean addAll(Collection<? extends V> c) {
            throw new UnsupportedOperationException("Not supported.");
		}

		@Override
		public boolean removeAll(Collection<?> c) {
			 return removeRetain(c, true);
		}

		@Override
		public boolean retainAll(Collection<?> c) {
            return removeRetain(c, false);
		}

		@Override
		public void clear() {
			ObservableMapWrapper.this.clear();
		}
		
		@Override
        public String toString() {
            return backingMap.values().toString();
        }

        @Override
        public boolean equals(Object obj) {
            return backingMap.values().equals(obj);
        }

        @Override
        public int hashCode() {
            return backingMap.values().hashCode();
        }
		
		private boolean removeRetain(Collection<?> c, boolean remove){
			boolean removed = false;
			for(Iterator<Entry<K, V>> i = backingMap.entrySet().iterator(); i.hasNext();){
				Entry<K, V> e = i.next();
				if(remove == c.contains(e.getValue())){
					removed = true;
					K key = e.getKey();
					V value = e.getValue();
					i.remove();
					fireRemove(key, value);
				}
			}
			return removed;
		}
	}
	
	private class ObservableEntry implements Entry<K, V>{
		private final Entry<K, V> backingEntry;
		ObservableEntry(Entry<K, V> backingEntry){
			this.backingEntry = backingEntry;
		}
		
		@Override
		public K getKey() {
			return backingEntry.getKey();
		}

		@Override
		public V getValue() {
			return backingEntry.getValue();
		}

		@Override
		public V setValue(V value) {
			V oldValue = backingEntry.setValue(value);
			firePut(getKey(), value, oldValue);
			return oldValue;
		}
		
		@Override
		public final boolean equals(Object o){
			if (!(o instanceof Map.Entry)) {
                return false;
            }
			
			Map.Entry<?, ?> e = (Map.Entry<?, ?>) o;
            Object k1 = getKey();
            Object k2 = e.getKey();
            if (k1 == k2 || (k1 != null && k1.equals(k2))) {
                Object v1 = getValue();
                Object v2 = e.getValue();
                if (v1 == v2 || (v1 != null && v1.equals(v2))) {
                    return true;
                }
            }
            return false;
		}
		
		@Override
        public final int hashCode() {
            return (getKey() == null ? 0 : getKey().hashCode())
                    ^ (getValue() == null ? 0 : getValue().hashCode());
        }

        @Override
        public final String toString() {
            return new StringBuilder(32).append(getKey()).append("=").append(getValue()).toString();
        }
	}
	
	private class ObservableEntrySet implements Set<Entry<K, V>>{

		@Override
		public int size() {
			return backingMap.size();
		}

		@Override
		public boolean isEmpty() {
			return backingMap.isEmpty();
		}

		@Override
		public boolean contains(Object o) {
            return backingMap.entrySet().contains(o);
		}

		@Override
		public Iterator<java.util.Map.Entry<K, V>> iterator() {
			return new Iterator<Entry<K, V>>(){
				private Iterator<Entry<K, V>> backingIt = backingMap.entrySet().iterator();
				private K lastKey;
				private V lastValue;
				
				@Override
				public boolean hasNext() {
					return backingIt.hasNext();
				}
				
				@Override
				public java.util.Map.Entry<K, V> next() {
					Entry<K, V> last = backingIt.next();
					lastKey = last.getKey();
					lastValue = last.getValue();
					return new ObservableEntry(last);
				}
				
				@Override
				public void remove(){
					backingIt.remove();
					fireRemove(lastKey, lastValue);
				}	
			};
		}

		@SuppressWarnings("unchecked")
		@Override
		public Object[] toArray() {
			Object[] array = backingMap.entrySet().toArray();
			for (int i = 0; i < array.length; ++i) {
				array[i] = new ObservableEntry((Entry<K, V>)array[i]);
			}
			return array;
		}

		@SuppressWarnings("unchecked")
		@Override
		public <T> T[] toArray(T[] a) {
			T[] array = backingMap.entrySet().toArray(a);
            for (int i = 0; i < array.length; ++i) {
                array[i] = (T) new ObservableEntry((Entry<K, V>)array[i]);
            }
            return array;
		}

		@Override
		public boolean add(java.util.Map.Entry<K, V> e) {
            throw new UnsupportedOperationException("Not supported.");
		}

		@SuppressWarnings("unchecked")
		@Override
		public boolean remove(Object o) {
			boolean ret = backingMap.entrySet().remove(o);
            if (ret) {
                Entry<K,V> entry = (Entry<K, V>) o;
                fireRemove(entry.getKey(), entry.getValue());
            }
            return ret;
		}

		@Override
		public boolean containsAll(Collection<?> c) {
            return backingMap.entrySet().containsAll(c);
		}

		@Override
		public boolean addAll(Collection<? extends java.util.Map.Entry<K, V>> c) {
            throw new UnsupportedOperationException("Not supported.");
		}

		@Override
		public boolean retainAll(Collection<?> c) {
            return removeRetain(c, false);
		}

		@Override
		public boolean removeAll(Collection<?> c) {
            return removeRetain(c, true);
		}

		@Override
		public void clear() {
			ObservableMapWrapper.this.clear();
		}
		
		@Override
        public String toString() {
            return backingMap.entrySet().toString();
        }

        @Override
        public boolean equals(Object obj) {
            return backingMap.entrySet().equals(obj);
        }

        @Override
        public int hashCode() {
            return backingMap.entrySet().hashCode();
        }
        
		private boolean removeRetain(Collection<?> c, boolean remove){
			boolean removed = false;
			for(Iterator<Entry<K, V>> i = backingMap.entrySet().iterator(); i.hasNext();){
				Entry<K, V> e = i.next();
				if(remove == c.contains(e)){
					removed = true;
					K key = e.getKey();
					V value = e.getValue();
					i.remove();
					fireRemove(key, value);
				}
			}
			return removed;
		}
	}
}