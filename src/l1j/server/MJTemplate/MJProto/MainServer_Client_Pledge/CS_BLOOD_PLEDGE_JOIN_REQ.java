	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (ClanBanListTable.getInstance().checkClanBanlist(clanname, pc.getName())) {
				pc.sendPackets("해당혈은 가입 차단이 되어있어 가입이 불가능합니다.");
				return this;
			}
			...
			try {
				...
				case 1:
					if (pc.getLevel() < limit_level) {
						pc.sendPackets("해당 혈맹은 "+limit_level+"레벨 이상부터 가입 가능합니다.");
						return this;
					}
					set_join_type(ePLEDGE_JOIN_REQ_TYPE.ePLEDGE_JOIN_REQ_TYPE_CONFIRMATION);
					if (crown == null) {
				...
				case 0:
					if (clan.getClanName().equalsIgnoreCase("신규보호")) {
						if (pc.getLevel() >= Config.ServerAdSetting.NEWPLAYERLEVELPURGE) {
							pc.sendPackets(new S_SystemMessage(Config.ServerAdSetting.NEWPLAYERLEVELPURGE + "레벨 이상은 신규혈맹에 가입할 수 없습니다."));
							return this;
						}
