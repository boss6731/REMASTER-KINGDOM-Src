...
public class CS_BLOOD_PLEDGE_FIND_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static CS_BLOOD_PLEDGE_FIND_REQ newInstance(){
		return new CS_BLOOD_PLEDGE_FIND_REQ();
	}
	private byte[] _pledge_name;
	private int _memorizedSerializedSize = -1;
	private byte _memorizedIsInitialized = -1;
	private int _bit;
	private CS_BLOOD_PLEDGE_FIND_REQ(){
	}
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (get_pledge_name() != null)
				clan_name = new String(get_pledge_name());
			
			if (clan_name != null)
				SC_BLOOD_PLEDGE_FIND_ACK.sendClanInfo(pc, clan_name);
