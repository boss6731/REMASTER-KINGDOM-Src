...
public class SC_BLOOD_PLEDGE_LIST_ACK implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static void clan_list_send(L1PcInstance pc) {
		...
		for (L1Clan clan : L1World.getInstance().getAllClans()) {
			if (clan != null) {
				if (clan.getClanName().equalsIgnoreCase("붉은 기사단"))
					continue;
				
				BloodPledgeListInfoT info = BloodPledgeListInfoT.newInstance();
				info.set_pledge_name(clan.getClanName().getBytes());
				info.set_master_name(clan.getLeaderName().getBytes());
				info.set_emblem_id(clan.getEmblemId());
				info.set_member_count(clan.getClanMemberList().size());
				info.set_introduction_message(clan.getIntroduction() != null ? clan.getIntroduction().getBytes() : "".getBytes());
				info.set_weekly_contribution_total(0);
				info.set_join_type(ePLEDGE_JOIN_REQ_TYPE_COMMON.fromInt(clan.getJoinSetting() == 1 ? clan.getJoinType() : 3));
				info.set_enable_join(clan.getJoinSetting() == 1 ? true : false);
				noti.add_blood_pledge_list_info(info);
			}
		}
		noti.set_is_last(true);
		pc.sendPackets(noti, MJEProtoMessages.SC_BLOOD_PLEDGE_LIST_ACK, true);
	}
