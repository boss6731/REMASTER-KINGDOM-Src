	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (get_join_type().toInt() == 2) {
				pc.sendPackets("현재 암호가입 설정을 할 수 없습니다.");
				return this;
			}
			...
			if (isInvalidName(clan_name)) {
				pc.sendPackets(new S_SystemMessage("잘못된 혈맹 이름입니다."));
				return this;
			}
...
//						}
					...
					pc.sendPackets(new S_SystemMessage("이미 혈맹이 있거나 "+ Config.ServerAdSetting.CROWNBLOODLEVEL +"레벨 이상만 창설할 수 있습니다.")); // 추가
				}
			} else {
