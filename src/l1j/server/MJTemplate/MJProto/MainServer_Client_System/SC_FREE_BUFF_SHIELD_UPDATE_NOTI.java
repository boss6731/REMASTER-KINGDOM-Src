...
public class SC_FREE_BUFF_SHIELD_UPDATE_NOTI implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static void send(L1PcInstance pc) {
		SC_FREE_BUFF_SHIELD_UPDATE_NOTI noti = SC_FREE_BUFF_SHIELD_UPDATE_NOTI.newInstance();
		...
		noti.set_disable_state(disable);
		/*PC_CAFE_SHIELD(0),
		EVENT_BUFF_SHIELD(1),
		FREE_BUFF_SHIELD(2),*/
		...
		for (int i = 0; i < 3; i++) {
			...
			noti.add_free_buff_shield_info(shield_info);
		}
		pc.sendPackets(noti, MJEProtoMessages.SC_FREE_BUFF_SHIELD_INFO_ACK, true);
	...
	public static SC_FREE_BUFF_SHIELD_UPDATE_NOTI newInstance(){
		return new SC_FREE_BUFF_SHIELD_UPDATE_NOTI();
	}
	...
	private int _bit;
	private SC_FREE_BUFF_SHIELD_UPDATE_NOTI(){
	}
