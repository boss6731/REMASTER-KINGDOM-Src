...
public class CS_EXTEND_SLOT_CURRENCY_SELECT implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (pc.getLevel() < 60) {
				pc.sendPackets("레벨이 부족하여 슬롯을 개방할 수 없습니다.");
				return this;
			}
...
//			System.out.println(get_slot_position());
			...
			case 0:
				...
				break;
			case 1:
				if (!pc.getQuest().isEnd(L1Quest.QUEST_RING_RIGHT_SLOT60) && pc.getQuest().isEnd(L1Quest.QUEST_RING_LEFT_SLOT60)) {
					...
					pc.sendPackets("왼쪽 반지 슬롯을 먼저 개방 후 이용하시기 바랍니다.");
					return this;
				}
				break;
			case 2:
			...
			case 5:
				...
				break;
			case 6:
				if (!pc.getQuest().isEnd(L1Quest.QUEST_RING_LEFT_SLOT95) && pc.getQuest().isEnd(L1Quest.QUEST_RING_RIGHT_SLOT60)) {
					if (pc.getInventory().checkItem(3000400, 1)) {
						dicount = true;
					} else {
					...
					pc.sendPackets("오른쪽 반지 슬롯을 먼저 개방 후 이용하시기 바랍니다.");
					return this;
				}
				break;
			case 7:
				if (!pc.getQuest().isEnd(L1Quest.QUEST_RING_RIGHT_SLOT100) && pc.getQuest().isEnd(L1Quest.QUEST_RING_LEFT_SLOT95)) {
					if (pc.getInventory().checkItem(3000401, 1)) {
						dicount = true;
					} else {
					...
					pc.sendPackets("95레벨 반지 슬롯을 먼저 개방 후 이용하시기 바랍니다.");
					return this;
				}
				break;
			case 8:
				if (!pc.getQuest().isEnd(L1Quest.QUEST_EARRING_LEFT_SLOT101) && pc.getQuest().isEnd(L1Quest.QUEST_RING_RIGHT_SLOT100)) {
					if (pc.getInventory().checkItem(3000401, 1)) {
						dicount = true;
					} else {
					...
					pc.sendPackets("100레벨 반지 슬롯을 먼저 개방 후 이용하시기 바랍니다.");
					return this;
				}
				break;
				
			case 9:
				if (!pc.getQuest().isEnd(L1Quest.QUEST_EARRING_RIGHT_SLOT103) && pc.getQuest().isEnd(L1Quest.QUEST_EARRING_LEFT_SLOT101)) {
					if (pc.getInventory().checkItem(3000401, 1)) {
						dicount = true;
					} else {
						if (pc.getInventory().checkItem(40308, 100000000)) {
