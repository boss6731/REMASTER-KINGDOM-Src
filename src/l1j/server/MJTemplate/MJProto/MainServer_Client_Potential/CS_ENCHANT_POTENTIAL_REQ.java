	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes) {
		...
		try {
			...
			if (magicDollItem != null) {
				...
				if (pc.isGm()) {
					System.out.println(String.format("(GM:인형:잠재력) : %d단계 / 강화:%s / 최종확률:%s)", bonus_type + 1, success == true ? "성공" : "실패", random_per));
				}
				LoggerInstance.getInstance().addDollPotencial(String.format("[캐릭명]:%s [단계]:%d [강화]:%s [최종확률]:%d", pc.getName(), bonus_type + 1, success == true ? "성공" : "실패", random_per));
...
*///					bonus_value = MJRnd.next(132, 143);
				...
				if (pc.getInventory().consumeItem(Config.MagicDollInfo.POTION_ITEMID, potion_count)) {
					...
					System.out.println(String.format("마법인형 연금술 : %s 비 정상적인 업그레이드 시도. (잠재력 강화의 비약 없이 시도 중계기 의심)", pc.getName()));
					return this;
				}
			} else {
				System.out.println(String.format("마법인형 연금술 : %s 비 정상적인 업그레이드 시도. (마법 인형 없이 시도 중계기 의심)", pc.getName()));
				return this;
			}
