...
public class SC_EVENT_PUSH_INFO_LIST_ACK implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	public static SC_EVENT_PUSH_INFO_LIST_ACK newInstance(L1PcInstance pc) {
		...
		for(MJChaPushModel model : pc.get_push_info()) {
			...
			einfo.set_item_icon(MJPushItemData.getlist().get(model.getPushId()).isUseitem_at_once() ? 9904 : item.getGfxId());
			einfo.set_item_desc(MJPushItemData.getlist().get(model.getPushId()).isUseitem_at_once() ? "$31343" : item.getNameId());
			einfo.set_item_extra_desc(MJPushItemData.getlist().get(model.getPushId()).isUseitem_at_once() ? "아인하사드 축복 보너스".getBytes() : item.getName().getBytes());
