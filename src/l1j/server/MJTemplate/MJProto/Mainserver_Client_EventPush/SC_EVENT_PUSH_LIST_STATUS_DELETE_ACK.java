...
public class SC_EVENT_PUSH_LIST_STATUS_DELETE_ACK implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	public static SC_EVENT_PUSH_LIST_STATUS_DELETE_ACK newInstance(L1PcInstance pc, CS_EVENT_PUSH_LIST_STATUS_DELETE_REQ req) {
		...
		for(EventPushKey val : req.get_event_push_key_list()){
			...
			for(int i = 0; i < pc.get_push_info().size(); i++) {
				if(pc.get_push_info().get(i).getPushId() == val.get_event_push_id() && pc.get_push_info().get(i).getPushId() == val.get_reset_num()) {
	...
	public static SC_EVENT_PUSH_LIST_STATUS_DELETE_ACK newInstance_timover(L1PcInstance pc, int push_id, int reset_num) {
		SC_EVENT_PUSH_LIST_STATUS_DELETE_ACK noti = newInstance();
		EventPushResult result = EventPushResult.newInstance();
		EventPushKey key = EventPushKey.newInstance();
