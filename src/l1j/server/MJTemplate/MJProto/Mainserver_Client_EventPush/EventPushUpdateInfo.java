...
public class EventPushUpdateInfo implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static EventPushUpdateInfo newInstance(L1PcInstance pc, int push_id, int reset_num, int state) {
		...
		for(MJChaPushModel model : pc.get_push_info()) {
			if(MJPushItemData.getlist().get(model.getPushId()).getpushid() == push_id && MJPushItemData.getlist().get(model.getPushId()).getpushid() == reset_num) {
				int remaintime = (int)(MJPushItemData.getlist().get(model.getPushId()).getExpiredate() - System.currentTimeMillis() / 1000);
