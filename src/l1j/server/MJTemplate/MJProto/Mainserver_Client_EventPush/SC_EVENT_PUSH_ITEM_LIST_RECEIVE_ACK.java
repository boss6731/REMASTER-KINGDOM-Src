...
public class SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static ProtoOutputStream make_stream(L1PcInstance pc, CS_EVENT_PUSH_ITEM_LIST_RECEIVE_REQ req) {
		SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK noti = newInstance(pc, req);
		ProtoOutputStream stream = noti.writeTo(MJEProtoMessages.SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK);
		noti.dispose();
		return stream;
	}

	public static SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK newInstance(L1PcInstance pc, CS_EVENT_PUSH_ITEM_LIST_RECEIVE_REQ req) {
		SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK noti = newInstance();
		for(EventPushKey val : req.get_event_push_key_list()){
			...
			for(int i = 0; i < pc.get_push_info().size(); i++) {
				if(pc.get_push_info().get(i).getPushId() == val.get_event_push_id() && pc.get_push_info().get(i).getPushId() == val.get_reset_num() && pc.get_push_info().get(i).getPushstate() != 2) {
					if(MJPushItemData.getlist().get((int)val.get_event_push_id()).isUseitem_at_once()) {
						pc.getAccount().addBlessOfAin((1000000), pc, "푸시");
					} else {
						pc.getInventory().storeItem(MJPushItemData.getlist().get((int)val.get_event_push_id()).getItemId(), MJPushItemData.getlist().get((int)val.get_event_push_id()).getItemCount(), MJPushItemData.getlist().get((int)val.get_event_push_id()).getEnchantlevel());
					}
					pc.get_push_info().get(i).setPushstate(2);
					MJPushChaData.getInstance().Update_Info(pc.getId(), pc.get_push_info().get(i));
					break;
				}
			}
		}
	...
	public static SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK newInstance(){
		return new SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK();
	}
	...
	private int _bit;
	private SC_EVENT_PUSH_ITEM_LIST_RECEIVE_ACK(){
	}
