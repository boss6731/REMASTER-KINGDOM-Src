...
public class SC_ARENACO_BYPASS_INDUN_MATCHING_SUCCESS_NOTI implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static SC_ARENACO_BYPASS_INDUN_MATCHING_SUCCESS_NOTI newInstance(){
		return new SC_ARENACO_BYPASS_INDUN_MATCHING_SUCCESS_NOTI();
	}
	...
	private int _bit;
	private SC_ARENACO_BYPASS_INDUN_MATCHING_SUCCESS_NOTI(){
	...
	public int getSerializedSize(){
		int size = 0;
		if (has_is_owner()){
			size += l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream.computeBoolSize(1, _is_owner);
		}
		if (has_room_id()){
			size += l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream.computeUInt32Size(2, _room_id);
		}
		if (has_mapkind()){
			size += l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream.computeEnumSize(3, _mapkind.toInt());
		}
	...
	public void writeTo(l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream output) throws java.io.IOException{
		if (has_is_owner()){
			output.writeBool(1, _is_owner);
		}
		if (has_room_id()){
			output.writeUInt32(2, _room_id);
		}
		if (has_mapkind()){
			output.writeEnum(3, _mapkind.toInt());
		}
	}
	@Override
