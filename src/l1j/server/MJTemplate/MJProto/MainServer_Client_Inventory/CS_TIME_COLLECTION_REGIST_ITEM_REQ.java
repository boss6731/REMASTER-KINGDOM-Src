	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (registItem.isEquipped() || registItem.getBless() >= 128 || registItem.getEndTime() != null) {
				pc.sendPackets(new S_ServerMessage(9023), true);// 선택한 장비를 전시 등록할 수 없습니다.
				return this;
			}
			...
			if (collection == null) {
				System.out.println(String.format(
						"[A_TimeCollectionRegist] NOT_COLLECTION : TYPE(%s), COLLECTION_INDEX(%d), ITEMID(%d), NAME(%s)",
			...
			if (material == null || !material.isMaterial(registItem)) {
				System.out.println(String.format(
						"[A_TimeCollectionRegist] NOT_MATERIAL : SLOT_INDEX(%d), ITEMID(%d), NAME(%s)",
			...
			if (!pc.getTimeCollection().regist(pc, collection, registItem, _slotNo)) {
				System.out.println(String.format(
						"[A_TimeCollectionRegist] REGIST_FAILURE : SLOT_INDEX(%d), ITEMID(%d), NAME(%s)",
						_slotNo, registItem.getItemId(), pc.getName()));
				return this;
			}
