...
public class CS_FAVOR_ENGRAVE_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	private eCraftMakeReqResultType doMake(L1PcInstance pc, CraftOutputItemResult outputResult) {
		...
		if (removedCount < _slotInfo.size()) {
			System.out.println(String.format("[(제작)유저의 인벤토리에서 재료 정보를 찾을 수 없습니다.성물] 캐릭터이름 : %s, 제작아이디 : %d, 네임아이디 : %d, 찾은 수 : %d, 찾아야할 수 : %d", pc.getName(), _craft_id, craft.get_craft_attr().get_desc(), removedCount, _slotInfo.size()));
			System.out.println();
		...
		if (outputItem == null) {
			if (outputResult.isSuccess) {
				System.out.println(String.format("[(제작)출력 아이템을 찾을 수 없습니다.] 캐릭터이름 : %s, 제작아이디 : %d, 네임아이디 : %d", pc.getName(), _craft_id, craft.get_craft_attr().get_desc()));
...
//				sInfo.doRemovedFavor(pc.getInventory(), _favorInv, _favorUser, _favorItem, _craft_id);
		...
		if (l1item == null) {
			System.out.println(String.format("[(제작)출력 아이템에 해당하는 서버 아이템을 찾을 수 없습니다.] 캐릭터이름 : %s, 제작아이디 : %d, 네임아이디 : %d, 데스크아이디 : %d", pc.getName(), _craft_id, craft.get_craft_attr().get_desc(), outputItem.get_name_id()));
	...
	public MJIProtoMessage readFrom(GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (_count <= 0) {
				System.out.println(String.format("[(제작)제작 수량이 비정상적입니다.] 캐릭터이름 : %s, 제작아이디 : %d, 수량 : %d", pc.getName(), _craft_id, _count));
				return this;
			}
			...
			if (craft == null) {
				...
				System.out.println(String.format("[(제작)제작 아이디를 찾을 수 없습니다.] 캐릭터이름 : %s, 제작아이디 : %d", pc.getName(), _craft_id));
				return this;
			}
			...
			if (!inputs.has_arr_input_item()) {
				...
				System.out.println(String.format("[(제작)제작 재료 아이템 리스트를 찾을 수 없습니다.] 캐릭터이름 : %s, 제작아이디 : %d, 네임아이디 : $%d", pc.getName(), _craft_id, craft.get_craft_attr().get_desc()));
				return this;
			}
			...
			if (calcSlotAndIncreaseProb[0] - 1 != _slotInfo.size()) {
				eCraftMakeReqResultType.RP_ERROR_INVALID_INPUT.sendCachedMessage(pc);
				System.out.println(String.format("[(제작)재료 슬롯 정보가 일치하지 않습니다.] 캐릭터이름 : %s, 제작아이디 : %d, 네임아이디: %d, 슬롯번호 : %d, 유저가 보내온 패킷 슬롯 수 : %d", pc.getName(), _craft_id, craft.get_craft_attr().get_desc(), calcSlotAndIncreaseProb[0] - 1, _slotInfo.size()));
				return this;
			}
