...
public class CS_TIME_COLLECTION_RESET_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static CS_TIME_COLLECTION_RESET_REQ newInstance(){
		return new CS_TIME_COLLECTION_RESET_REQ();
	}
	...
	private int _bit;
	private CS_TIME_COLLECTION_RESET_REQ(){
	}
		...
		try{
			...
			if (collection == null) {
				System.out.println(String.format(
						"[A_TimeCollectionDelete] NOT_COLLECTION : TYPE(%s), INDEX(%d), NAME(%s)", 
						type.getName(), _setID, pc.getName()));
				return this;
			}
			
			// 제거
			pc.getTimeCollection().delete(pc, collection, L1TimeCollectionStatus.DELETE);
