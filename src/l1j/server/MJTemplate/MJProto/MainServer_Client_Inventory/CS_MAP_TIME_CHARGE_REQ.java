...
//			System.out.println(item.length);
			...
			if (item.length > 1) {
				System.out.println("숨겨진 차원의 모래시계 사용 오류 (케릭 아이디: "+pc.getName()+" 사용 아이템 Desc_Id: "+itemid);
				return this;
			}
			...
			if (group == 1) { //일반 숨사
				...
				if (progress == null) {
					pc.sendPackets(String.format("충전석 사용전 던전(%s)에 최초 한번은 입장하셔야 합니다.", dtInfo.get_description()));
					return this;
				}
