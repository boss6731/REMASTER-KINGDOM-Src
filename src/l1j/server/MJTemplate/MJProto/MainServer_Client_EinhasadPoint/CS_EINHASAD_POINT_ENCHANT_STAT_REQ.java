	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (pc.getAccount().getBlessOfAinBonusPoint() < 5000) {
				System.out.println(String.format("아이하사드 스탯 오류 : 중계기 사용 의심됨 캐릭명(%s)", pc.getName()));
				return this;
			}
			
			AinhasadSpecialStatInfo Info = AinhasadSpecialStatLoader.getInstance().getSpecialStat(pc.getId());
			if (Info != null) {
				int percent = MJRnd.next((Info.get_cur_enchant_level() + 1) * Config.ServerRates.AinBonusPoint_Kard_Probability);
...
//				success = true;//무조건 성공할때
				...
				String log = "[캐릭명]:" + pc.getName() + " [인챈단계]:" + Info.get_cur_enchant_level() + " [보너스포인트]:" + point + " [총포인트]:" + Info.get_total_stat() + " [결과]:" + (success ? "성공" : "실패");
				LoggerInstance.getInstance().addBlessOfAinPointCard(log);
				pc.getAccount().minusBlessOfAinBonusPoint(ainpoint);
				SC_EINHASAD_POINT_ENCHANT_STAT_ACK.send_point_enchant_stat(pc, point, bonus, Info.get_cur_enchant_level(), Info.get_total_stat());
