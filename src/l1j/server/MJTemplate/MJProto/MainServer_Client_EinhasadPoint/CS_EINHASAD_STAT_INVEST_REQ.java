...
public class CS_EINHASAD_STAT_INVEST_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (Info != null) {
				int current_stat = Info.get_bless() + Info.get_lucky() + Info.get_vital() + Info.get_invoke() + Info.get_restore() + Info.get_potion() + 1;
				if (current_stat > Info.get_total_stat()) {
					System.out.println(String.format("아인하사드 스탯 초과 : 중계기 사용 의심됨 캐릭명(%s)", pc.getName()));
					return this;
				}
				...
				case 3:{
					...
					case 35:
						...
						break;
					default: {
						System.out.println(String.format("비정상 적인 방법으로 스탯을 취득.(캐릭명 : %s)", pc.getName()));
						return this;
					}
					}
					break;
