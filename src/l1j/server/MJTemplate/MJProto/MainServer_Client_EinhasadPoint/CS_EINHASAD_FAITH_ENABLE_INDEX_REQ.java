	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if (!Config.ServerAdSetting.EINHASAD_FAITH_SYSTEM) {
				pc.sendPackets("미공개: 추후 공개 예정 (홈페이지참고)");
				return this;
			}
			int resultcode = 1;
			int index = _indexId;
			...
			if (completeGroup1 || completeGroup2) {
				...
				if (index >= 1 && index <= 4) {
					while(iter.hasNext()) {
						...
						if (index >=1 && index <= 4) {
							int comparetime = (Timestamp.valueOf(pc.getAinhasad_faith_EndTime(index).toString()).getDate());
							if (endTimeint == 0) {
...
//						AinhasadSpecialStat2Loader.getInstance().addSpecialStat2(pc.getId(), pc.getName(), 101);
...
//						AinhasadSpecialStat2Loader.getInstance().addSpecialStat2(pc.getId(), pc.getName(), 102);
