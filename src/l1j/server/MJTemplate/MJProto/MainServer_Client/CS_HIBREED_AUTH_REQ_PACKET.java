		...
		try {
			...
			if (Config.Login.UseShiftServer)
				...
				if (reserved_no == SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_FISLAND || reserved_no == SC_CONNECT_HIBREEDSERVER_NOTI_PACKET.RESERVED_DOM_TOWER
					...
					if (array.length != 10) {
						System.out.println(String.format("[���ͼ���:invalid local shift info... kick. reserved : %d, client ip : %s]", get_reservednumber(), clnt.getIp()));
						clnt.kick();
						return this;
					}
					...
					if (account == null) {
						System.out.println(String.format("[���ͼ���:invalid local shift account info... kick. reserved : %d, client ip : %s, account_name : %s, character_name : %s]", get_reservednumber(),
					...
					// /TODO ���⼭ �ñ�°ǵ�..��..?
					if (old_client == null) {
						System.out.println(String.format("[���ͼ���:invalid locatl tempmap client....kick reserved : %d, client ip : %s, account_name : %s, character_name : %s]", get_reservednumber(),
								clnt.getIp(), account_name, character_name));
						clnt.kick();
						return this;
					}
					...
					if (LoginController.getInstance().containsAccount(account_name)) {
						GeneralThreadPool.getInstance().execute(new Runnable() {
							...
							public void run() {
								try {
									/**
									 * TODO ���ͼ��� ���� ���� TODO ���� ���� �׽�Ʈ(10->1) 10�� �ÿ� �õ����ϰ� ���������� �ñ��� �ʰ�, ������ �ѹ��Ų��.
									 * �׽�Ʈ GameClient.java ���� ���� �˻��� 3000[3��]�� ���� �� ���� ī��Ʈ 10->1�� ���� Thread.sleep 1000L�� 2000L�� ���� 
									 * TODO 10->1�� ������ ������ 10���� �������� �ñ��.
									 **/
									for (int i = 0; i < 1; ++i) {
										Thread.sleep(1000L);
										System.out.println("[���ͼ��� �������� ���� ��õ� ó�� : ���� (" + account_name + ") �õ�Ƚ�� : " + (i + 1) + "ȸ]");
										if (!LoginController.getInstance().containsAccount(account_name)) {
					...
					try {
						...
						if (failureMembers.containsKey(object_id)) {
							GeneralThreadPool.getInstance().schedule(new Runnable() {
								...
								public void run() {
									...
									if (pc != null) {
										pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\f3���� ���� ����� ���� ���� �̵��� �����߽��ϴ�.\r\n\\f3��� �� ��õ����ּ���."));
										pc.sendPackets("\\f3���� ���� ����� ���� ���� �̵��� �����߽��ϴ�. ��� �� ��õ����ּ���.");
										System.out.println("[���ͼ��� �������� ���� ��õ� ó�� ��Ʈ(Ȯ�ο�) : ���� ���� ����� ���� ���� �̵��� �����߽��ϴ�]");
									}
                                    return new L1PcInstance[0];
                                }
							}, 2000L);
						...
						String s = String.format("���ͼ���:�ִ� �����ο� �ʰ� : (%s) �α����� �����߽��ϴ�.", clnt.getIp());
						System.out.println(s);
						clnt.close();
					} catch (AccountAlreadyLoginException e) {
						String s = String.format("���ͼ���:������ ID�� ���� : (%s) ���� ���� �߽��ϴ�.", clnt.getIp());
						System.out.println(s);
						clnt.close();
					} catch (Exception e) {
						e.printStackTrace();
						String s = String.format("���ͼ���:���������� �α��� ���� . account=%s host=%s", account_name, clnt.getHostname());
						System.out.println(s);
						clnt.close();
					}
				} else {
					System.out.println(String.format("���ͼ���:falsed use shift server : %s", Config.Login.UseShiftServer));
				}
			}
		} catch (AccountAlreadyLoginException e) {
			System.out.println(String.format("���ͼ���:��������%s ��%s����IP��%s ������ ���� ĳ�� �ߺ��α���", clnt.getAccountName(), MJNSHandler.getLocalTime(), clnt.getIp()));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return this;
