...
public class SC_TOP_RANKER_NOTI extends MJAbstractComparable<SC_TOP_RANKER_NOTI> implements l1j.server.MJTemplate.MJProto.MJIProtoMessage {
	...
	static {
		...
		_buffs = new MJRankBuff[] { null, new MJRankBuffLv1(), new MJRankBuffLv2(), new MJRankBuffLv3(), new MJRankBuffLv3(), new MJRankBuffLv4(), new MJRankBuffLv5(), new MJRankBuffLv6(), new MJRankBuffLv7(), new MJRankBuffLv8(), };
	}
...
//		rnk.set_rank(MJRankLoadManager.MRK_SYS_TOTAL_RANGE);
	...
	/**
	 * 랭킹시스템하고 캐시경험치하고 동기화 걸릴수가있어서 밑에 처럼 처리한다. 팅겼을때 경험치가 랭킹에 계산이 안될 가능성이 있어서 처리함
	 */
				...
				for (int r = 0; r < rank_limit.length; r++) {
					...
					if (get_total_ranker().get_rank() == limit) {
						if (!_characterInstance.getInventory().checkItem(itemid, 1, enchant, 128, 0 )) {
							...
							_characterInstance.sendPackets(new S_ServerMessage(813, "랭킹진입", give_tem.getLogName(), _characterInstance.getName()));
						}
					}
				}
...
//				if (get_total_ranker().get_previous_rank() != get_total_ranker().get_rank()) {
					...
					for (int r = 0; r < rank_limit.length; r++) {
						...
						if (get_total_ranker().get_rank() == limit) {
							if (!_characterInstance.getInventory().checkItem(itemid)) {
								...
								_characterInstance.sendPackets(new S_ServerMessage(813, "랭킹진입", tem.getLogName(), _characterInstance.getName()));
							}
						}
...
//			if (total_rating < 9 && class_place <= 3) {
	...
	public l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream writeTo(l1j.server.MJTemplate.MJProto.MJEProtoMessages message) {
		l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream stream = l1j.server.MJTemplate.MJProto.IO.ProtoOutputStream.newInstance(getSerializedSize() + WireFormat.WRITE_EXTENDED_SIZE, message.toInt());
		try {
			writeTo(stream);
		} catch (IOException e) {
			e.printStackTrace();
