	...
	private eCraftMakeReqResultType doMake(L1PcInstance pc, CraftOutputItemResult outputResult) {
		...
		if (_craft_id != 0) {
			if (!CraftListNewAllow.getInstance().isCraft(_craft_id)) {
				pc.sendPackets("해당 제작은 서비스가 일시적으로 중단되었거나, 클라이언트를 재실행 하시길 바랍니다.");
				if (pc.isGm())
					System.out.println("[CS_CRAFT_MAKE_REQ](GM)2 : (불가)craft_id:" + _craft_id);
...
//		L1ItemInstance result = ItemTable.getInstance().createItem(l1item);// 제작할 아이템 생성
...
/*		
		...
		if (inputInfo.get_elemental_enchant_value() != 0)
			result.setAttrEnchantLevel(L1ItemInstance.calculateElementalEnchant(inputInfo.get_elemental_enchant_type(), inputInfo.get_elemental_enchant_value()));
...
//			System.out.println(sInfo.get_item_name_id());
		...
		if (outputResult.isSuccess) {
			if (pc != null && ItemMessageTable.getInstance().isCraftMessage(result.getItemId())) {
					...
					if (temp.getType() == 3) {
						if (temp.isMentuse()) {
							...
							if (temp.getMent() != null && !temp.getMent().equalsIgnoreCase("")) {
								L1World.getInstance().broadcastPacketToAll(SC_MESSAGE_NOTI.newDropMessage(result.getItem().getItemDescId(), temp.getMent()), true);
								...
								String message = String.format("" + men + " (%s) 제작에 성공하였습니다.", itemName);
								// String message1 = String.format("" + men + " (%s\\fH) 제작에 성공하였습니다.", itemName);

								L1World.getInstance().broadcastPacketToAll(SC_MESSAGE_NOTI.newDropMessage(result.getItem().getItemDescId(), message), true);
								/*
								 * L1World.getInstance().broadcastPacketToAll(new ServerBasePacket[]{ new S_SystemMessage(message), new S_PacketBox(S_PacketBox.GREEN_MESSAGE, message1) });
								 */
							}
						}
					}
				...
				if (get_craft_id() >= 30227 && get_craft_id() <= 30231 || get_craft_id() >= 4934 && get_craft_id() <= 4949 || get_craft_id() == 4070 || get_craft_id() == 633 || get_craft_id() == 2861 || get_craft_id() == 819 || get_craft_id() == 821
						|| get_craft_id() == 518 || get_craft_id() >= 423 && get_craft_id() <= 425 || get_craft_id() >= 5217 && get_craft_id() <= 5224 || get_craft_id() >= 30217 && get_craft_id() <= 30226 && !pc.isGm()) {
			...
			if (_npc_id != 0) {
				...
				if (nInfo != null) {
					...
					if (!l1j.server.MJTemplate.MJString.isNullOrEmpty(message))
						pc.sendPackets(String.format("%s 획득: %s", result.getItem().getName(), message));
				}
			}
		}
		...
		try {
			...
			if (_craft_id != 0) {
				if (!CraftListNewAllow.getInstance().isCraft(get_craft_id())) {
					pc.sendPackets("해당 제작은 서비스가 일시적으로 중단되었거나, 클라이언트를 재실행 하시길 바랍니다.");
					if (pc.isGm())
					System.out.println("[CS_CRAFT_MAKE_REQ](GM)1 : (불가)craft_id:" + _craft_id);
					return this;
				}
			}
			...
			if (limit != null) {
				if (limit.get_craft_current_count() > limit.get_craft_max_count() || _count + limit.get_craft_current_count() > limit.get_craft_max_count()) {
					...
					// System.out.println(String.format("[(제작)제작 제한 수를 초과 하였습니다.] 캐릭터이름 : %s, 제작아이디 : %d, 아이템명 : %s, 시도 제작횟수 : %d, 최대 제작횟수 : %d, 현재 제작횟수 : %d ", pc.getName(), get_craft_id(), limit.get_craft_item_name(), _count,
			...
			for (int i = 0; i < _count; ++i) {
				CraftOutputItemResult outputResult = craft.get_outputs().createOutputItem(calcSlotAndIncreaseProb[1], get_craft_id(), pc);
				...
				if (eResult.equals(eCraftMakeReqResultType.RP_SUCCESS)) {
					...
					if (_count == 1) {
						LoggerInstance.getInstance().addcraftMake(pc, craft.get_craft_attr().get_desc(), "성공", _count);
					}
				} else if (eResult.equals(eCraftMakeReqResultType.RP_FAILURE)) {
					SC_CRAFT_MAKE_ACK.sendResultPacket(pc, eResult, null, i + 1, _count);
					if (_count == 1) {
						LoggerInstance.getInstance().addcraftMake(pc, craft.get_craft_attr().get_desc(), "실패", _count);
					}
				} else {
	...
	public static class CraftInputItemSlotInfo implements l1j.server.MJTemplate.MJProto.MJIProtoMessage {
		...
		public void doRemovedFavor(L1Inventory inv, L1FavorBookInventory favorInv, L1FavorBookUserObject favorUser, L1ItemInstance favorItem, int craftId) {
			for (TemporaryRemovedItemInstance removed : _temporaryRemovedItems) {
				if (_temporaryRemovedCount <= 0)
					break;
		...
		public boolean validation(l1j.server.MJTemplate.MJProto.MainServer_Client.CraftCommonBin.SC_CRAFT_LIST_ALL_ACK.Craft.CraftInputList.CraftInputItem input) {
