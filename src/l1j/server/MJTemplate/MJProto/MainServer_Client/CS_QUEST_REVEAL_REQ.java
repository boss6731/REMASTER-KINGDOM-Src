...
public class CS_QUEST_REVEAL_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static CS_QUEST_REVEAL_REQ newInstance(){
		return new CS_QUEST_REVEAL_REQ();
	}
	private int _id;
	private int _memorizedSerializedSize = -1;
	private byte _memorizedIsInitialized = -1;
	private int _bit;
	private CS_QUEST_REVEAL_REQ(){
	}
	public int get_id(){
		return _id;
	}
	...
	public MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			MJBeginnerControllerProvider.provider().clientController().onReveal(pc, get_id());
		} catch(Exception e){
			e.printStackTrace();
		}
		return this;
