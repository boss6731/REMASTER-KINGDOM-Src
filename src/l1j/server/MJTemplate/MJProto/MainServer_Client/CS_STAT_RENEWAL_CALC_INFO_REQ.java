		...
		try{
			...
			if (pc == null){
				if (this.get_infotype() == 8) {
					SC_STAT_RENEWAL_INFO_NOTI.send_stat_renewal_info(clnt, this.get_option_value(), this.get_class(), this.get_infotype(), this.get_str(), this.get_int(), this.get_wis(), this.get_dex(), this.get_con(), this.get_cha());
				} else if (this.get_infotype() == 1) {
					SC_STAT_RENEWAL_INFO_NOTI.send_stat_renewal_info(clnt, this.get_option_value(), this.get_class(), this.get_infotype(), this.get_str(), this.get_int(), this.get_wis(), this.get_dex(), this.get_con(), this.get_cha());
				}
				//System.out.println(String.format("info %d option %d str %d", this.get_infotype(), this.get_option_value(), this.get_str()));
				return this;
			}
			// TODO : 아래부터 처리 코드를 삽입하십시오. made by MJSoft.
			if (pc.getReturnStat() != 0)
				SC_STAT_RENEWAL_INFO_NOTI.send_stat_renewal_info(clnt, get_class(), get_infotype(), get_str(), get_int(), get_wis(), get_dex(), get_con(), get_cha());
			else
				SC_STAT_RENEWAL_INFO_NOTI.send_stat_renewal_info(clnt, get_option_mask(), get_class(), get_infotype(), get_str(), get_int(), get_wis(), get_dex(), get_con(), get_cha());
			SC_STAT_RENEWAL_BASESTAT_INFO_RES.send_bonus_stat(pc, get_str(), get_int(), get_wis(), get_dex(), get_con(), get_cha());
		} catch (Exception e){
			e.printStackTrace();
		}
		return this;
