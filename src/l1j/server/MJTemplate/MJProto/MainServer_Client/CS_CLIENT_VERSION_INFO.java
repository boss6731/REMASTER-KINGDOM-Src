...
//			clnt.set_version(_client_version);
			...
			/*if (_client_instance_count > MJNetServerLoadManager.DESKTOP_CLIENT_PERMISSION) {
			...
			/*if (Config.version_check) {
				clnt.set_version(_client_version);
				SC_SERVER_VERSION_INFO.send(clnt);
			} else if (_client_version == MJNetSafeLoadManager.NS_CLIENT_VERSION && _client_instance_count >= 1) {
				clnt.set_version(_client_version);
				SC_SERVER_VERSION_INFO.send(clnt);
			} else {
