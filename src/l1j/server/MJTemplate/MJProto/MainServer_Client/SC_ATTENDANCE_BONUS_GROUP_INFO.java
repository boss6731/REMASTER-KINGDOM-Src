	...
	public static void openinfo(L1PcInstance pc) {
		...
		if(MJAttendanceLoadManager.ATTEN_PREMIUM_USE) {
			...
			if(!account.getAttendance_Premium()) {
				pc.sendPackets(SC_ATTENDANCE_BONUS_GROUP_INFO.make_stream(pc, AttendanceGroupType.PREMIUM, item.getItemDescId(), MJAttendanceLoadManager.ATTEN_PREMIUM_NEEDITEMCOUNT, 0), true);
			}
		}
		if(MJAttendanceLoadManager.ATTEN_SPECIAL_USE) {
			...
			if(!account.getAttendance_Special()) {
				pc.sendPackets(SC_ATTENDANCE_BONUS_GROUP_INFO.make_stream(pc, AttendanceGroupType.SPECIAL, item.getItemDescId(), MJAttendanceLoadManager.ATTEN_SPECIAL_NEEDITEMCOUNT, 0), true);
			}
		}
		if(MJAttendanceLoadManager.ATTEN_BRAVE_WARRIOR_USE) {
			...
			if(!account.getAttendance_Brave_Warrior()) {
				pc.sendPackets(SC_ATTENDANCE_BONUS_GROUP_INFO.make_stream(pc, AttendanceGroupType.BRAVE_WARRIOR, item.getItemDescId(), MJAttendanceLoadManager.ATTEN_BRAVE_WARRIOR_NEEDITEMCOUNT, 1), true);
			}
		}
		if(MJAttendanceLoadManager.ATTEN_ADEN_WORLD_USE) {
			...
			if(!account.getAttendance_Aden_World()) {
				pc.sendPackets(SC_ATTENDANCE_BONUS_GROUP_INFO.make_stream(pc, AttendanceGroupType.ADEN_WORLD, item.getItemDescId(), MJAttendanceLoadManager.ATTEN_ADEN_WORLD_NEEDITEMCOUNT, 1), true);
			}
		}
		if(MJAttendanceLoadManager.ATTEN_BRAVERY_MEDAL_USE) {
			...
			if(!account.getAttendance_Bravery_Medal()) {
				pc.sendPackets(SC_ATTENDANCE_BONUS_GROUP_INFO.make_stream(pc, AttendanceGroupType.BRAVERY_MEDAL, item.getItemDescId(), MJAttendanceLoadManager.ATTEN_BRAVERY_MEDAL_NEEDITEMCOUNT, 1), true);
			}
		}
	}
	
	public static ProtoOutputStream make_stream(L1PcInstance pc, AttendanceGroupType type, int descid, int costcount, int i) {
		SC_ATTENDANCE_BONUS_GROUP_INFO ginfo = newInstance();
	...
	public MJIProtoMessage readFrom(l1j.server.MJTemplate.MJProto.IO.ProtoInputStream input) throws IOException{
		while(!input.isAtEnd()){
			...
			switch(tag){
				...
				case 0x0000001A:{
					set_randomGroup((AttendanceBonusRandomGroup)input.readMessage(AttendanceBonusRandomGroup.newInstance()));
					break;
				}
				case 0x00000020:{
