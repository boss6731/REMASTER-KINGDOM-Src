...
public class CS_ATTENDANCE_TAB_OPEN_REQ implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.server.GameClient clnt, byte[] bytes){
		...
		try{
			...
			if(account.getAttendance_Premium() && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.PREMIUM) {
				return this;
			}
			if(account.getAttendance_Special() && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.SPECIAL) {
				return this;
			}
			if (account.getBuff_PC��() == null && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.PC_CAFE) {
				return this;
			}
			if(account.getAttendance_Brave_Warrior() && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVE_WARRIOR) {
				return this;
			}
			if(account.getAttendance_Aden_World() && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.ADEN_WORLD) {
				return this;
			}
			if(account.getAttendance_Bravery_Medal() && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVERY_MEDAL) {
				return this;
			}
			...
			if(!MJAttendanceLoadManager.ATTEN_PREMIUM_USE && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.PREMIUM) {
				return this;
			}
			if(!MJAttendanceLoadManager.ATTEN_SPECIAL_USE && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.SPECIAL) {
				return this;
			}
			if(!MJAttendanceLoadManager.ATTEN_BRAVE_WARRIOR_USE && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVE_WARRIOR) {
				return this;
			}
			if(!MJAttendanceLoadManager.ATTEN_ADEN_WORLD_USE && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.ADEN_WORLD) {
				return this;
			}
			if(!MJAttendanceLoadManager.ATTEN_BRAVERY_MEDAL_USE && AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVERY_MEDAL) {
				return this;
			}
			int itemid = 0;
			int count = 0;
...
//			System.out.println("������: "+itemid + ","+"����: "+count);
			if(!pc.getInventory().checkItem(itemid, count)) {
				...
				pc.sendPackets(String.format("%s(��)�� %s�� �ʿ��մϴ�.", item.getName(), count));
				return this;
			} else {
...
/*			if(AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.PREMIUM) {
				account.setAttendance_Premium(true);
			} else if (AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.SPECIAL) {
				account.setAttendance_Special(true);
			} else if (AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVE_WARRIOR) {
				account.setAttendance_Brave_Warrior(true);
			} else if (AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.ADEN_WORLD) {
				account.setAttendance_Aden_World(true);
			} else if (AttendanceGroupType.fromInt(_group_id) == AttendanceGroupType.BRAVERY_MEDAL) {
				account.setAttendance_Bravery_Medal(true);
			}
*/			
			...
			pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_server(pc, AttendanceGroupType.fromInt(_group_id), i), true);
			pc.sendPackets(SC_ATTENDANCE_TAB_OPEN_ACK.make_stream_db(pc, AttendanceGroupType.fromInt(_group_id), i), true);
		} catch (Exception e){
			e.printStackTrace();
		}
		return this;
