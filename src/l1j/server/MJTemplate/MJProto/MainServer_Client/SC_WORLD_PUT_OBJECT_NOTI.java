...
public class SC_WORLD_PUT_OBJECT_NOTI implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	// 잊섬 마크 표시할지 여부...(트루 : 사용, 펄스 : 사용 안함) (재개편완료 잘사용됨:true)
...
//		}
	...
	public static SC_WORLD_PUT_OBJECT_NOTI newInstance(L1Character c, int emotion, String shop_title, int weapon_sprite, boolean isclan) {
		...
		if (c instanceof L1PcInstance) {
			...
			/*if (pc.hasSkillEffect(L1SkillId.USER_WANTED1)) {
				noti.set_title("\\fe[긴급 수배중][1단계]");
			} else if (pc.hasSkillEffect(L1SkillId.USER_WANTED2)) {
				noti.set_title("\\fe[긴급 수배중][2단계]");
			} else if (pc.hasSkillEffect(L1SkillId.USER_WANTED3)) {
				noti.set_title("\\fe[긴급 수배중][3단계]");
			} else {
...
//			noti.set_boundarylevelindex(value1);
	...
	public static SC_WORLD_PUT_OBJECT_NOTI newInstance(L1Character c, int emotion, String shop_title, int weapon_sprite, int alig) {
		...
		if (c instanceof L1PcInstance) {
			...
			noti.set_desc(parse_name(pc));
			if (alig > 0) {	
				noti.set_title("\\aj놀토_파이널");
			} else if (alig <= 0) {
				noti.set_title("\\ag놀토_파이널");
			}
			int speedData = c.isHaste() ? 1 : 0;
			speedData += (c.isSlow() ? -1 : 0);
			noti.set_speeddata(speedData);
...
//			noti.set_boundarylevelindex(value1);
	...
	private static int writeTeamId(L1PcInstance pc, boolean isClan, int mark) {
		...
		if (instobj != null) {
			...
			if (pc.getMapId() >= 1708 && pc.getMapId() <= 1710 || pc.getMapId() >= 12852 && pc.getMapId() <= 12862
					|| pc.getMapId() == 15871 || pc.getMapId() == 15881 || pc.getMapId() == 15891 || pc.getMapId() == 10500 || pc.getMapId() == 10502) {// 다른사람이 보여지는 기준
	...
	public static SC_WORLD_PUT_OBJECT_NOTI newInstance(L1PcInstance pc, String name, boolean isclan) {
		...
		if (pc.getMapId() == 13005) {
			...
			for (L1PcInstance player : DeathMatchSystem.getInstance().getTeamRedList()) {
				...
				noti.set_pledgename("Red팀");
			}
			for (L1PcInstance player : DeathMatchSystem.getInstance().getTeamBlueList()) {
				noti.set_emblemid(332027079);
				noti.set_pledgename("Blue팀");		
			}
		} else {
	...
	public static SC_WORLD_PUT_OBJECT_NOTI RedKnight_newInstance(L1PcInstance pc) {
		...
		noti.set_pledgename("붉은기사단");// 붉은기사단
		noti.set_team_id(13);// 붉은기사단 혈마크 13
        int value = 0;
		if(pc.getLevel() >= 80)		value = 11;
		else if(pc.getLevel() >= 55)	value = (pc.getLevel() - 25) / 5;
		else if(pc.getLevel() >= 52)	value = 5;
		else if(pc.getLevel() >= 50)	value = 4;
		else if(pc.getLevel() >= 15)	value = pc.getLevel() / 15;
//		noti.set_boundarylevelindex(value);
	...
	private static int get_presentation_npc_action(L1NpcInstance npc) {
		...
		if (npc.getNpcTemplate().is_doppel()) {
			if(sprid == 727 || sprid == 985 || sprid == 98 || sprid == 6632 || sprid == 6634 || sprid == 6636 || sprid == 6638) {
				return 4;
			}
		}
...
//		noti.set_altitude(5);
        ...
        int value = 0;
		if(npc.getLevel() >= 80)		value = 11;
		else if(npc.getLevel() >= 55)	value = (npc.getLevel() - 25) / 5;
		else if(npc.getLevel() >= 52)	value = 5;
		else if(npc.getLevel() >= 50)	value = 4;
		else if(npc.getLevel() >= 15)	value = npc.getLevel() / 15;
		noti.set_boundarylevelindex(value);
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.MJTemplate.MJProto.IO.ProtoInputStream input) throws java.io.IOException{
		while(!input.isAtEnd()){
			...
			switch(tag){
				...
				case 0x00000142:{
					set_companion((SC_WORLD_PUT_OBJECT_NOTI.Companion)input.readMessage(SC_WORLD_PUT_OBJECT_NOTI.Companion.newInstance()));
					break;
				}
				case 0x0000014A:{
				...
				case 0x00000162:{
					set_drop_info((SC_WORLD_PUT_OBJECT_NOTI.DropItemInfo)input.readMessage(SC_WORLD_PUT_OBJECT_NOTI.DropItemInfo.newInstance()));
					break;
				}
				case 0x00000168:{
				...
				case 0x0000019A:{
					set_speed_bonus((SC_SPEED_BONUS_NOTI.Bonus)input.readMessage(SC_SPEED_BONUS_NOTI.Bonus.newInstance()));
					break;
				}
				case 0x000001A2:{
					set_speed_change_rate((SC_SPEED_BONUS_NOTI.Bonus)input.readMessage(SC_SPEED_BONUS_NOTI.Bonus.newInstance()));
					break;
				}
				case 0x000001A8:{
