...
public class SC_STAT_RENEWAL_INFO_NOTI implements l1j.server.MJTemplate.MJProto.MJIProtoMessage{
	public static void send_stat_renewal_info(GameClient client, int stat, int c_class, int info_type, int str, int intel, int wis, int dex, int con, int cha) {
		...
		if (stat == 1 || stat == 16) {
			if (str > 0 && stat == 16) {
				...
				int dmgup = CalcStat.calcDmgup(str) + (pc != null ? CalcStat.calcPureMeleeDmgup(pc.getAbility().getStr()) : 0);
				int hitup = CalcStat.calcHitup(str) + (pc != null ? CalcStat.calcPureMeleeHitup(pc.getAbility().getStr()) : 0);
				int critical = CalcStat.calcDmgCritical(c_class, str) + (pc != null ? CalcStat.calcPureMeleeCritical(pc.getAbility().getStr()) : 0);
			...
			if (intel > 0) {
				...
				int magicdmg = CalcStat.calcMagicDmg(intel) + (pc != null ? CalcStat.calcPureMagicDmgup(pc.getAbility().getInt()) : 0);
				int magichit = CalcStat.calcMagicHitUp(intel) + (pc != null ? CalcStat.calcPureMagicHitup(pc.getAbility().getInt()) : 0);
				int magiccritical = CalcStat.calcMagicCritical(intel) + (pc != null ? CalcStat.calcPureMagicCritical(pc.getAbility().getInt()) : 0);
			...
			if (dex > 0) {
				...
				int bowdmg = CalcStat.calcBowDmgup(dex) + (pc != null ? CalcStat.calcPureMissileDmgup(pc.getAbility().getDex()) : 0);
				int bowhitup = CalcStat.calcBowHitup(dex) + (pc != null ? CalcStat.calcPureMissileHitup(pc.getAbility().getDex()) : 0);
				int bowcritical = CalcStat.calcBowCritical(dex) + (pc != null ? CalcStat.calcPureMissileCritical(pc.getAbility().getDex()) : 0);
			...
			if (cha > 0) {
				...
				int pierceall = CalcStat.calcPierceAll(cha) + (pc != null ? CalcStat.calcPurePierceAll(pc.getAbility().getCha()) : 0);
				int decreasecooltime = CalcStat.calcDecreaseCoolTime(cha) + (pc != null ? CalcStat.calcPureDecreaseCoolTime(pc.getAbility().getCha()) : 0);
				int decreaseCCduration = CalcStat.calcDecreaseCCDuration(cha) + (pc != null ? CalcStat.calcPureDecreaseCCDuration(pc.getAbility().getCha()) : 0);
	...
	public static void send_stat_renewal_info(GameClient client, int c_class, int info_type, int str, int intel, int wis, int dex, int con, int cha) {
		SC_STAT_RENEWAL_INFO_NOTI noti = newInstance();
		L1PcInstance pc = client.getActiveChar();
		noti.set_infotype(info_type);
		if (str > 0) {
			...
			int critical = CalcStat.calcDmgCritical(c_class, str) + CalcStat.calcPureMeleeCritical(pc.getAbility().getStr());
			int strweight = CalcStat.getMaxWeight(str, con);
			str_info.set_damagebonus(dmgup);
			str_info.set_hitbonus(hitup);
			str_info.set_carrybonus(strweight);
		...
		if (intel > 0) {
			...
			int magiccritical = CalcStat.calcMagicCritical(intel) + CalcStat.calcPureMagicCritical(pc.getAbility().getInt());
			int magicbonus = CalcStat.calcMagicBonus(intel);
			int magicdecreasemp = CalcStat.calcDecreaseMp(intel);
			int_info.set_damagebonus(magicdmg);
			int_info.set_hitbonus(magichit);
		...
		if (dex > 0) {
			...
			int bowcritical = CalcStat.calcBowCritical(dex) + CalcStat.calcPureMissileCritical(pc.getAbility().getDex());
			int dexac = CalcStat.calcAc(dex);
			int dexer = CalcStat.ER(dex);
			dex_info.set_damagebonus(bowdmg);
			dex_info.set_hitbonus(bowhitup);
		...
		if (cha > 0) {
			...
			int decreasecooltime = CalcStat.calcDecreaseCoolTime(cha) + CalcStat.calcPureDecreaseCoolTime(pc.getAbility().getCha());
			int decreaseCCduration = CalcStat.calcDecreaseCCDuration(cha) + CalcStat.calcPureDecreaseCCDuration(pc.getAbility().getCha());
			cha_info.set_pierceall(pierceall);
			cha_info.set_decreasecooltime(decreasecooltime);
			cha_info.set_decreaseCCduration(decreaseCCduration);
			cha_info.set_dummy(false);
	...
	public l1j.server.MJTemplate.MJProto.MJIProtoMessage readFrom(l1j.server.MJTemplate.MJProto.IO.ProtoInputStream input) throws java.io.IOException{
		while(!input.isAtEnd()){
			...
			switch(tag){
				...
				case 0x00000012:{
					set_strinfo((SC_STAT_RENEWAL_INFO_NOTI.STR_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.STR_INFO.newInstance()));
					break;
				}
				case 0x0000001A:{
					set_intinfo((SC_STAT_RENEWAL_INFO_NOTI.INT_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.INT_INFO.newInstance()));
					break;
				}
				case 0x00000022:{
					set_wisinfo((SC_STAT_RENEWAL_INFO_NOTI.WIS_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.WIS_INFO.newInstance()));
					break;
				}
				case 0x0000002A:{
					set_dexinfo((SC_STAT_RENEWAL_INFO_NOTI.DEX_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.DEX_INFO.newInstance()));
					break;
				}
				case 0x00000032:{
					set_coninfo((SC_STAT_RENEWAL_INFO_NOTI.CON_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.CON_INFO.newInstance()));
					break;
				}
				case 0x0000003A:{
					set_chainfo((SC_STAT_RENEWAL_INFO_NOTI.CHA_INFO)input.readMessage(SC_STAT_RENEWAL_INFO_NOTI.CHA_INFO.newInstance()));
					break;
				}
				default:{
					return this;
