...
public class MJSpellProbabilityInfo {
	...
	int calc_probability(L1PcInstance pc, L1Character target, int attacker_int, int target_mr) {
		...
		case COUNTER_BARRIER: // 카운터 배리어
			int probability_veteran = Config.MagicAdSetting_Knight.COUNTER_VETERAN; // 카운터 배리어 : 베테랑
			if (pc != null && pc.isPassive(MJPassiveID.COUNTER_BARRIER_VETERAN.toInt())) {
				int lvl = pc.getLevel();
				if (lvl >= 85){ //카운터 배이러 베테랑 본섭화 
					probability += lvl - 84;
				}
...
//				}
		...
		if (_is_loggin) {
			if (pc.isGm()) {
				System.out.println(String.format("스킬 : %s(%d,%d~%d) 공격자:%s(%d), 타겟:%s(%d), 확률:%d", _description,
						_skill_id, _min_probability, _max_probability, pc.getName(), PiercePoint, target.getName(),
						ResisPoint, probability));
