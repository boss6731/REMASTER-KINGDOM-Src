...
public abstract class MJSpawnInfo {
	...
	public L1NpcInstance do_spawn() {
		try {
			delete_instance();
			L1Npc template = NpcTable.getInstance().getTemplate(get_npc_id());
			if(template == null) {
				System.out.println(String.format("MJSpawnInfo::do_spawn()...\r\nnot found npc template information... npc id : %s", get_npc_id()));
				return null;
			}
			...
			npc.setId(get_spawn_id());
			npc.setHeading(MJRnd.next(8));
			npc.setMap((short)get_mapid());
			npc.setMovementDistance(get_movement_distance());
			npc.setRest(false);
			update_location(npc);
			if (npc instanceof L1MonsterInstance) {
				...
				merchant.getMap().setPassable(merchant.getLocation(), false);
			}
			npc.setRespawn(false);
			npc.set_spawn_ex(this);
			...
			if (get_group_id() != 0) {
				L1MobGroupSpawn.getInstance().doSpawn(npc, get_group_id(), spawntype, false);
			}
			...
			System.out.println(String.format("MJSpawnInfo::do_spawn()...\r\nspawn id : %d / npc id : %d", get_spawn_id(), get_npc_id()));
			e.printStackTrace();
		}
		return null;
	}
