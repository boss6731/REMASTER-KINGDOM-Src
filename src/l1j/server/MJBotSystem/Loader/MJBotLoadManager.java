...
public class MJBotLoadManager {
	...
	public static void commands(L1PcInstance gm, String param){
		try{
			...
			gm.sendPackets(new S_SystemMessage("[1.查詢][2.創建機器人][3.刪除機器人][4.重新加載][5.指定魔法娃娃階段]"));
		}
	}
	...
	public static void statusCommands(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			...
			switch(argsQ.poll()){
				case 1:{
					if(argsQ.isEmpty()){
						gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 1 1 [腦袋ID]"));
						return;
					}
					int num = argsQ.poll();
					MJBotBrain brn = MJBotBrainLoader.getInstance().get(num);
					if(brn == null)
						gm.sendPackets(new S_SystemMessage(String.format("無法找到腦袋ID(%d)。", num)));
					else
				...
				case 2:{
					if(argsQ.isEmpty()){
						gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 1 2 [庫存物品ID]"));
						return;
					}
					int num = argsQ.poll();
					MJBotInvItem item = MJBotInvItemLoader.getInstance().get(num);
					if(item == null)
						gm.sendPackets(new S_SystemMessage(String.format("無法找到庫存物品ID(%d)。", num)));
					else
				...
				case 3:{
					if(argsQ.isEmpty()){
						gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 1 3 [技能ID]"));
						return;
					}
					int num = argsQ.poll();
				...
				case 4:{
					if(argsQ.isEmpty()){
						gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 1 4 [類型]"));
						return;
					}
					int num = argsQ.poll();
			...
			gm.sendPackets(new S_SystemMessage(
					"使用方法 .人工智能 1 [選擇]" +"[1. 腦袋][2. 物品][3. 技能][4. 機器人現狀]"
			)
			);
	...
	public static void delBotItem(L1PcInstance gm, int targetId){
		...
		if(body.getAI() == null){
			gm.sendPackets(new S_SystemMessage(String.format("[%s]不是機器人.", body.getName())));
			return;
		}

		gm.sendPackets(new S_SystemMessage(String.format("正在刪除[%s].", body.getName())));
	...
	private static void createSingleBot(L1PcInstance gm, ArrayDeque<Integer> argsQ, boolean is_fixed_location){
		try{
			...
			int			lvl		= argsQ.poll();
			if(type == null){
				gm.sendPackets(new S_SystemMessage(String.format("未知類型。[ %s ]", MJBotType.enumString())));
				return;
			}
			
			int x = gm.getX() + MJCommons.HEADING_TABLE_X[gm.getHeading()];
			int y = gm.getY() + MJCommons.HEADING_TABLE_Y[gm.getHeading()];
			MJBotLastError err = MJAIScheduler.getInstance().setSchedule(bid, type, lvl, x, y, gm.getMapId(), is_fixed_location);
			if(err.ai == null && err.message != null)
				gm.sendPackets(new S_SystemMessage(err.message));
			else if (type == MJBotType.FISH)
				gm.sendPackets(new S_SystemMessage("釣魚機器人已創建。"));
			else {
				gm.sendPackets(new S_SystemMessage(String.format("[%s]已創建。", err.ai.getBody().getName())));
				type.add(err.ai);
			}
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 2 1 \n[腦袋ID][類型][等級]"));
		}
	}
	...
	private static void createSingleRandomBot(L1PcInstance gm, ArrayDeque<Integer> argsQ, boolean is_fixed_location){
		try{
			...
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 2 2 \n[類型][等級]"));
		}
	}
	...
	private static void createMultipleBots(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			...
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 2 3 \n[腦袋ID][類型][等級][數量]"));
		}
	}
	...
	private static void createMultipleRandomBots(L1PcInstance gm, ArrayDeque<Integer> argsQ) {
		try {
			...
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 2 4 \n[類型][等級][數量]"));
		}
	}

	
	public static <gm> void removeCommands(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			if(argsQ.isEmpty())
				throw new Exception("");
			...
			gm sendPackets;(new S_SystemMessage("使用方法 .人工智能 3\n [選項][1 選擇刪除][2 全部刪除]");
		}
	}
	...
	private static void removeSingleBot(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			...
			if(ai == null){
				gm.sendPackets(new S_SystemMessage(String.format("[%d] 是不存在的機器人編號。", num)));
			} else {
				if (ai.getBody() != null)
					gm.sendPackets(new S_SystemMessage(String.format("正在刪除 [%s]。", ai.getBody().getName())));
				else
					gm.sendPackets(new S_SystemMessage(String.format("正在刪除編號為 [%d] 的機器人。", num)));
				MJAIScheduler.getInstance().removeSchedule(ai);
			}
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 3 1 \n[類型][機器人編號]"));
		}
	}
	...
	private static void removeMultipleBots(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			...
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 3 2\n[類型]"));
		}
	}
		...
		try{
			...
			switch(argsQ.poll()){
				case 1:
					...
					gm.sendPackets(new S_SystemMessage("[機器人腦重載完成]"));
					break;
				case 2:
					MJBotNameLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人名稱重載完成]"));
					break;
				case 3:
					MJBotInvItemLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人庫存物品重載完成]"));
					break;
				case 4:
					MJBotLocationLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人位置重載完成]"));
					break;
				case 5:
					MJBotSpellLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人法術重載完成]"));
					break;
				case 6:
					MJBotMentLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人語言重載完成]"));
					break;
				case 7:
					getInstance().loadConfig();
					gm.sendPackets(new S_SystemMessage("[機器人配置重載完成]"));
					break;
				case 8:
					MJBotDropItemLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人掉落物品重載完成]"));
					break;
				case 9:
					MJBotBossNotifierLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人老板通知器重載完成]"));
					break;
				case 10:
					MJBotDollLoader.reload();
					gm.sendPackets(new S_SystemMessage("[機器人娃娃重載完成]"));
					break;
			case 11:
			...
			gm.sendPackets(new S_SystemMessage(
			"使用方法 .人工智能 4 [選項]\n" +
				"[1 腦] [2 名稱] [3 庫存物品] [4 坐標] [5 法術]\n" +
				"[6 語言] [7 配置] [8 掉落物品] [9 老板通知] [10 娃娃]\n" +
				"[11 血盟] [12 全部]")
				);
		}
	}
	...
	public static void updateDollsLevelCommands(L1PcInstance gm, ArrayDeque<Integer> argsQ){
		try{
			...
			gm.sendPackets(new S_SystemMessage("使用方法 .人工智能 5 [階段]"));
		}
	}
