...
public class L1ClanDugeon implements Runnable {
	...
	public L1ClanDugeon(int mapid, int type, L1PcInstance pc) {
		_DugeonType = type;
		_mapnum = mapid;
		Portal = L1SpawnUtil.spawnnpc(pc.getX(), pc.getY(),pc.getMapId(), type == 3 ? 120620 : 120599 + type, 0, ClanDugeon.getInstance().ClanDugeonInfo.waitingtimes *  1000, _mapnum);
		initdoor();
		GeneralThreadPool.getInstance().schedule(new timer(), ClanDugeon.getInstance().ClanDugeonInfo.waitingtimes * 1000);
	}
				...
				if(_DugeonType == 3) {
					if(Dooropencheack()) {
						if(stage == 1) {
							...
							_HurryCainPortal[0] = L1SpawnUtil.spawnnpc(32784, 32956, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
							_HurryCainPortal[1] = L1SpawnUtil.spawnnpc(32793, 32960, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
							_HurryCainPortal[2] = L1SpawnUtil.spawnnpc(32791, 32970, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
							_HurryCainPortal[3] = L1SpawnUtil.spawnnpc(32784, 32971, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
							_HurryCainPortal[4] = L1SpawnUtil.spawnnpc(32779, 32967, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
							_HurryCainPortal[5] = L1SpawnUtil.spawnnpc(32778, 32960, (short)_mapnum, 120815, 3, 3600 * 1000, 0);
					...
					if(_DugeonType == 3 && subround == 2 && round <= 4) {
						for(int i =0; i < stageunit.size(); i++) {
							while(!stageunit.get(i).isDead() && counter <= 6000 && getMembersCount() > 0){
								...
								if (counter >= 6000) {
									sendMessage("該回合限制時間已超過10分鐘，強制返回。", 5000);
									quit();
									break;
								}
				...
				if(stageboss == null) {
					...
					for(int i = 0; i < stageboss.length; i++) {
						while(!stageboss[i].isDead()) {
							...
							if (counter > 6000) {
								sendMessage("該回合首領限制時間已超過10分鐘，強制返回。", 5000);
								quit();
								break;
							}
	...
	private void waitForNextRound(int curRound) throws InterruptedException {
		...
		for (int i = 0; i < wait; i++) {
			Thread.sleep(1000);
			if((_DugeonType == 1 && curRound < 3 || _DugeonType == 2 && curRound < 11 || _DugeonType == 3 && curRound < 5) && !_close) {
				switch(ClanDugeon.getInstance().ClanDugeonInfo.waitingtimer - i) {
					case 60:
						if(_DugeonType == 1) {
							removeRetiredMembers();
							sendMessage("距離 " + (curRound + 1) + " 波怪物出現還剩 1 分鐘。", 5000);
					...
					case 30:
						if(_DugeonType == 1) {
							removeRetiredMembers();
							sendMessage("距離第 " + (curRound + 1) + " 波怪物出現還剩 30 秒。", 5000);
						} else if(_DugeonType == 2) {
							removeRetiredMembers();
						} else {
							removeRetiredMembers();
					...
					case 20:
						if(_DugeonType == 1) {
							removeRetiredMembers();
							sendMessage("距離第 " + (curRound + 1) + " 波怪物出現還剩 20 秒。", 5000);
						} else if(_DugeonType == 2) {
							removeRetiredMembers();
						} else {
							removeRetiredMembers();
					...
					case 10:
						if(_DugeonType == 1) {
							removeRetiredMembers();
							sendMessage("距離第 " + (curRound + 1) + " 波怪物出現還剩 10 秒。", 5000);
						} else if(_DugeonType == 2) {
							removeRetiredMembers();
						} else {
							removeRetiredMembers();
							sendMessage("距離第 " + (curRound + 1) + " 區域啟動還剩 10 秒。", 5000);
						}
						break;
					case 1:
						if(_DugeonType == 1) {
				...
				switch(ClanDugeon.getInstance().ClanDugeonInfo.waitingtimer - i) {
					case 60:
						removeRetiredMembers();
						sendMessage("請返回，60 秒後將強制返回。", 5000);
						break;
					case 30:
						removeRetiredMembers();
						sendMessage("請返回，30 秒後將強制返回。", 5000);
						break;
					case 20:
						removeRetiredMembers();
						sendMessage("請返回，20 秒後將強制返回。", 5000);
						break;
					case 10:
						removeRetiredMembers();
						sendMessage("請返回，10 秒後將強制返回。", 5000);
						break;
					case 1:
						removeRetiredMembers();
						sendMessage("請返回，稍後將強制返回。", 1000);
						break;
					default:
						removeRetiredMembers();
						break;
	...
	private void countDown() throws InterruptedException {
		...
		if(_DugeonType == 1 || _DugeonType == 2) {
			sendMessage("距離一群怪物出現還有1分鐘。", 5000); // 10秒前
			Thread.sleep(30000); // 睡眠30秒
			removeRetiredMembers();

			sendMessage("距離一群怪物出現還有30秒。", 5000); // 10秒前
			Thread.sleep(10000); // 睡眠10秒
			removeRetiredMembers();

			sendMessage("距離一群怪物出現還有20秒。", 1000); // 5秒前
			Thread.sleep(10000); // 睡眠10秒
			removeRetiredMembers();

			sendMessage("距離一群怪物出現還有10秒。", 1000); // 4秒前
			Thread.sleep(10000); // 睡眠10秒
			removeRetiredMembers();

			sendMessage("怪物即將出現。", 1000); // 3秒前
			removeRetiredMembers();
			for (L1PcInstance pc : getMembersArray()) {
				pc.sendPackets(S_DisplayEffect.newInstance(10));
				pc.sendPackets(S_DisplayEffect.newInstance(7));
			...
			sendMessage("亞丁勇士集結中....", 5000); // 10秒前
			Thread.sleep(8000);
			removeRetiredMembers();
			sendMessage("偵測到黑暗龍的使者正率領軍隊準備入侵...", 1000); // 10秒前
			Thread.sleep(8000);
			sendMessage("為了亞丁的和平，在入侵開始前，我們將找出並消滅黑暗龍的使者....", 1000); // 5秒前
			removeRetiredMembers();
			Thread.sleep(8000);
			sendMessage("必須突破總共四個地區，才能到達黑暗龍的使者....", 1000); // 4秒前
			removeRetiredMembers();
			Thread.sleep(8000);
			sendMessage("實力...運氣...一切都要完備....", 1000); // 3秒前
			Thread.sleep(8000);
			sendMessage("祝勇士們旅程順利....", 1000); // 3秒前
			Thread.sleep(10000);
			sendMessage("距離第一地區激活還有10秒....", 1000); // 3秒前
			removeRetiredMembers();
			Thread.sleep(10000);
			sendMessage("怪物即將出現。", 1000); // 3秒前
			removeRetiredMembers();
	...
	private void quit() {
		_close = true;
		System.out.println("血盟地下城結束 地圖編號 : " + _mapnum);
		L1PcInstance[] temp = getMembersArray();
		for (int i = 0; i < temp.length; i++) {
			removeMember(temp[i]);
		}
